...
class DeviceSerialClass:
    def __init__(self):
        ...
        if self.Unidirectional == 'False' or self._DeviceID != '00':
            self.AddMatchString(re.compile(b'c [0-9a-f]{2} OK(0[1269])x', re.I), self.__MatchAspectRatio, None)
            self.AddMatchString(re.compile(b'e [0-9a-f]{2} OK(0[01])x', re.I), self.__MatchAudioMute, None)
            self.AddMatchString(re.compile(b'm [0-9a-f]{2} OK(0[01])x', re.I), self.__MatchExecutiveMode, None)
            self.AddMatchString(re.compile(b'b [0-9a-f]{2} OK(00|01|10|11|20|40|60|90|91)x', re.I), self.__MatchInput, None)
            self.AddMatchString(re.compile(b'l [0-9a-f]{2} OK(0[01])x', re.I), self.__MatchOnScreenDisplay, None)
            self.AddMatchString(re.compile(b'a [0-9a-f]{2} OK(0[01])x', re.I), self.__MatchPower, None)
            self.AddMatchString(re.compile(b'd [0-9a-f]{2} OK([01]{2})x'), self.__MatchVideoMute, None)
            self.AddMatchString(re.compile(b'f [0-9a-f]{2} OK([0-9a-fA-F]{2})x', re.I), self.__MatchVolume, None)
            self.AddMatchString(re.compile(b'([cembladfu]) [0-9a-f]{2} NG(.*)x', re.I), self.__MatchError, None)
    ...
    def __SetHelper(self, command, commandstring, value, qualifier):
        ...
        if self.Unidirectional == 'True' or command == 'UserDefinedCommand' or self._DeviceID == '00':
    ...
    def __MatchError(self, match, tag):
        ...
        self.Error(['{0} Error: {1}'.format(State[match.group(1).decode().lower()], match.group(2).decode())])
...
class DeviceEthernetClass:
    ...
    def SetPowerOff(self, value, qualifier):
        ...
        PowerOffCmdString = 'ka {} 00\r'.format(self._DeviceID)
