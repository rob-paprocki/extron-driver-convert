...
class DeviceClass:
    def __init__(self):
        ...
        if self.Unidirectional == 'False':
            ...
            self.AddMatchString(re.compile(b'\xAA\xFF[\x00-\xFF]\x03\x41\x14(\x14|\x18|\x08|\x20|\x21|\x22|\x23|\\x24|\x1F|\x30|\x40|\x25)[\x00-\xFF]'), self.__MatchInput, None)
    ...
    def DeviceID(self, value):
        if value == 'Broadcast':
            ...
            self.Error(['DeviceID variable is set to an invalid value. It should be a number between 0-99 or Broadcast'])
    ...
    def SetAspectRatio(self, value, qualifier):
        ...
        cks = int(hex(0x18 + self._DeviceID + 0x01 + AspectRatioState[value])[-2:], 16)
        AspectRatioCmdString = pack('>BBBBBB', 0xAA, 0x18, self._DeviceID, 0x01, AspectRatioState[value], cks)
    ...
    def SetAudioMute(self, value, qualifier):
        ...
        cks = int(hex(0x13 + self._DeviceID + 0x01 + AudioMuteState[value])[-2:], 16)
        AudioMuteCmdString = pack('>BBBBBB', 0xAA, 0x13, self._DeviceID, 0x01, AudioMuteState[value], cks)
    ...
    def SetButtonLock(self, value, qualifier):
        ...
        cks = int(hex(0x5F + self._DeviceID + 0x01 + ButtonLockState[value])[-2:], 16)
        ButtonLockCmdString = pack('>BBBBBB', 0xAA, 0x5F, self._DeviceID, 0x01, ButtonLockState[value], cks)
    ...
    def __MatchInput(self, match, tag):
        ...
        value = InputState[ord(match.group(1))]
    ...
    def SetPIPMode(self, value, qualifier):
        ...
        cks = int(hex(0x3C + self._DeviceID + 0x01 + PIPModeState[value])[-2:], 16)
        PIPModeCmdString = pack('>BBBBBB', 0xAA, 0x3C, self._DeviceID, 0x01, PIPModeState[value], cks)
