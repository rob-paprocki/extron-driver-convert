...
class DeviceClass:
    def __init__(self):
        ...
        self.Commands = {
            ...
            'FaderPolarity': {'Parameters': ['Index'], 'Status': {}},
            'Heartbeat': {'Status': {}},
            'RecallPreset': {'Status': {}},
            'RecallSnapshot': {'Status': {}},
        ...
        if self.Unidirectional == 'False':
            self.AddMatchString(re.compile(b'(?:NOTIFY|OK) set MTX:Index_(\d{1,4}) 0 0 (-?\d{1,5}).*\n'), self.__MatchFunction, None)
            self.AddMatchString(re.compile(b'OK get MTX:Index_(\d{1,4}) 0 0 (-?\d{1,5})\n'), self.__MatchFunction, None)
            self.AddMatchString(re.compile(b'OK ssrecall ([0-9]{1,3})\n'), self.__MatchRecallPreset, None)
            self.AddMatchString(re.compile(b'OK sscurrent ([0-9]{1,3}) (?:unmodified|modified)\n'), self.__MatchRecallPreset, None)
            self.AddMatchString(re.compile(b'NOTIFY sscurrent ([0-9]{1,3})\n'), self.__MatchRecallPreset, None)
    ...
    def __MatchFunction(self, match, tag):
        ...
        if command == 'FaderLevel':
            ...
            if not (-138.01 <= value <= 10):
                return
        elif command in {'FaderMute', 'FaderPolarity'}:
            if value not in {'0', '1'}:
                return
            value = self.states_map[command][value]
        elif command == 'Router':
            value_as_int = int(value)
            if not (0 <= value_as_int <= 64):
                return
    ...
    def SetFaderLevel(self, value, qualifier):
        ...
        if 1 <= index <= 1048 and -138.01 <= value <= 10:
            FaderLevelCmdString = 'set MTX:Index_{i} 0 0 {v}\n'.format(i=index, v=int(value * 100))
    ...
    def UpdateFaderLevel(self, value, qualifier):
        ...
        if 1 <= index <= 1048 and self.add('FaderLevel', index):
            FaderLevelCmdString = 'get MTX:Index_{i} 0 0\n'.format(i=index)
    ...
    def SetFaderMute(self, value, qualifier):
        ...
        if 1 <= index <= 1048:
            FaderMuteCmdString = 'set MTX:Index_{i} 0 0 {v}\n'.format(i=index, v=ValueStateValues[value])
    ...
    def UpdateFaderMute(self, value, qualifier):
        ...
        if 1 <= index <= 1048 and self.add('FaderMute', index):
            FaderMuteCmdString = 'get MTX:Index_{i} 0 0\n'.format(i=index)
            self.__UpdateHelper('FaderMute', FaderMuteCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for UpdateFaderMute')

    def SetFaderPolarity(self, value, qualifier):
        ...
        if 1 <= index <= 1048:
            FaderPolarityCmdString = 'set MTX:Index_{i} 0 0 {v}\n'.format(i=index, v=ValueStateValues[value])
            self.__SetHelper('FaderPolarity', FaderPolarityCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetFaderPolarity')

    def UpdateFaderPolarity(self, value, qualifier):

        index = int(qualifier['Index'])

        if 1 <= index <= 1048 and self.add('FaderPolarity', index):
            FaderPolarityCmdString = 'get MTX:Index_{i} 0 0\n'.format(i=index)
            self.__UpdateHelper('FaderPolarity', FaderPolarityCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for UpdateFaderPolarity')
    ...
    def SetRecallPreset(self, value, qualifier):
        ...
        if 1 <= int(value) <= 50:
            RecallPresetCmdString = 'ssrecall {v}\n'.format(v=int(value))
    ...
    def SetRecallSnapshot(self, value, qualifier):
        ...
        if 1 <= value <= 1048:
            RecallSnapshotCmdString = 'ssrecall_ex list {v}\n'.format(v=value)
    ...
    def SetRouter(self, value, qualifier):
        ...
        if 1 <= index <= 1048 and 0 <= int(value) <= 64:
            RouterCmdString = 'set MTX:Index_{i} 0 0 {v}\n'.format(i=index, v=int(value))
    ...
    def UpdateRouter(self, value, qualifier):
        ...
        if 1 <= index <= 1048 and self.add('Router', index):
            RouterCmdString = 'get MTX:Index_{i} 0 0\n'.format(i=index)
