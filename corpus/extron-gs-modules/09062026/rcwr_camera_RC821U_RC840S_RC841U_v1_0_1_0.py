...
class DeviceClass:
    ...
    def SetFocusMode(self, value, qualifier):
        ...
        if value in ValueStateValues:
            FocusModeCmdString = pack('6B', self._DeviceID, 0x01, 0x04, ValueStateValues[value][0], ValueStateValues[value][1], 0xFF)
    ...
    def SetPanTilt(self, value, qualifier):
        ...
        if 1 <= qualifier['Pan Speed'] <= 24 and 1 <= qualifier['Tilt Speed'] <= 20 and value in ValueStateValues:
            if value == 'Home' or value == 'Reset':
                ...
                PanTiltCmdString = pack('>6BHB', self._DeviceID, 0x01, 0x06, 0x01, qualifier['Pan Speed'], qualifier['Tilt Speed'], ValueStateValues[value], 0xFF)
    ...
    def SetTracking(self, value, qualifier):
        ...
        if value in ValueStateValues:
            TrackingCmdString = pack('6B', self._DeviceID, 0x0A, 0x11, 0x54, ValueStateValues[value], 0xFF)
    ...
    def __CheckResponseForErrors(self, sourceCmdName, response):
        ...
        if response:
            if 0x60 <= response[-3] <= 0x60 and response[-2] in ErrorMatchValues:
                self.Error(['Error: {}, following command: {}.'.format(ErrorMatchValues[response[-2]], sourceCmdName)])
                response = ''
