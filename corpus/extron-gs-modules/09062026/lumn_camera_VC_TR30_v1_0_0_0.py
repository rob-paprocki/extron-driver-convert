...
class DeviceClass:
    ...
    def DeviceID(self, value):
        if self.ConnectionType == 'Serial':
            if 1 <= int(value) <= 7:
                self._DeviceID = int(value) + 0x81
            else:
                self.Error(['Invalid Devic ID Parameter'])
        else:
            self._DeviceID = 0x81
    ...
    def SetAutoFramingTrigger(self, value, qualifier):

        AutoFramingTriggerCmdString = self.set_header(pack('>7B', self._DeviceID, 0x01, 0x0B, 0x04, 0x00, 0x00, 0xFF))
        self.__SetHelper('AutoFramingTrigger', AutoFramingTriggerCmdString, value, qualifier)
    ...
    def SetPreset(self, value, qualifier):
        ...
        if action in ActionStates and 1 <= int(value) <= 128:
            PresetCmdString = self.set_header(pack('>7B', self._DeviceID, 0x01, 0x04, 0x3F, ActionStates[action], int(value) - 1, 0xFF))
