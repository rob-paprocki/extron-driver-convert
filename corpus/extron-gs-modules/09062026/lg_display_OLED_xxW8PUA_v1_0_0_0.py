...
class DeviceSerialClass:
    def __init__(self):
        ...
        if self.Unidirectional == 'False':
            ...
            self.AddMatchString(re.compile(b'b ([0-9a-f]{2}) OK(00|01|10|11|20|90|91|92|93)x',re.I), self.__MatchInput, None)
            self.AddMatchString(re.compile(b'a ([0-9a-f]{2}) OK0([01])x',re.I), self.__MatchPower, None)
            self.AddMatchString(re.compile(b'd ([0-9a-f]{2}) OK(00|01|10)x',re.I), self.__MatchScreenMute, None)
            self.AddMatchString(re.compile(b'f ([0-9a-f]{2}) OK([0-9a-f]{2})x',re.I), self.__MatchVolume, None)
            self.AddMatchString(re.compile(b'([cembadf]) ([0-9a-f]{2}) NG(.*?)x',re.I), self.__MatchError, None)
    ...
    def __MatchAspectRatio(self, match, tag):
        ...
        if self.DeviceID == match.group(1).decode().upper():
    ...
    def __MatchError(self, match, tag):
        ...
        cmd = CmdTags[match.group(1).decode().lower()]
        ID = match.group(2).decode().upper()
        code = match.group(3).decode()
        self.Error(['{0} Error, Device ID {1}: {2}'.format(cmd, ID, code)])
