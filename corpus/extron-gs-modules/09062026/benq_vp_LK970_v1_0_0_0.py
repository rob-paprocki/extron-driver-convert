...
class DeviceClass:
    def __init__(self):
        ...
        if self.Unidirectional == 'False':
            self.AddMatchString(re.compile(b'\*ASP=(4:3|16:9|2.35|AUTO)#'), self.__MatchAspectRatio, None)
            self.AddMatchString(re.compile(b'\*SOUR=(RGB|HDMI|HDMI2|HDBASET)#'), self.__MatchInput, None)
            self.AddMatchString(re.compile(b'\*LAMPM=(ECO|LNOR|DIMMING|CUSTOM)#'), self.__MatchLampMode, None)
            self.AddMatchString(re.compile(b'\*LTIM=([0-9]{1,5})'), self.__MatchLampUsage, None)
            self.AddMatchString(re.compile(b'\*APPMOD=(PRESET|SRGB|BRIGHT|CINE|DICOM|USER1|USER2)#'), self.__MatchPictureMode, None)
