...
class DeviceSerialClass:
    def __init__(self):
        ...
        if self.Unidirectional == 'False' and self._DeviceID != '00':
            self.AddMatchString(re.compile(b'c [a-f0-9]{2} OK([01][a-f0-9])x', re.I), self.__MatchAspectRatio, None)
            ...
            self.AddMatchString(re.compile(b'b [a-f0-9]{2} OK(90|91|92|93|20|00|10|11|01)x', re.I), self.__MatchInput, None)
...
class DeviceEthernetClass:
    ...
    def KeyCode(self, value):
        self._KeyCode = value
        self.__aes = AESCipher(self._KeyCode)
    ...
    def SetAspectRatio(self, value, qualifier):
        ...
        ValueStateValues = {
            '4:3':      '4by3',
            '16:9':     '16by9',
            'Original': 'setbyoriginal'
        }

        AspectRatioCmdString = self.__aes.encrypt('ASPECT_RATIO {}\r'.format(ValueStateValues[value]))
        self.__SetHelper('AspectRatio', AspectRatioCmdString, value, qualifier)
    ...
    def SetAudioMute(self, value, qualifier):
        ...
        AudioMuteCmdString = self.__aes.encrypt('VOLUME_MUTE {}\r'.format(ValueStateValues[value]))
        self.__SetHelper('AudioMute', AudioMuteCmdString, value, qualifier)
    def SetChannel(self, value, qualifier):
        ...
        ChannelCmdString = self.__aes.encrypt('KEY_ACTION {}\r'.format(ValueStateValues[value]))
        self.__SetHelper('Channel', ChannelCmdString, value, qualifier)
    def SetClosedCaption(self, value, qualifier):

        ClosedCaptionCmdString = self.__aes.encrypt('KEY_ACTION captionsubtitle\r')
        self.__SetHelper('ClosedCaption', ClosedCaptionCmdString, value, qualifier)
    def SetExecutiveMode(self, value, qualifier):
        ...
        ExecutiveModeCmdString = self.__aes.encrypt('REMOTECONTROLER_LOCK {}\r'.format(ValueStateValues[value]))
        self.__SetHelper('ExecutiveMode', ExecutiveModeCmdString, value, qualifier)
    def SetInput(self, value, qualifier):
        ...
        InputCmdString = self.__aes.encrypt('INPUT_SELECT {}\r'.format(ValueStateValues[value]))
        self.__SetHelper('Input', InputCmdString, value, qualifier)
    def SetKeypad(self, value, qualifier):
        ...
        KeypadCmdString = self.__aes.encrypt('KEY_ACTION {}\r'.format(ValueStateValues[value]))
        self.__SetHelper('Keypad', KeypadCmdString, value, qualifier)
    def SetMenuNavigation(self, value, qualifier):
        ...
        MenuNavigationCmdString = self.__aes.encrypt('KEY_ACTION {}\r'.format(ValueStateValues[value]))
        self.__SetHelper('MenuNavigation', MenuNavigationCmdString, value, qualifier)
    def SetOnScreenDisplay(self, value, qualifier):
        ...
        OnScreenDisplayCmdString = self.__aes.encrypt('OSD_SELECT {}\r'.format(ValueStateValues[value]))
        self.__SetHelper('OnScreenDisplay', OnScreenDisplayCmdString, value, qualifier)
    def SetPowerOff(self, value, qualifier):

        PowerOffCmdString = self.__aes.encrypt('POWER off\r')
    ...
    def SetVideoMute(self, value, qualifier):
        ...
        ValueStateValues = {
            'On':               'screenmuteon',
            'Off':              'allmuteoff',
            'On (With OSD)':    'videomuteon'
        }

        VideoMuteCmdString = self.__aes.encrypt('SCREEN_MUTE {}\r'.format(ValueStateValues[value]))
        self.__SetHelper('VideoMute', VideoMuteCmdString, value, qualifier)
    def SetVolume(self, value, qualifier):
        ...
        if ValueConstraints['Min'] <= value <= ValueConstraints['Max']:
            VolumeCmdString = self.__aes.encrypt('VOLUME_CONTROL {}\r'.format(value))
            self.__SetHelper('Volume', VolumeCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetVolume')
...
class AESModeOfOperationCBC(AESBlockModeOfOperation):
    ...
    def encrypt(self, plaintext):
        if len(plaintext) == 0 or len(plaintext) % 16 != 0:
            raise ValueError('plaintext block must be a multiple of 16 bytes')

        plaintext = _string_to_bytes(plaintext)

        to_return = []
        for block in range(0, len(plaintext) // 16):
            precipherblock = [(p ^ l) for (p, l) in zip(plaintext[block * 16:(block + 1) * 16], self._last_cipherblock)]
            self._last_cipherblock = self._aes.encrypt(precipherblock)
            to_return += self._last_cipherblock

        return _bytes_to_string(to_return)

    def decrypt(self, ciphertext):
        if len(ciphertext) == 0 or len(ciphertext) % 16 != 0:
            raise ValueError('ciphertext block must be a multiple of 16 bytes')

        cipherblock = _string_to_bytes(ciphertext)

        to_return = []
        for block in range(0, len(cipherblock) // 16):
            plaintext = [(p ^ l) for (p, l) in zip(self._aes.decrypt(cipherblock[block * 16:(block + 1) * 16]), self._last_cipherblock)]
            self._last_cipherblock = cipherblock[block * 16:(block + 1) * 16]
            to_return += plaintext
...
class AESCipher:
    def __init__(self, keycode):
        if not isinstance(keycode, str):
            raise TypeError('Key Code type must be str')
        if len(keycode) != 8:
            raise ValueError('Key Code must be a length of 8')
        if not keycode.isalnum():
            raise ValueError('Key Code must be alphanumeric')

        self.__key = hashlib.pbkdf2_hmac('sha256', keycode.encode(), b'\x63\x61\xb8\x0e\x9b\xdc\xa6\x63\x8d\x07\x20\xf2\xcc\x56\x8f\xb9', 2 ** 14, 16)

    def encrypt(self, command):
        if not isinstance(command, str) and not isinstance(command, bytes):
            raise TypeError('Command type must be str or bytes')

        initialization_vector = bytes([random.randrange(0, 256) for _ in range(0, 16)])
        encrypted_initialization_vector = AESModeOfOperationECB(self.__key).encrypt(initialization_vector)
        if isinstance(command, str):
            command = command.encode()

        padding_length = 16 - (len(command) % 16)
        padding = struct.pack('B', padding_length)
        command += padding * padding_length

        encrypted_command = AESModeOfOperationCBC(self.__key, initialization_vector).encrypt(command)

        return encrypted_initialization_vector + encrypted_command

    def decrypt(self, ctext):
        initialization_vector = AESModeOfOperationECB(self.__key).decrypt(ctext[:16])
        return AESModeOfOperationCBC(self.__key, initialization_vector).decrypt(ctext[16:])
