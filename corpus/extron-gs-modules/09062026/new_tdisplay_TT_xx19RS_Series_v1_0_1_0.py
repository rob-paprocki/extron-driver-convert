...
class DeviceClass:
    def __init__(self):
        ...
        if self.Unidirectional == 'False':
            self.AddMatchString(re.compile(b'\x7F\x09\x99\xA2\xB3\xC4\x02\xFF\x01\x81([\x00\x01])\xCF'), self.__MatchBacklight, None)
            ...
            self.AddMatchString(re.compile(b'\x7F\x09\x99\xA2\xB3\xC4\x02\xFF\x01\x82([\x00\x01])\xCF'), self.__MatchMute, None)
            self.AddMatchString(re.compile(b'\x7F\x09\x99\xA2\xB3\xC4\x02\xFF\x01\x37([\x00\x01])\xCF'), self.__MatchPower, None)
            self.AddMatchString(re.compile(b'\x7F\x09\x99\xA2\xB3\xC4\x02\xFF\x01\x84([\x00\x01])\xCF'), self.__MatchScreenLock, None)
            self.AddMatchString(re.compile(b'\x7F\x09\x99\xA2\xB3\xC4\x02\xFF\x01\x33([\x00-\x64])\xCF'), self.__MatchVolume, None)
            self.AddMatchString(re.compile(b'\x7F\x09\x99\xA2\xB3\xC4\x02\xFF\x01\x83([\x00\x01])\xCF'), self.__MatchWhiteboard, None)
    ...
    def UpdateBacklight(self, value, qualifier):
        ...
        BacklightCmdString = b'\x7F\x08\x99\xA2\xB3\xC4\x02\xFF\x01\x81\xCF'
    ...
    def UpdateScreenLock(self, value, qualifier):

        ScreenLockCmdString = b'\x7F\x08\x99\xA2\xB3\xC4\x02\xFF\x01\x84\xCF'
        self.__UpdateHelper('ScreenLock', ScreenLockCmdString, value, qualifier)

    def __MatchScreenLock(self, match, tag):
        ...
        value = ValueStateValues[match.group(1)]
        self.WriteStatus('ScreenLock', value, None)
    ...
    def SetVolume(self, value, qualifier):
        ...
        if ValueConstraints['Min'] <= value <= ValueConstraints['Max']:
            VolumeCmdString = b''.join([b'\x7F\x08\x99\xA2\xB3\xC4\x02\xFF\x05', value.to_bytes(1, 'big'), b'\xCF'])
    ...
    def UpdateWhiteboard(self, value, qualifier):

        WhiteboardCmdString = b'\x7F\x08\x99\xA2\xB3\xC4\x02\xFF\x01\x83\xCF'
        self.__UpdateHelper('Whiteboard', WhiteboardCmdString, value, qualifier)

    def __MatchWhiteboard(self, match, tag):
        ...
        value = ValueStateValues[match.group(1)]
        self.WriteStatus('Whiteboard', value, None)
