...
class DeviceClass:
    def __init__(self):
        ...
        self.RelayListRegEx = re.compile(r'P([1-8])=(ON|OFF)')
    ...
    def UpdateActivePowerStatus(self, value, qualifier):
        ...
        ActivePowerStatusCmdString = 'linesensor all "0,1,2,3" show\r'
        res = self.__UpdateHelper('ActivePowerStatus', ActivePowerStatusCmdString, value, qualifier)
        if res:
            regex_result = re.match(self.ActivePowerRegEx, res)

            try:    # Total Active Energy Status
                value = round(float(regex_result.group(1)) / 1000, 3)  # Converting Wh to kWh, see Web page
            ...
            try:    # Active Power Status
                value = round(float(regex_result.group(2)), 1)
            ...
            try:    # Current Status
                value = round(float(regex_result.group(4)), 3)
    ...
    def SetRelay(self, value, qualifier):
        ...
        if 1 <= int(qualifier['Number']) <= 8 and value in ValueStateValues:
            RelayCmdString = 'port {} {}\r'.format(qualifier['Number'], ValueStateValues[value])
