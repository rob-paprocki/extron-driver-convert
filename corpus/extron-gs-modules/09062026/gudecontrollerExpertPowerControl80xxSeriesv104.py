...
class DeviceClass:
    def __init__(self):
        ...
        self.Debug = False
        self.Models = {
            'Expert Power Control 8041-1': self.gude_20_4396_12,
            'Expert Power Control 8041-2': self.gude_20_4396_12,
            'Expert Power Control 8041-3': self.gude_20_4396_12,
            'Expert Power Control 8045-1': self.gude_20_4396_12,
            'Expert Power Control 8045-2': self.gude_20_4396_12,
            'Expert Power Control 8045-3': self.gude_20_4396_12,
            'Expert Power Control 8045-1 R2': self.gude_20_4396_12,
            'Expert Power Control 8021-1': self.gude_20_4396_4,
            'Expert Power Control 8021-2': self.gude_20_4396_4,
            'Expert Power Control 8021-3': self.gude_20_4396_4,
            'Expert Power Control 8025-1': self.gude_20_4396_4,
            'Expert Power Control 8025-2': self.gude_20_4396_4,
            'Expert Power Control 8025-3': self.gude_20_4396_4,
            'Expert Power Control 8031-1': self.gude_20_4396_8,
            'Expert Power Control 8031-2': self.gude_20_4396_8,
            'Expert Power Control 8031-3': self.gude_20_4396_8,
            'Expert Power Control 8035-1': self.gude_20_4396_8,
            'Expert Power Control 8035-2': self.gude_20_4396_8,
            'Expert Power Control 8035-3': self.gude_20_4396_8,
        }
        ...
        self.Commands = {
            ...
            'IndividualCurrentStatus': {'Parameters': ['Port'], 'Status': {}},
        ...
        ap_regex_str = 'L=1,L=\".*?\",OVP=([01]),0=\"(\d+?\.?\d*?)Wh\",1=\"(\d+?\.?\d*?)W\",2=\"(\d+?\.?\d*?)V\",3=\"(\d+?\.?\d*?)A\",21=\"(\d+?\.?\d*?)A\"\r\n>'
        self.ActivePowerRegEx = compile(ap_regex_str)
        self.ProbeNumberRegEx = compile('E=([12])')
        self.ProbeValuesRegEx = compile('([01345])=\"(\d*(?:\.\d+)?).+?\"')
    ...
    def UpdateActivePowerStatus(self, value, qualifier):
        ...
        if res:
            regex_result = search(self.ActivePowerRegEx, res)
            try:
        ...
        if sensor in ('1', '2'):
            ...
            if res:
                ...
                for result in result_list:
                    probe_num = int(search(self.ProbeNumberRegEx, result).group(1))
    ...
    def UpdateIndividualCurrentStatus(self, value, qualifier):

        port_list = (str(port) for port in range(1, 13))

        port = qualifier['Port']
        if port in port_list:
            IndividualCurrentStatusCmdString = 'portsensor all "3" show\r'
            res = self.__UpdateHelper('IndividualCurrentStatus', IndividualCurrentStatusCmdString, value, qualifier)
            if res:
                try:
                    result_list = findall(self.IndividualCurrentStatusListRegEx, res)
                    for result in result_list:
                        value = round(float(result[1]), 3)
                        qualifier = {'Port': result[0]}
                        self.WriteStatus('IndividualCurrentStatus', value, qualifier)
                except (ValueError, IndexError):
                    self.Error(['Individual Current Status: Invalid/unexpected response'])
        else:
            self.Discard('Invalid Command for UpdateIndividualCurrentStatus')
    ...
    def SetPower(self, value, qualifier):
        ...
        port_list = (str(port) for port in range(1, 13))
        if port in port_list:
            PowerCmdString = 'port {} state set {}\r'.format(port, ValueStateValues[value])
    ...
    def gude_20_4396_4(self):

        self.IndividualCurrentStatusListRegEx = compile(r'P=([1-4]),.+?3="(\d*(?:\.\d+)?)A"')
        self.PowerListRegEx = compile(r'P([1-4])=(ON|OFF)')

    def gude_20_4396_8(self):

        self.IndividualCurrentStatusListRegEx = compile(r'P=([1-8]),.+?3="(\d*(?:\.\d+)?)A"')
        self.PowerListRegEx = compile(r'P([1-8])=(ON|OFF)')

    def gude_20_4396_12(self):

        self.IndividualCurrentStatusListRegEx = compile(r'P=([1-9]|1[0-2]),.+?3="(\d*(?:\.\d+)?)A"')
        self.PowerListRegEx = compile(r'P([1-9]|1[0-2])=(ON|OFF)')
