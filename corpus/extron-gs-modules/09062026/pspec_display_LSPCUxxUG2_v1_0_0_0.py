...
class DeviceClass:
    ...
    def __init__(self):
        ...
        if self.Unidirectional == 'False':  # Matchstrings implement re.I to account for unknown response case sensitivity
            self.AddMatchString(re.compile(b'#\*MUTE (ON|OFF)[\r\n]*?', re.I), self.__MatchAudioMute, None)
            self.AddMatchString(re.compile(b'#\*source is (HDMI1|VGA|DP|DVI|OPS|7|12|18|19|20)[\r\n]*?', re.I), self.__MatchInput, None)
            self.AddMatchString(re.compile(b'#\*Quick Stand?by is (ON|OFF)[\r\n]*?', re.I), self.__MatchPower, None)
            self.AddMatchString(re.compile(b'#\*volume level is (\d{1,3})[\r\n]*?', re.I), self.__MatchVolume, None)
            self.AddMatchString(re.compile(b'#\*(Invalid|Incorrect|NACK).*?', re.I), self.__MatchError, None)
    ...
    def __MatchInput(self, match, tag):
        ...
        if res.isdigit():
            ...
            if 'HDMI' in res:
                ...
                value = match.group(1).decode().upper()
    ...
    def SetVolume(self, value, qualifier):
        ...
        if 0 <= value <= 100:
            VolumeCmdString = 'STV {}\r'.format(value)
