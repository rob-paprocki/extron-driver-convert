...
class DeviceSerialClass:
    ...
    def __MatchVideoWallSize(self, match, tag):
        ...
        qualifier = {'Device ID': str(unpack('B', match.group(1))[0]), 'Columns': str(rowXcol % 16), 'Rows': str(rowXcol // 16)}
...
class DeviceEthernetClass:
    ...
    def DeviceID(self, value):
        if value == 'Broadcast':
            ...
            self.Error(['Device ID should be a value between 0 to 253 or Broadcast'])
    ...
    def SetAspectRatio(self, value, qualifier):
        ...
        AspectRatioCmdString = b'\xaa\x18' + self._DeviceID + b'\x01' + ValueStateValues[value]
        AspectRatioCmdString = AspectRatioCmdString + self.calcChecksum(AspectRatioCmdString)
        self.__SetHelper('AspectRatio', AspectRatioCmdString, value, qualifier)

    def UpdateAspectRatio(self, value, qualifier):

        AspectRatioCmdString = b'\xaa\x18' + self._DeviceID + b'\x00'
        AspectRatioCmdString = AspectRatioCmdString + self.calcChecksum(AspectRatioCmdString)
        self.__UpdateHelper('AspectRatio', AspectRatioCmdString, value, qualifier)
    ...
    def SetAudioMute(self, value, qualifier):
        ...
        AudioMuteCmdString = b'\xaa\x13' + self._DeviceID + b'\x01' + ValueStateValues[value]
        AudioMuteCmdString = AudioMuteCmdString + self.calcChecksum(AudioMuteCmdString)
        self.__SetHelper('AudioMute', AudioMuteCmdString, value, qualifier)

    def UpdateAudioMute(self, value, qualifier):

        AudioMuteCmdString = b'\xaa\x13' + self._DeviceID + b'\x00'
        AudioMuteCmdString = AudioMuteCmdString + self.calcChecksum(AudioMuteCmdString)
        self.__UpdateHelper('AudioMute', AudioMuteCmdString, value, qualifier)
    ...
    def SetVideoWallSize(self, value, qualifier):
        ...
        if 1 <= rows <= 15 and 1 <= columns <= 15 and 1 <= value <= 100:
            if value <= (rows * columns) <= 100:
                VideoWallSizeCmdString = b'\xaa\x89' + self._DeviceID + b'\x02' + pack('B', rows * 16 + columns) + pack('B', value)
                VideoWallSizeCmdString = VideoWallSizeCmdString + self.calcChecksum(VideoWallSizeCmdString)
                self.__SetHelper('VideoWallSize', VideoWallSizeCmdString, value, qualifier)
            else:
                self.Discard('Invalid Command for SetVideoWallSize')
    ...
    def __MatchVideoWallSize(self, match, tag):
        ...
        qualifier = {'Columns': str(rowXcol % 16), 'Rows': str(rowXcol // 16)}
