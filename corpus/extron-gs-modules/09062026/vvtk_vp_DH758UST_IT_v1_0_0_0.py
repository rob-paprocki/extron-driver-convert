...
class DeviceClass:
    ...
    def __init__(self):
        ...
        self.regex_set = re.compile(b'(F|P)\r?')
        self.regex4_update = re.compile(b'(F\r?|P[0-9]{4}\r?)')
        self.regex3_update = re.compile(b'(F\r?|P[0-9]{3}\r?)')
        self.regex2_update = re.compile(b'(F\r?|P(\+|\-)?[0-9]{2}\r?)')
        self.regex1_update = re.compile(b'(F\r?|P(\+|\-)?[0-9]{1}\r?)')
    ...
    def SetDisplayMode(self, value, qualifier):
        ...
        DisplayModeCmdString = 'V{0}S0108{1}\r'.format(self._DeviceID, ValueStateValues[value])
    ...
    def UpdateDisplayMode(self, value, qualifier):
        ...
        DisplayModeCmdString = 'V{0}G0108\r'.format(self._DeviceID)
    ...
    def UpdateZoom(self, value, qualifier):
        ...
        res = self.__UpdateHelper('Zoom', ZoomCmdString, value, qualifier, 3)
        if res:
            try:
                if res[-1] == 13:
                    value = int(res[1:-1])
    ...
    def __CheckResponseForErrors(self, sourceCmdName, response):
        ...
        DEVICE_ERROR_CODES = {'F': "Error: Invalid Command",
                              'F\r': "Error: Invalid Command"}
        if response:
    ...
    def __SetHelper(self, command, commandstring, value, qualifier):
        ...
        if self.Unidirectional == 'True':
            ...
            res = self.SendAndWait(commandstring, self.DefaultResponseTimeout, deliRex=self.regex_set)
            if not res:
                print('No Response')
    ...
    def __UpdateHelper(self, command, commandstring, value, qualifier, length=1):
        ...
        UpdateRegex = {
            1: self.regex1_update,
            2: self.regex2_update,
            3: self.regex3_update,
            4: self.regex4_update
            }
        if self.Unidirectional == 'True' or self._DeviceID == '99':
            ...
            res = self.SendAndWait(commandstring, self.DefaultResponseTimeout, deliRex=UpdateRegex[length])
            if not res:
                if length >= 1:
