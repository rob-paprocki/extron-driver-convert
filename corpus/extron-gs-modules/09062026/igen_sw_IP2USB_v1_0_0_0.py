...
class DeviceSerialClass:
    def __init__(self):
        ...
        self.Commands = {
            'SelectCameraCommand': { 'Status': {}}
        }

    def SetSelectCameraCommand(self, value, qualifier):

        SelectCameraCommandCmdString = 'CAMERASELECT {}\r\n'.format(value)
        self.__SetHelper('SelectCameraCommand', SelectCameraCommandCmdString, value, qualifier)
...
class DeviceHTTPClass:
    ...
    def SetSelectCameraCommand(self, value, qualifier):
        ...
        url = 'api/v1/cameraSelect?id={}'.format(value)
        self.__SetHelper('SelectCameraCommand', value, qualifier, url=url)

    def __CheckResponseForErrors(self, sourceCmdName, response):

        try:
            res = loads(response.read().decode('iso-8859-1'))
            if res['message'] in ['error', 'authorization error']:
                self.Error(['Error: {}, for command {}'.format(res['message'], sourceCmdName)])
                return ''
            return res
...
class HTTPClass(DeviceHTTPClass):
    def __init__(self, ipAddress, port, Model=None, SSLVerifyMode='On'):
