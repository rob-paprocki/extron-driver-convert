...
class DeviceClass:
    def __init__(self):
        ...
        if self.Unidirectional == 'False':
            ...
            self.AddMatchString(re.compile(b'\xAA\xFF[\x00-\xFF]\x03\x41\x14(\x14|\x18|\x0C|\x08|\x20|\x21|\x23|\x1F|\x30|\x40|\x24|\x25|\x22|\x31|\x32)[\x00-\xFF]'), self.__MatchInput, None)
            ...
            self.AddMatchString(re.compile(b'\xAA\xFF([\x00-\xFF])\x03\x4E(\x18|\x5D|\x84|\x14|\x3C|\x11|\x5C|\x12)([\x00-\xFF])[\x00-\xFF]'), self.__MatchError, None)
    ...
    def SetInput(self, value, qualifier):
        ...
        InputCmdString = b'\xAA\x14' + self._DeviceID + b'\x01' + ValueStateValues[value] + checksum
        ...
        if 0 < column <= 15:
            if 0 < displayNum <= 100:
                ...
                if row <= 0x60 and column <= 15 and displayNum <= 90: 
                    ...
                    Valid = False
                if Valid:
                    checksum = pack('>B', (0x89 + self._DeviceID[0] + 0x02 + size + displayNum) & 0xFF)
                    CmdString = b'\xAA\x89' + self._DeviceID + b'\x02' + pack('>2B', size, displayNum) + checksum
    ...
    def UpdateVideoWallSize(self, value, qualifier): 
        ...
        checksum = pack('>B', (0x89 + self._DeviceID[0]) & 0xFF)
        VideoWallSizeCmdString =  b'\xAA\x89' + self._DeviceID + b'\x00' + checksum
    ...
    def __MatchError(self, match, tag):
        ...
        self.Error(['Device Id: {0}, Command: {1}, Error: {2}'.format(match.group(1), Error[match.group(2).decode()], match.group(3))])
