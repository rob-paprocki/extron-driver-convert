...
class DeviceClass:
    def __init__(self):
        ...
        self.AspectRatioRegex = re.compile('ASPECT=([3456]0)( 30)?\r:')
        ...
        self.InputRegex = re.compile('SOURCE=(10|20|30|A0|41|51|52|53|56|59|58)\r:')
        ...
        self.update_functions = {
            'AspectRatio': self.UpdateAspectRatio,
            'AVMute': self.UpdateAVMute,
            'DeviceStatus': self.UpdateDeviceStatus,
            'Freeze': self.UpdateFreeze,
            'Input': self.UpdateInput,
            'LampMode': self.UpdateLampMode,
            'LampUsage': self.UpdateLampUsage,
            'OperationHours': self.UpdateOperationHours,
    ...
    def UpdateAspectRatio(self, value, qualifier):
        ...
        if res:
            try:
                ...
                if valueMatch.group(2):
                    value = ValueStateValues[valueMatch.group(2).strip()]
                else:
    ...
    def UpdateAVMute(self, value, qualifier):
        ...
        if res:
            try:
                ...
                value = valueMatch.group(1).title()
    ...
    def __CheckResponseForErrors(self, sourceCmdName, response):
        ...
        DEVICE_ERROR_CODES = {'ERR\r:': 'An error occurred'}
        if response:
            if response in DEVICE_ERROR_CODES:
                self.Error(['{} for command: {}'.format(DEVICE_ERROR_CODES[response], sourceCmdName)])
                response = ''
        return response
