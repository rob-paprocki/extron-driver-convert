...
class DeviceClass:
    ...
    def __init__(self):
        ...
        self.Commands = {
            ...
            'DigitalIO': {'Parameters': ['Port'], 'Status': {}},
            'DigitalIOPortMode': {'Parameters': ['Port'], 'Status': {}},
            'DisplayMute': {'Status': {}},
            'DisplayPower': {'Status': {}},
        ...
        if self.Unidirectional == 'False':
            ...
            self.AddMatchString(re.compile(b'Sw(Prs|Rls)\*(001|002|009|010|011|012)\r'), self.__MatchButtonResponse, None)
            ...
            self.AddMatchString(re.compile(b'Sio(1|2|3)\*(0|1)\r'), self.__MatchDigitalIO, None)
            self.AddMatchString(re.compile(b'Iom(1|2|3)\*(0|1|2|3)\r'), self.__MatchDigitalIOPortMode, None)
    ...
    def __MatchControlModuleLED(self, match, tag):
        ...
        if ButtNum < 24:
            qualifier = {'LED': LEDnames[str(int(match.group(1).decode()))]}
    ...
    def SetDigitalIO(self, value, qualifier):
        ...
        if PortConstraints['Min'] <= PortNum <= PortConstraints['Max']:
            DigitalIOCmdString = '{0}*{1}]'.format(PortNum, DigitalIOValues[value])

            self.__SetHelper('DigitalIO', DigitalIOCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetDigitalIO')
    ...
    def UpdateDigitalIO(self, value, qualifier):
        ...
        PortNum = int(qualifier['Port'])
        if PortConstraints['Min'] <= PortNum <= PortConstraints['Max']:
            DigitalIOCmdString = '{0}]'.format(PortNum)
            self.__UpdateHelper('DigitalIO', DigitalIOCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for UpdateDigitalIO')
    ...
    def __MatchDigitalIO(self, match, tag):
        ...
        value = DigitalIONames[match.group(2).decode()]
        self.WriteStatus('DigitalIO', value, qualifier)
    ...
    def SetDigitalIOPortMode(self, value, qualifier):
        ...
        if PortConstraints['Min'] <= PortNum <= PortConstraints['Max']:
            DigitalIOPortModeCmdString = '{0}*{1}['.format(PortNum, DigitalIOPortMode[value])

            self.__SetHelper('DigitalIOPortMode', DigitalIOPortModeCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetDigitalIOPortMode')
    ...
    def UpdateDigitalIOPortMode(self, value, qualifier):
        ...
        PortNum = int(qualifier['Port'])
        if PortConstraints['Min'] <= PortNum <= PortConstraints['Max']:
            DigitalIOPortModeCmdString = '{0}['.format(PortNum)
            self.__UpdateHelper('DigitalIOPortMode', DigitalIOPortModeCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for UpdateDigitalIOPortMode')
    ...
    def __MatchDigitalIOPortMode(self, match, tag):
        ...
        qualifier = {'Port': match.group(1).decode()}
        value = DigitalIOPortModeNames[match.group(2).decode()]
        self.WriteStatus('DigitalIOPortMode', value, qualifier)
