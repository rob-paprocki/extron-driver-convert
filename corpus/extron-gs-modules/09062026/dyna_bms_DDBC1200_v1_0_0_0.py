...
class DeviceClass:
    ...
    def SetRecallPreset(self, value, qualifier):
        ...
        if FadeConstraints['Min'] <= fade <= FadeConstraints['Max'] and 0 <= area <= 255:

            fade = int((fade * 0.1) * 10)
    ...
    def SetRecordPreset(self, value, qualifier):
        ...
        if 0 <= int(value) <= 255:
            ...
            chksum = 0x1c + int(value) + 0x08 + 0xFF
            chksum = ((chksum ^ 0xFFFF) + 1) & 0xFF

            RecordPresetCmdString = pack('>8B', 0x1c, int(value), 0x00, 0x66, 0x00, 0x00, 0xFF, chksum)
