...
class DeviceClass:
    def __init__(self):
        ...
        self._MeetingDateFormat = '%m/%d'
        self._MeetingTimeFormat = '%I:%M%p'
        ...
        self._PhonebookFolderSearchLimit = 50
        self._PhonebookSearchOffset = 50

        self.Models = {
            'Webex Room Kit EQ RoomOS 11.X.X': self.csco_12_6148_eq,
            'Webex Room Kit EQX RoomOS 11.X.X': self.csco_12_6148_eqx,
            'Webex Room Kit Mini RoomOS 11.X.X': self.csco_12_6148_mini,
            'Webex Room Kit RoomOS 11.X.X': self.csco_12_6148_non_p,
            'Webex Room Kit Plus RoomOS 11.X.X': self.csco_12_6148_plus,
            'Webex Room Kit Pro RoomOS 11.X.X': self.csco_12_6148_pro,
        }
        ...
        self.Commands = {
            ...
            'CallCapabilityRecordingStatus': {'Parameters':['Call','State'], 'Status': {}},
            ...
            'CallMeetingPlatform': {'Parameters':['Call'], 'Status': {}},
            ...
            'Hook': {'Parameters': ['Protocol', 'Call Type'], 'Status': {}},
            'Input': {'Status': {}},
            ...
            'MicrosoftTeamsCallStatus': { 'Status': {}},
            'MuteOnEntry': {'Parameters':['Call'], 'Status': {}},
            ...
            'PhonebookUpdate': {'Parameters':['Phonebook Type', 'Search Filter'], 'Status': {}},
            'Presentation': {'Status': {}},
            ...
            'PresenterTrackPresenterDetectedStatus': {'Status': {}},
            'PresetRecall': {'Status': {}},
            'PresetSave': {'Status': {}},
            'Reboot': {'Status': {}},
            'Recording': {'Parameters':['Call'], 'Status': {}},
            'RemoteNumber': {'Parameters': ['Call'], 'Status': {}},
            'SelfView': {'Status': {}},
            'SelfViewDefaultFullscreenMode': {'Status': {}},
            'SelfViewDefaultOnMonitorRole': { 'Status': {}},
            'SelfViewFullscreenMode': {'Status': {}},
            'SelfViewPosition': {'Status': {}},
            'SIPDisplayNameCommand': {'Status': {}},
            'SIPDisplayNameStatus': {'Status': {}},
            'SIPRegistrationStatus': {'Status': {}},
            'SIPURICommand': {'Status': {}},
            'SIPURIStatus': {'Status': {}},
            'SleepTimer': {'Status': {}},
            'SpeakerTrackControl': {'Status': {}},
            'SpeakerTrackFrames': { 'Status': {}},
        ...
        self.callIDPattern = compile('\*s Call (\d+) Status: (?:Idle|Dialling|Ringing|Connecting|Connected|Disconnecting|OnHold|EarlyMedia|Preserved|RemotePreserved)\r\n')
        self.callStatusPattern = compile('\*s Call \d+ Status: (Idle|Dialling|Ringing|Connecting|Connected|Disconnecting|OnHold|EarlyMedia|Preserved|RemotePreserved)\r\n')
        ...
        self.answerStatePattern = compile('\*s Call \d+ AnswerState: (Unanswered|Ignored|Autoanswered|Answered)\r\n')

        self.contactName = compile('\*r PhonebookSearchResult Contact (\d+) Name: "([^\r\n]+)"\r\n')
        self.contactNumber = compile('\*r PhonebookSearchResult Contact (\d+) ContactMethod 1 Number: "([^\r\n]+)"\r\n')

        self.folderName = compile('\*r PhonebookSearchResult Folder (\d+) Name: "([^\r\n]+)"\r\n')
        self.folderID = compile('\*r PhonebookSearchResult Folder (\d+) FolderId: "([^\r\n]+)"\r\n')
        
        self.peripheralName = compile('\*s Peripherals ConnectedDevice \d+ Name: "([^\r\n]+)"\r\n')
        self.peripheralType = compile('\*s Peripherals ConnectedDevice \d+ Type: (\w+)\r\n')
        self.peripheralMAC = compile('\*s Peripherals ConnectedDevice \d+ ID: "(|[\w:]{17})"\r\n')
        ...
        self.MeetingID = compile('\*r BookingsListResult Booking (\d+) Id: "([^\r\n]*)"\r\n')
        self.Title = compile('\*r BookingsListResult Booking (\d+) Title: "([^\r\n]*)"\r\n')
        self.Agenda = compile('\*r BookingsListResult Booking (\d+) Agenda: "([^\r\n]*)"\r\n')
        self.FirstName = compile('\*r BookingsListResult Booking (\d+) Organizer FirstName: "([^\r\n]*)"\r\n')
        self.LastName = compile('\*r BookingsListResult Booking (\d+) Organizer LastName: "([^\r\n]*)"\r\n')
        ...
        self.DialPlatform = compile('\*r BookingsListResult Booking (\d+) MeetingPlatform: "([^\r\n]*)"\r\n')
        self.DialNumber = compile('\*r BookingsListResult Booking (\d+) DialInfo Calls Call 1 Number: "([^\r\n]*)"\r\n')
        self.DialProtocol = compile('\*r BookingsListResult Booking (\d+) DialInfo Calls Call 1 Protocol: ([^\r\n]*)\r\n')
        self.DialMode = compile('\*r BookingsListResult Booking (\d+) DialInfo ConnectMode: (OBTP|Manual)\r\n')

        self.meeting_platforms = {}
        self.meeting_protocols = {}
        
        
        if self.Unidirectional == 'False':
            self.AddMatchString(compile(b'\*c xConfiguration Audio Output (InternalSpeaker|Line [1-6]|HDMI [1-3]|USB(?:Interface|C) 1) Mode: (On|Off|UltrasoundOnly)\r\n'), self.__MatchAudioOutput, None)
            self.AddMatchString(compile(b'\*c xConfiguration Conference AutoAnswer Mode: (Off|On)\r\n'), self.__MatchAutoAnswer, None)
            self.AddMatchString(compile(b'\*s Conference Call (\d+) AuthenticationRequest: (None|HostPinOrGuest|HostPinOrGuestPin|PanelistPin|AnyHostPinOrGuestPin|GuestPin|PanelistPinOrAttendeePin|PanelistPinOrAttendee)\r\n'), self.__MatchCallAuthenticationRequestStatus, None)
            self.AddMatchString(compile(b'\*s Conference Call (\d+) Capabilities Recording (Pause|Resume|Start|Stop): (Available|Unavailable)\r\n'), self.__MatchCallCapabilityRecordingStatus, None)
            self.AddMatchString(compile(b'\*s Conference Call (\d+) MeetingPlatform: (Unknown|GoogleMeet|MSTeams|Webex|Zoom)\r\n'), self.__MatchCallMeetingPlatform, None)
            ...
            self.AddMatchString(compile(b'\*c xConfiguration Conference FarEndControl Mode: (Off|On|Auto)\r\n'), self.__MatchFarEndControl, None)
            self.AddMatchString(compile(b'version="([^"]+)" apiVersion'), self.__MatchFirmwareVersion, None)
            ...
            self.AddMatchString(compile(b'\*c xConfiguration Audio Input (Microphone [1-8]|HDMI [1-5]|USB(?:Interface|C) 1) Mode: (On|Off)\r\n'), self.__MatchInputMute, None)
            self.AddMatchString(compile(b'\*s Video Input Connector ([1-6]) Connected: (True|False|Unknown)\r\n'), self.__MatchInputSignal, None)
            ...
            self.AddMatchString(compile(b'\*s Network 1 Ethernet MacAddress: "(|[\w:]{17})"\r\n'), self.__MatchMACAddress, None)
            ...
            self.AddMatchString(compile(b'\*s MicrosoftTeams Calling InCall: (False|True)\r\n'), self.__MatchMicrosoftTeamsCallStatus, None)
            ...
            self.AddMatchString(compile(b'\*c xConfiguration Cameras PresenterTrack PresenterDetectedStatus: (Enabled|Disabled)\r\n'), self.__MatchPresenterTrackPresenterDetectedStatus, None)
            ...
            self.AddMatchString(compile(b'\*c xConfiguration Video Selfview Default OnMonitorRole: (Current|First|Second|Third)\r\n'), self.__MatchSelfViewDefaultOnMonitorRole, None)
            self.AddMatchString(compile(b'\*s Video Selfview FullscreenMode: (On|Off)\r\n'), self.__MatchSelfViewFullscreenMode, None)
            ...
            self.AddMatchString(compile(b'\*c xConfiguration SIP DisplayName: "([^\r\n]*)"\r\n'), self.__MatchSIPDisplayNameStatus, None)
            ...
            self.AddMatchString(compile(b'\*s Cameras SpeakerTrack Frames Status: (Active|Inactive)\r\n'), self.__MatchSpeakerTrackFrames, None)
    ...
    def NumberofCallHistory(self, value):
        if int(value) > 20 or int(value) < 1:
            self.Error(['Number of Call History Parameter is out of range.'])
        else:
            self._NumberofCallHistory = int(value)

    @property
    def MeetingDateFormat(self):
        return self._MeetingDateFormat

    @MeetingDateFormat.setter
    def MeetingDateFormat(self, value):
        if value in ['mm/dd', 'dd/mm']:
            self._MeetingDateFormat = {'mm/dd':'%m/%d', 'dd/mm':'%d/%m'}[value]
        else:
            self.Error(['Invalid Meeting Date Format Parameter.'])

    @property
    def MeetingTimeFormat(self):
        return self._MeetingTimeFormat

    @MeetingTimeFormat.setter
    def MeetingTimeFormat(self, value):
        if value in ['12 Hour', '24 Hour']:
            self._MeetingTimeFormat = {'12 Hour':'%I:%M%p', '24 Hour':'%H:%M'}[value]
        else:
            self.Error(['Invalid Meeting Time Format Parameter.'])
    ...
    def NumberofPeripheral(self, value):
        if int(value) > 10 or int(value) < 1:
            self.Error(['Number of Peripheral Parameter is out of range.'])
        else:
    ...
    def NumberofPhonebookFolderSearch(self, value):
        if int(value) > 10 or int(value) < 1:
            self.Error(['Number of Phonebook Folder Search Parameter is out of range.'])
        else:
    ...
    def NumberofPhonebookSearch(self, value):
        if int(value) > 10 or int(value) < 1:
            self.Error(['Number of Phonebook Search Parameter is out of range.'])
        else:
    ...
    def PhonebookSearchOffset(self, value):
        if 20 <= int(value) <= 250:
            ...
            self.Error(['Phonebook Search Offset Parameter is out of range.'])

    @property
    def PhonebookFolderSearchLimit(self):
        return self._PhonebookFolderSearchLimit

    @PhonebookFolderSearchLimit.setter
    def PhonebookFolderSearchLimit(self, value):
        if 20 <= int(value) <= 250:
            self._PhonebookFolderSearchLimit = int(value)
        else:
            self.Error(['Phonebook Folder Search Limit Parameter is out of range.'])
    ...
    def SetAudioOutput(self, value, qualifier):
        ...
        if qualifier['Output'] in self.AudioOutputParamStates and value in self.AudioOutputStates:
            AudioOutputCmdString = 'xConfiguration Audio Output {0} Mode:{1}\r'.format(self.AudioOutputParamStates[qualifier['Output']], self.AudioOutputStates[value])
    ...
    def UpdateAudioOutput(self, value, qualifier):
        ...
        if qualifier['Output'] in self.AudioOutputParamStates:
            AudioOutputCmdString = 'xConfiguration Audio Output {0} Mode\r'.format(self.AudioOutputParamStates[qualifier['Output']])
    ...
    def __MatchAudioOutput(self, match, tag):
        ...
        value = self.AudioOutputValues[match.group(2).decode()]
        self.WriteStatus('AudioOutput', value, {'Output': self.AudioOutputParamValues[match.group(1).decode()]})
    ...
    def UpdateCallAuthenticationRequestStatus(self, value, qualifier):
        ...
        if 1 <= call <= 5:
            self.Send('xFeedback register /Status/Conference/Call\r')
            try: # if in a call
                CallAuthenticationRequestStatusCmdString = 'xStatus Conference Call {} AuthenticationRequest\r'.format(list(self.callStatusData.keys())[call-1])
                self.__UpdateHelper('CallAuthenticationRequestStatus', CallAuthenticationRequestStatusCmdString, value, qualifier)
    ...
    def __MatchCallAuthenticationRequestStatus(self, match, tag):
        ...
        ValueStateValues = {
            ...
            'AnyHostPinOrGuestPin'     : 'Any Host Pin or Guest Pin',
            'GuestPin'                 : 'Guest Pin',
            'PanelistPinOrAttendeePin' : 'Panelist Pin or Attendee Pin',
            'PanelistPinOrAttendee'    : 'Panelist Pin or Attendee'
        }
    ...
    def SetCallAuthenticationResponseCommand(self, value, qualifier):
        ...
        pin = qualifier['Pin']
        if (call == 'Not Specified' or 1 <= int(call) <= 5) and pRole in ['Host', 'Panelist', 'Guest'] and (pin is None or 0 <= len(pin) <= 32):
    ...
    def UpdateCallCapabilityRecordingStatus(self, value, qualifier):
        ...
        if 1 <= call <= 5 and state in StateValues:
            self.Send('xFeedback register /Status/Conference/Call\r')
            try: # if in a call
                CallCapabilityRecordingStatusCmdString = 'xStatus Conference Call {} Capabilities Recording {}\r'.format(list(self.callStatusData.keys())[call-1], state)
                self.__UpdateHelper('CallCapabilityRecordingStatus', CallCapabilityRecordingStatusCmdString, value, qualifier)
            except IndexError: # if not in a call
                self.WriteStatus('CallCapabilityRecordingStatus', 'Unavailable', qualifier) # default to 'Unavailable'
        else:
            self.Discard('Invalid Command for UpdateCallCapabilityRecordingStatus')

    def __MatchCallCapabilityRecordingStatus(self, match, tag):

        try:
            qualifier = {
                'Call': str(list(self.callStatusData.keys()).index(match.group(1).decode())+1),
                'State': match.group(2).decode()
            }

            value = match.group(3).decode()
            self.WriteStatus('CallCapabilityRecordingStatus', value, qualifier)
        except ValueError:
            pass
    ...
    def SetCallHistorySelect(self, value, qualifier):
        ...
        if 1 <= int(value) <= self.call_history.window:
            ...
            if number and number not in [self.call_history.end, self.call_history.fill]:
                self.SetHook('Dial', {'Protocol': 'Auto', 'Call Type': 'Auto', 'Number': number})
        else:
            self.Discard('Invalid Command for SetCallHistorySelect')

    def UpdateCallMeetingPlatform(self, value, qualifier):

        call = int(qualifier['Call'])
        if 1 <= call <= 5:
            self.Send('xFeedback register /Status/Conference/Call\r')
            try: # if in a call
                CallMeetingPlatformCmdString = 'xStatus Conference Call {} MeetingPlatform\r'.format(list(self.callStatusData.keys())[call-1])
                self.__UpdateHelper('CallMeetingPlatform', CallMeetingPlatformCmdString, value, qualifier)
            except IndexError: # if not in a call
                self.WriteStatus('CallMeetingPlatform', 'None', qualifier) # default to 'None'
        else:
            self.Discard('Invalid Command for UpdateCallMeetingPlatform')

    def __MatchCallMeetingPlatform(self, match, tag):
        ...
        try:
            qualifier = {'Call': str(list(self.callStatusData.keys()).index(match.group(1).decode())+1)}
            value = ValueStateValues[match.group(2).decode()]
            self.WriteStatus('CallMeetingPlatform', value, qualifier)
        except ValueError:
            pass
    ...
    def __MatchCallStatusIdle(self, match, tag):
        ...
        self.callStatusData.clear()  # clear call status data if no calls
    ...
    def __MatchCallStatus(self, match, tag):
        ...
        if ' Status: ' in res or ' CallType: ' in res or ' DisplayName: ' in res or ' RemoteNumber: ' in res or ' DeviceType: ' in res or ' AnswerState: ' in res or ' (ghost=True):' in res:  # only update status if useful to driver
            ...
            if callEndIDList: # if any calls have ended
                ...
                answerStateList = findall(self.answerStatePattern, res)
                ...
                for index, callID in enumerate(callIDList):
                    if callID not in self.callStatusData:  # if call doesnt exist yet in self.callStatusData
                        self.callStatusData[callID] = {'Status': 'Idle', 'CallType': 'Unknown', 'DisplayName': '', 'RemoteNumber': '', 'DeviceType': 'Unknown', 'AnswerState': 'Unknown'}  # initialize it
                    ...
                    if callStatusList[index] == 'Idle':  # if call status is idle
                        ...
                        try:
                            ...
                            pass
                        try:
                            self.callStatusData[callID]['AnswerState'] = answerStateList[index]
                        except IndexError:
                            pass
            for i in range(5):
                try:
                ...
                try:
                    self.WriteStatus('CallAnswerState', answerStates[self.callStatusData[item]['AnswerState']], {'Call': str(i + 1)})
                except KeyError:
                    self.WriteStatus('CallAnswerState', 'Unknown', {'Call': str(i + 1)})
    ...
    def SetCameraFocus(self, value, qualifier):
        ...
        camID = '1' if not qualifier else qualifier.get('Camera', '1')
        if 1 <= int(camID) <= 7:
    ...
    def SetDisplayMode(self, value, qualifier):
        ...
        if value in self.DisplayModeStates:
    ...
    def UpdateDNSServerAddress(self, value, qualifier):
        ...
        if 1 <= int(qualifier['Server']) <= 5:  # 1-5 verified to work with device
    ...
    def UpdateH323AliasE164Status(self, value, qualifier):
        ...
        self.__UpdateHelper('H323AliasE164Status', H323AliasE164StatusCmdString, value, qualifier)
    ...
    def SetHook(self, value, qualifier):
        ...
        if value in ['Accept', 'Reject', 'Disconnect All']:
        ...
        elif 'Resume' in value or 'Disconnect' in value or 'Hold' in value or ('Join' in value and value not in ['Webex Join', 'WebRTC Join']):
            val = value.split(' ')
            cmd = val[0]
            index = int(val[1]) - 1
            try:
            ...
            if number and protocol in ProtocolStates and qualifier['Call Type'] in ['Auto', 'Audio', 'Video']:
                if protocol == 'Auto':
                    self.__SetHelper('Hook', 'xCommand Dial Number:\"{0}\" CallType:{1}\r'.format(number, qualifier['Call Type']), value, qualifier)
                else:
                    self.__SetHelper('Hook', 'xCommand Dial Number:\"{0}\" Protocol:{1} CallType:{2}\r'.format(number, ProtocolStates[protocol], qualifier['Call Type']), value, qualifier)
            else:
                self.Discard('Invalid Command for SetHook')
        elif value == 'Webex Join':
            number = qualifier['Meeting Number']
            if number:
                self.__SetHelper('Hook', 'xCommand Webex Join Number: {}\r'.format(number), value, qualifier)
            else:
                self.Discard('Invalid Command for SetHook')
        elif value == 'WebRTC Join':
            number = qualifier['Meeting Number']
            if number:
                self.__SetHelper('Hook', 'xCommand WebRTC Join MeetingNumber: {} Type: GoogleMeet\r'.format(number), value, qualifier)
            else:
                self.Discard('Invalid Command for SetHook')
        else:
            self.Discard('Invalid Command for SetHook')
    ...
    def SetInputHDMILevel(self, value, qualifier):
        ...
        if qualifier['Input'] in self.InputHDMILevelStates and ValueConstraints['Min'] <= value <= ValueConstraints['Max']:
            InputHDMILevelCmdString = 'xConfiguration Audio Input {0} Level:{1}\r'.format(self.InputHDMILevelStates[qualifier['Input']], value)
    ...
    def UpdateInputHDMILevel(self, value, qualifier):
        ...
        if qualifier['Input'] in self.InputHDMILevelStates:
            InputHDMILevelCmdString = 'xConfiguration Audio Input {0} Level\r'.format(self.InputHDMILevelStates[qualifier['Input']])
    ...
    def __MatchInputHDMILevel(self, match, tag):
        ...
        self.WriteStatus('InputHDMILevel', value, {'Input': self.InputHDMILevelValues[match.group(1).decode()]})
    ...
    def SetInputMicLevel(self, value, qualifier):
        ...
        if qualifier['Input'] in self.InputMicLevelStates and ValueConstraints['Min'] <= value <= ValueConstraints['Max']:
    ...
    def UpdateInputMicLevel(self, value, qualifier):
        ...
        if qualifier['Input'] in self.InputMicLevelStates:
    ...
    def SetInputMute(self, value, qualifier):
        ...
        if qualifier['Input'] in self.InputMuteStates and value in ValueStateValues:
            InputMuteCmdString = 'xConfiguration Audio Input {0} Mode:{1}\r'.format(self.InputMuteStates[qualifier['Input']], ValueStateValues[value])
    ...
    def UpdateInputSignal(self, value, qualifier):
        ...
        if 1 <= int(qualifier['Input']) <= self.InputMax:
            self.__UpdateHelper('InputSignal', 'xStatus Video Input Connector {0} Connected\r'.format(qualifier['Input']), value, qualifier)
            self.Send('xFeedback register /Status/Video/Input/Connector/Connected\r')
        else:
    ...
    def SetInputVideoMute(self, value, qualifier):
        ...
        if value in ValueStateValues:
            ...
            self.Discard('Invalid Command for SetInputVideoMute')

    def UpdateInputVideoMute(self, value, qualifier):
        InputVideoMuteCmdString = 'xStatus Video Input MainVideoMute\r'
        self.__UpdateHelper('InputVideoMute', InputVideoMuteCmdString, value, qualifier)
    ...
    def SetLayoutSet(self, value, qualifier):
        ...
        if qualifier['Target'] in TargetStates and value in ValueStateValues:
    ...
    def SetMeetingRefresh(self, value, qualifier):
        ...
        res = self.SendAndWait(MeetingRefreshCmdString, 10, deliTag=b'** end')
        res = res.decode()
        if res:
            idList = findall(self.MeetingID, res)
            titleList = findall(self.Title, res)
            agendaList = findall(self.Agenda, res)
            firstNameList = findall(self.FirstName, res)
            lastNameList = findall(self.LastName, res)
            startTimeList = findall(self.StartTime, res)
            endTimeList = findall(self.EndTime, res)
            dialModeList = findall(self.DialMode, res)
            dialNumberDict = dict(findall(self.DialNumber, res))

            self.meeting_platforms = dict(findall(self.DialPlatform, res))
            self.meeting_protocols = dict(findall(self.DialProtocol, res))
            ...
            if idList:
                ...
                for id_, info, in idList:
                    ...
                    startMeeting = StartDateTimeTZ.strftime('{0} {1}'.format(self._MeetingDateFormat, self._MeetingTimeFormat))
                    if '-' in startMeeting:
                    ...
                    endMeeting = EndDateTimeTZ.strftime('{0} {1}'.format(self._MeetingDateFormat, self._MeetingTimeFormat))
                    if '-' in endMeeting:
    ...
    def SetMeetingJoin(self, value, qualifier):
        ...
        if 1 <= int(qualifier['Meeting']) <= 5:
            ...
            if meetingNumber and meetingNumber not in ['No number to dial', 'No bookings found']:
                if self.meeting_platforms.get(qualifier['Meeting'], None) in ['MicrosoftTeams', 'MSTeams'] and \
                    self.meeting_protocols.get(qualifier['Meeting'], None) == 'WebRTC':

                    MeetingJoinCmdString = 'xCommand WebRTC Join URL:{0}\r'.format(meetingNumber)
                else:
    ...
    def UpdateMicrophonesMute(self, value, qualifier):
        ...
        self.__UpdateHelper('MicrophonesMute', 'xStatus Audio Microphones Mute\r', value, qualifier)
        self.Send('xFeedback register /Status/Audio/Microphones/Mute\r')
    ...
    def UpdateMicrosoftTeamsCallStatus(self, value, qualifier):

        MicrosoftTeamsCallStatusCmdString = 'xStatus MicrosoftTeams Calling InCall\r'
        self.__UpdateHelper('MicrosoftTeamsCallStatus', MicrosoftTeamsCallStatusCmdString, value, qualifier)

    def __MatchMicrosoftTeamsCallStatus(self, match, tag):
        ...
        self.WriteStatus('MicrosoftTeamsCallStatus', value, None)

    def SetMuteOnEntry(self, value, qualifier):
        ...
        if 1 <= call <= 5 and value in ValueStateValues:
            try:
                callID = list(self.callStatusData.keys())[call - 1]
            except IndexError:
                self.Discard('Invalid Command for SetMuteOnEntry')
                return

            MuteOnEntryCmdString = 'xCommand Conference MuteOnEntry CallId: {} MuteOnEntry: {}\r'.format(callID, value)
            self.__SetHelper('MuteOnEntry', MuteOnEntryCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetMuteOnEntry')
    ...
    def SetOutputLevel(self, value, qualifier):
        ...
        if qualifier['Output'] in ['Line 1', 'Line 2', 'Line 3', 'Line 4', 'Line 5', 'Line 6'] and ValueConstraints['Min'] <= value <= ValueConstraints['Max']:
            OutputLevelCmdString = 'xConfiguration Audio Output {0} Level:{1}\r'.format(qualifier['Output'], value)
    ...
    def UpdateOutputLevel(self, value, qualifier):
        ...
        if qualifier['Output'] in ['Line 1', 'Line 2', 'Line 3', 'Line 4', 'Line 5', 'Line 6']:
            OutputLevelCmdString = 'xConfiguration Audio Output {0} Level\r'.format(qualifier['Output'])
    ...
    def __MatchOutputLevel(self, match, tag):
        ...
        value = int(match.group(2).decode())
        self.WriteStatus('OutputLevel', value, {'Output': match.group(1).decode()})

    def UpdatePeopleCountCurrent(self, value, qualifier):

        self.__UpdateHelper('PeopleCountCurrent', 'xStatus RoomAnalytics PeopleCount Current\r', value, qualifier)
        self.Send('xFeedback register /Status/RoomAnalytics/PeopleCount/Current\r')
    ...
    def UpdatePeoplePresence(self, value, qualifier):
        ...
        self.__UpdateHelper('PeoplePresence', 'xStatus RoomAnalytics PeoplePresence\r', value, qualifier)
        self.Send('xFeedback register /Status/RoomAnalytics/PeoplePresence\r')
    ...
    def SetPhonebookSearchSet(self, value, qualifier):
        ...
        if value < 1 or value > self._NumberofPhonebookSearch:
            ...
            contact = self.phonebook[value-1]
            if contact not in [self.phonebook.end, self.phonebook.fill]:
                self.SetHook('Dial', {'Protocol': 'Auto', 'Call Type': 'Auto', 'Number': contact})

    def SetPhonebookUpdate(self, value, qualifier):
        self.Debug = True

        if qualifier['Phonebook Type'] in ['Corporate', 'Local'] and \
            qualifier['Search Filter'] in ['All', 'People', 'PMR', 'Rooms'] and \
            value in ['Refresh', 'Next Search', 'Previous Search']:
    ...
    def __UpdatePhonebookHelper(self, offset, size):
        ...
        searchFilter = self.phonebook_qualifier['Search Filter']
        ...
        if folderID:
            ...
            fldID = ''
        cmdStr = 'xCommand Phonebook Search PhonebookType:{0} {1}SearchField: Name SearchFilter: {2} ContactType: Contact {3} Offset: {4} Limit: {5}\r'.format(phonebookType, contact, searchFilter, fldID, offset, size)  # search for all contacts
        res = self.SendAndWait(cmdStr, 10, deliTag=b'** end')
        if res:
            to_add = OrderedDict()

            nameList = findall(self.contactName, res.decode())
            numberList = findall(self.contactNumber, res.decode())
    ...
    def SetPhonebookWriteHandler(self, value, qualifier):
        for i in zip(range(1, self.phonebook.window + 1), self.phonebook.format(lambda x: x['Name'])):
    ...
    def SetPhonebookFolderIDUpdate(self, value, qualifier):
        ...
        if phonebookValue in ['Local', 'Corporate']:
            if phonebookValue == 'Corporate' and self._PhonebookFolderSearchLimit > 100: # per API, max limit for Corporate phonebook is 100
                self.Discard('Invalid Command for SetPhonebookFolderIDUpdate')
                return
            cmdStr = 'xCommand Phonebook Search PhonebookType:{0} ContactType: Folder Offset: 0 Limit: {1}\r'.format(phonebookValue, self._PhonebookFolderSearchLimit)
            res = self.SendAndWait(cmdStr, 10, deliTag='** end')
            if res:
                to_add = OrderedDict()

                folderName = findall(self.folderName, res.decode())
                folderID = findall(self.folderID, res.decode())
    ...
    def UpdatePresentationModeStatus(self, value, qualifier):
        ...
        self.__UpdateHelper('PresentationModeStatus', 'xStatus Conference Presentation Mode\r', value, qualifier)
        self.Send('xFeedback register /Status/Conference/Presentation/Mode\r')
    ...
    def UpdatePresentationSendingModeStatus(self, value, qualifier):
        ...
        if 1 <= int(instanceID) <= 6:
            self.__UpdateHelper('PresentationSendingModeStatus', 'xStatus Conference Presentation LocalInstance {0} SendingMode\r'.format(instanceID), value, qualifier)
            self.Send('xFeedback register /Status/Conference/Presentation/LocalInstance/SendingMode\r')
        else:
    ...
    def UpdatePresentationSourceStatus(self, value, qualifier):
        ...
        instanceID = qualifier['Instance']
        if 1 <= int(instanceID) <= 6:
            self.__UpdateHelper('PresentationSourceStatus', 'xStatus Conference Presentation LocalInstance {0} Source\r'.format(instanceID), value, qualifier)
            self.Send('xFeedback register /Status/Conference/Presentation/LocalInstance/Source\r')
        else:
    ...
    def __MatchPresentationSourceStatusOff(self, match, tag):
        ...
        self.WriteStatus('PresentationSourceStatus', 'Off', {'Instance': match.group(1).decode()})
    ...
    def UpdatePresenterTrackControl(self, value, qualifier):
        ...
        self.__UpdateHelper('PresenterTrackControl', PresenterTrackControlCmdString, value, qualifier)
    ...
    def UpdatePresenterTrackMode(self, value, qualifier):
        ...
        self.__UpdateHelper('PresenterTrackMode', PresenterTrackModeCmdString, value, qualifier)
    ...
    def SetPresenterTrackPresenterDetectedStatus(self, value, qualifier):
        ...
        if value in ValueStateValues:
            PresenterTrackPresenterDetectedStatusCmdString = 'xConfiguration Cameras PresenterTrack PresenterDetectedStatus: {}\r'.format(ValueStateValues[value])
            self.__SetHelper('PresenterTrackPresenterDetectedStatus', PresenterTrackPresenterDetectedStatusCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetPresenterTrackPresenterDetectedStatus')

    def UpdatePresenterTrackPresenterDetectedStatus(self, value, qualifier):

        PresenterTrackPresenterDetectedStatusCmdString = 'xConfiguration Cameras PresenterTrack PresenterDetectedStatus\r'
        self.__UpdateHelper('PresenterTrackPresenterDetectedStatus', PresenterTrackPresenterDetectedStatusCmdString, value, qualifier)

    def __MatchPresenterTrackPresenterDetectedStatus(self, match, tag):
        ...
        value = ValueStateValues[match.group(1).decode()]
        self.WriteStatus('PresenterTrackPresenterDetectedStatus', value, None)
    ...
    def SetRecording(self, value, qualifier):
        ...
        if 1 <= call <= 5 and value in ValueStateValues:
            ...
            RecordingCmdString = 'xCommand Conference Recording {} CallId: {}\r'.format(value, callID)
    ...
    def UpdateSelfView(self, value, qualifier):
        ...
        self.Send('xFeedback register /Status/Video/Selfview/Mode\r')
    ...
    def SetSelfViewDefaultOnMonitorRole(self, value, qualifier):

        if value in self.SelfviewDefaultOnMonitorRoleStates:
            SelfViewDefaultOnMonitorRoleCmdString = 'xConfiguration Video Selfview Default OnMonitorRole:{0}\r'.format(value)
            self.__SetHelper('SelfViewDefaultOnMonitorRole', SelfViewDefaultOnMonitorRoleCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetSelfViewDefaultOnMonitorRole')

    def UpdateSelfViewDefaultOnMonitorRole(self, value, qualifier):

        SelfViewDefaultOnMonitorRoleCmdString = 'xConfiguration Video Selfview Default OnMonitorRole\r'
        self.__UpdateHelper('SelfViewDefaultOnMonitorRole', SelfViewDefaultOnMonitorRoleCmdString, value, qualifier)

    def __MatchSelfViewDefaultOnMonitorRole(self, match, tag):

        value = match.group(1).decode()
        self.WriteStatus('SelfViewDefaultOnMonitorRole', value, None)

    def SetSelfViewFullscreenMode(self, value, qualifier):

        if value in ['On', 'Off']:
            SelfViewFullscreenModeCmdString = 'xCommand Video Selfview Set FullscreenMode:{0}\r'.format(value)
            self.__SetHelper('SelfViewFullscreenMode', SelfViewFullscreenModeCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetSelfViewFullscreenMode')

    def UpdateSelfViewFullscreenMode(self, value, qualifier):

        SelfViewFullscreenModeCmdString = 'xStatus Video Selfview FullscreenMode\r'
        self.__UpdateHelper('SelfViewFullscreenMode', SelfViewFullscreenModeCmdString, value, qualifier)

    def __MatchSelfViewFullscreenMode(self, match, tag):

        value = match.group(1).decode()
        self.WriteStatus('SelfViewFullscreenMode', value, None)
    ...
    def UpdateSelfViewPosition(self, value, qualifier):
        ...
        self.Send('xFeedback register /Status/Video/Selfview/PIPPosition\r')
    ...
    def SetSIPDisplayNameCommand(self, value, qualifier):

        SIPDisplayNameString = value
        if SIPDisplayNameString:
            SIPDisplayNameCommandCmdString = 'xConfiguration SIP DisplayName: {}\r'.format(SIPDisplayNameString)
            self.__SetHelper('SIPDisplayNameCommand', SIPDisplayNameCommandCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetSIPDisplayNameCommand')

    def UpdateSIPDisplayNameStatus(self, value, qualifier):

        SIPDisplayNameStatusCmdString = 'xConfiguration SIP DisplayName\r'
        self.__UpdateHelper('SIPDisplayNameStatus', SIPDisplayNameStatusCmdString, value, qualifier)

    def __MatchSIPDisplayNameStatus(self, match, tag):

        value = match.group(1).decode()
        self.WriteStatus('SIPDisplayNameStatus', value, None)
    ...
    def SetSpeakerTrackFrames(self, value, qualifier):
        ...
        if value in ValueStateValues:
            SpeakerTrackFramesCmdString = 'xCommand Cameras SpeakerTrack Frames {0}\r'.format(ValueStateValues[value])
            self.__SetHelper('SpeakerTrackFrames', SpeakerTrackFramesCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetSpeakerTrackFrames')

    def UpdateSpeakerTrackFrames(self, value, qualifier):

        SpeakerTrackFramesCmdString = 'xStatus Cameras SpeakerTrack Frames Status\r'
        self.__UpdateHelper('SpeakerTrackFrames', SpeakerTrackFramesCmdString, value, qualifier)

    def __MatchSpeakerTrackFrames(self, match, tag):
        ...
        value = ValueStateValues[match.group(1).decode()]
        self.WriteStatus('SpeakerTrackFrames', value, None)
    ...
    def UpdateSystemTimeStatus(self, value, qualifier):
        ...
        if qualifier['Type'] in ['Date', 'Time']:
            ...
            self.__UpdateHelper('SystemTimeStatus', SystemTimeStatusCmdString, value, qualifier)
        else:
    ...
    def __MatchSystemTimeStatus(self, match, tag):
        ...
        time_ = '' if not match.group(1) else value.split('T')[1][0:8]
    ...
    def SetUserInterfaceTheme(self, value, qualifier):

        if value in ['Light', 'Night']:
            UserInterfaceThemeCmdString = 'xConfiguration UserInterface Theme Name: {0}\r'.format(value)
            self.__SetHelper('UserInterfaceTheme', UserInterfaceThemeCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetUserInterfaceTheme')
    ...
    def __SetHelper(self, command, commandstring, value, qualifier):
        ...
        if self.tshellerror:
            self.Discard('Inappropriate Command {}.\nDevice is not ready to connect.'.format(command))
        else:
    ...
    def csco_12_6148_mini(self):
        self.InputMax = 2

        self.InputHDMILevelStates = {
            'HDMI' : 'HDMI 1',
        }

        self.InputHDMILevelValues = {
            'HDMI 1' : 'HDMI',
        }
        ...
        self.SelfviewDefaultOnMonitorRoleStates = ['Current', 'First', 'Second']

    def csco_12_6148_non_p(self):
        self.InputMax = 2
        self.InputMicLevelMax = 24

        self.AudioOutputParamStates = {
            'Internal Speaker' : 'InternalSpeaker',
            'Line'             : 'Line 1'
        }

        self.AudioOutputParamValues = {
        ...
        self.AudioOutputStates = {
            'On'              : 'On',
            'Off'             : 'Off',
            'Ultrasound Only' : 'UltrasoundOnly'
        }

        self.AudioOutputValues = {
            'On'             : 'On',
            'Off'            : 'Off',
            'UltrasoundOnly' : 'Ultrasound Only'
        }
    ...
    def csco_12_6148_pro(self):
        ...
        self.SelfviewDefaultOnMonitorRoleStates = ['Current', 'First', 'Second', 'Third']


    def csco_12_6148_eq(self):
        self.InputMax = 4
