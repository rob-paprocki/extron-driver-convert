...
class DeviceClass:
    def __init__(self, ipAddress, port, devicePassword):
        ...
        self.Commands = {
            ...
            'AuxiliaryLayerSource': {'Parameters': ['Auxiliary', 'Layer', 'Target', 'Source Type'], 'Status': {}},
            'AuxiliaryLayerSourceID': {'Parameters': ['Auxiliary', 'Layer', 'Target'], 'Status': {}},
            'AuxiliaryLayerSourceType': {'Parameters': ['Auxiliary', 'Layer', 'Target'], 'Status': {}},
            ...
            'ScreenBackgroundLayerSource': {'Parameters': ['Screen', 'Target', 'Source Type'], 'Status': {}},
            'ScreenBackgroundLayerSourceID': {'Parameters': ['Screen', 'Target'], 'Status': {}},
            'ScreenBackgroundLayerSourceStatus': {'Parameters': ['Screen', 'Target'], 'Status': {}},
            'ScreenBackgroundLayerSourceType': {'Parameters': ['Screen', 'Target'], 'Status': {}},
            ...
            'SourceBackgroundSetEmpty': {'Parameters': ['Screen', 'Background Set'], 'Status': {}},
            'SourceBackgroundSetValid': {'Parameters': ['Screen', 'Background Set'], 'Status': {}},
        }
    ...
    def SetLogin(self, value, qualifier):
        if self.devicePassword:
            LoginCmdString = '/auth/login?identifier=Admin&password={}'.format(self.devicePassword)
            self.__SetHelper('Login', value, qualifier, url=LoginCmdString, data=None)
        else:
            self.Error(['Authentication is required, but the configured password is blank.'])
    ...
    def SetAuxiliaryLayerSource(self, value, qualifier):
        ...
        if all([1 <= auxiliary <= 96,
                1 <= layer <= 8,
                target in TargetStates,
                source_type in SourceTypeStates,
                1 <= value <= 24]):
            AuxiliaryLayerSourceCmdString = '/api/tpp/v1/auxiliary-screens/{auxiliary}/layers/{layer}/presets/{target}/source'.format(auxiliary=int(auxiliary), layer=int(layer), target=target.lower())
            data = {
    ...
    def UpdateAuxiliaryLayerSourceID(self, value, qualifier):
        ...
        if 1 <= auxiliary <= 96 and 1 <= layer <= 8 and target in TargetStates:
            AuxiliaryLayerSourceIDCmdString = '/api/tpp/v1/auxiliary-screens/{auxiliary}/layers/{layer}/presets/{target}/source'.format(auxiliary=int(auxiliary), layer=int(layer), target=target.lower())
            res = self.__UpdateHelper('AuxiliaryLayerSourceID', value, qualifier, url=AuxiliaryLayerSourceIDCmdString)
            if res:
                try:
                    value = str(res.get('sourceId', 'None'))
            ...
            self.Discard('Invalid Command for UpdateAuxiliaryLayerSourceID')
    ...
    def SetAuxiliaryPresetRecall(self, value, qualifier):
        ...
        if 1 <= auxiliary <= 96 and target in TargetStates and 1 <= value <= 1000:
    ...
    def SetGlobalTake(self, value, qualifier):
        ...
        try:
            ...
            return
        if (screens or auxiliary_screens) and all(1 <= i <= 20 for i in (screens + auxiliary_screens)):
    ...
    def UpdateMultiviewerSourceID(self, value, qualifier):
        ...
        if 1 <= multiviewer <= 2 and 1 <= widget <= 64:
            ...
            self.Discard('Invalid Command for UpdateMultiviewerSourceID')

    def UpdateMultiviewerSourceType(self, value, qualifier):

        self.UpdateMultiviewerSourceID(value, qualifier)

    def SetScreenBackgroundLayerSource(self, value, qualifier):
        ...
        if all([1 <= screen <= 24,
                ...
                source_type in SourceTypeStates,
                1 <= value <= 8]):
            ScreenBackgroundLayerSourceCmdString = '/api/tpp/v1/screens/{screen}/background-layer/presets/{target}/source'.format(screen=int(screen), target=target.lower())
            data = {
                'sourceType': SourceTypeStates[source_type],
                'sourceId': int(value)
            }

            self.__SetHelper('ScreenBackgroundLayerSource', value, qualifier, url=ScreenBackgroundLayerSourceCmdString, data=data)
        else:
            self.Discard('Invalid Command for SetScreenBackgroundLayerSource')

    def UpdateScreenBackgroundLayerSourceID(self, value, qualifier):
        ...
        if 1 <= screen <= 24 and target in TargetStates:
            ScreenBackgroundLayerSourceIDCmdString = '/api/tpp/v1/screens/{screen}/background-layer/presets/{target}'.format(screen=int(screen), target=target.lower())
            res = self.__UpdateHelper('ScreenBackgroundLayerSourceID', value, qualifier, url=ScreenBackgroundLayerSourceIDCmdString)
            if res:
                try:
                    value = str(res.get('sourceId', 'None'))
                    self.WriteStatus('ScreenBackgroundLayerSourceID', value, qualifier)
                except (ValueError, KeyError):
                    self.Error(['Screen Background Layer Source ID: Invalid/unexpected response'])
                ...
                try:
                    ...
                    self.WriteStatus('ScreenBackgroundLayerSourceStatus', value, qualifier)
                except KeyError:
                    self.Error(['Screen Background Layer Source Status: Invalid/unexpected response'])
                ...
                try:
                    ...
                    value = ValueStateValues[res['sourceType']]
                    self.WriteStatus('ScreenBackgroundLayerSourceType', value, qualifier)
                except KeyError:
                    self.Error(['Screen Background Layer Source Type: Invalid/unexpected response'])
        else:
            self.Discard('Invalid Command for UpdateScreenBackgroundLayerSourceID')

    def UpdateScreenBackgroundLayerSourceStatus(self, value, qualifier):

        self.UpdateScreenBackgroundLayerSourceID(value, qualifier)

    def UpdateScreenBackgroundLayerSourceType(self, value, qualifier):
    ...
    def UpdateScreenLayerSourceID(self, value, qualifier):
        ...
        if 1 <= screen <= 24 and 1 <= layer <= 128 and target in TargetStates:
            ...
            self.Discard('Invalid Command for UpdateScreenLayerSourceID')
    ...
    def SetScreenPresetRecall(self, value, qualifier):
        ...
        if 1 <= screen <= 24 and target in TargetStates and 1 <= value <= 1000:
    ...
    def UpdateSourceBackgroundSetEmpty(self, value, qualifier):

        screen = qualifier['Screen']
        background_set = qualifier['Background Set']

        if 1 <= screen <= 24 and 1 <= background_set <= 8:
            SourceBackgroundSetEmptyCmdString = '/api/tpp/v1/screens/{screen}/background-sets/{background_set}'.format(screen=screen, background_set=background_set)
            res = self.__UpdateHelper('SourceBackgroundSetEmpty', value, qualifier, url=SourceBackgroundSetEmptyCmdString)
            if res:
                try:
                    ValueStateValues = {
                        True: 'True',
                    ...
                    value = ValueStateValues[res['isEmpty']]
                    self.WriteStatus('SourceBackgroundSetEmpty', value, qualifier)
                except KeyError:
                    self.Error(['Source Background Set Empty: Invalid/unexpected response'])
                ...
                try:
                    ...
                    value = ValueStateValues[res['isValid']]
                    self.WriteStatus('SourceBackgroundSetValid', value, qualifier)
                except KeyError:
                    self.Error(['Source Background Set Valid: Invalid/unexpected response'])
        else:
            self.Discard('Invalid Command for UpdateSourceBackgroundSetEmpty')

    def UpdateSourceBackgroundSetValid(self, value, qualifier):

        self.UpdateSourceBackgroundSetEmpty(value, qualifier)

    def __CheckResponseForErrors(self, sourceCmdName, response):

        try:
            if sourceCmdName != 'Login':
...
class HTTPClass(DeviceClass):
    def __init__(self, ipAddress, port, devicePassword=None, Model=None):
        self.ConnectionType = 'HTTP'
        DeviceClass.__init__(self, ipAddress, port, devicePassword)
