...
class DeviceClass:
    def __init__(self):
        ...
        self.Commands = {
            ...
            'Clock': { 'Status': {}},
            'PowerInlineMode': {'Parameters':['Port Number','Time Range Name'], 'Status': {}},
        }
    ...
    def __MatchClock(self, match, tag):
        ...
        self.WriteStatus('Clock', value, None)

    def SetPowerInlineMode(self, value, qualifier):
        
        ValueStateValues = {
            'Auto' : 'auto', 
            'Never' : 'never'
        }
        if 1 <= qualifier['Port Number'] <= 65535:
            if qualifier['Time Range Name']:
                PowerInlineModeCmdString = 'configure\rinterface gi{}\rpower inline {} time-range {}\rexit\r'.format(qualifier['Port Number'], ValueStateValues[value], qualifier['Time Range Name'])
            else:
                PowerInlineModeCmdString = 'configure\rinterface gi{}\rpower inline {}\rexit\r'.format(qualifier['Port Number'], ValueStateValues[value])
            self.__SetHelper('PowerInlineMode', PowerInlineModeCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetPowerInlineMode')
    ...
    def __UpdateHelper(self, command, commandstring, value, qualifier):
        ...
        if self.Authentication == 'Successful':
