...
class DeviceClass:
    def __init__(self):
        ...
        self.Commands = {
            ...
            'SpeakerSetup': {'Parameters':['Output'], 'Status': {}},
            'Temperature': { 'Status': {}},
            }
        ...
        if self.Unidirectional == 'False':
            ...
            self.AddMatchString(re.compile(b'CntcM1\*([0-1])\r\n'), self.__MatchRemotePort, None)
            self.AddMatchString(re.compile(b'SpkcN([0-2])\*([0-2])\*([0-2])\*([0-2])\r\n'), self.__MatchSpeakerSetup, None)
    ...
    def SetGlobalAudioMute(self, value, qualifier):
        ...
        commandString = '{}Z'.format(valueStateValues[value])
        self.__SetHelper('GlobalAudioMute', commandString, value, qualifier)

    def UpdateGlobalAudioMute(self, value, qualifier):

        commandString = 'Z'
        self.__UpdateHelper('GlobalAudioMute', commandString, value, qualifier)
    ...
    def UpdateRemotePort(self, value, qualifier):

        cmdString = 'wM1CNTC\r'
        self.__UpdateHelper('RemotePort', cmdString, value, qualifier)
    ...
    def __MatchRemotePort(self, match, tag):

        RemotePortValues = {
            '1': 'Mute outputs when pins are open',
            '0': 'Mute outputs when pins are closed',
        }
        value = RemotePortValues[match.group(1).decode()]
        self.WriteStatus('RemotePort', value, None)

    def UpdateSpeakerSetup(self, value, qualifier):

        cmdString = 'wNSPKC\r'
        self.__UpdateHelper('SpeakerSetup', cmdString, value, qualifier)

    def __MatchSpeakerSetup(self, match, tag):
        ...
        speaker1 = speakerValues[match.group(1).decode()]
        self.WriteStatus('SpeakerSetup', speaker1, {'Output': '1'})
        speaker2 = speakerValues[match.group(2).decode()]
        self.WriteStatus('SpeakerSetup', speaker2, {'Output': '2'})
        speaker3 = speakerValues[match.group(3).decode()]
        self.WriteStatus('SpeakerSetup', speaker3, {'Output': '3'})
        speaker4 = speakerValues[match.group(4).decode()]
        self.WriteStatus('SpeakerSetup', speaker4, {'Output': '4'})
