...
class DeviceClass:
    def __init__(self):
        ...
        if self.Unidirectional == 'False':
            ...
            self.AddMatchString(re.compile(b'USB Active:?\s+(false|true)\r\n'), self.__MatchUSBActive, None)
        ...
        self.Regex = {
            'AudioMute': re.compile(b'mute:\s+(on|off)\r\n'),
            'AudioVolume': re.compile(b'volume:?\s+(-?\d+\.?\d+) dB\r\n'),
            'VideoMute': re.compile(b'mute:\s+(on|off)\r\n')
        }
    ...
    def UpdateAudioMute(self, value, qualifier):
        ...
        if channel in ['Master', 'Easy Mic 1', 'Easy Mic 2', 'USB Playback', 'Line Out 1', 'USB Record', 'IP Stream']:
            ...
            res = self.__UpdateSyncHelper('AudioMute', AudioMuteCmdString, value, qualifier)
            if res:
                try:
    ...
    def UpdateAudioVolume(self, value, qualifier):
        ...
        if channel in ['Master', 'Easy Mic 1', 'Easy Mic 2', 'USB Playback', 'Line Out 1', 'USB Record', 'IP Stream']:
            ...
            res = self.__UpdateSyncHelper('AudioVolume', AudioVolumeCmdString, value, qualifier)
            if res:
                try:
    ...
    def UpdateUSBActive(self, value, qualifier):

        USBActiveCmdString = 'streaming settings get\r\n'
        self.__UpdateHelper('USBActive', USBActiveCmdString, value, qualifier)

    def __MatchUSBActive(self, match, tag):

        value = match.group(1).decode().title()
        self.WriteStatus('USBActive', value, None)
    ...
    def UpdateVideoMute(self, value, qualifier):
        ...
        res = self.__UpdateSyncHelper('VideoMute', VideoMuteCmdString, value, qualifier)
        if res:
            try:
                value = re.search('mute:\s+(on|off)\r\n', res).group(1).title()
