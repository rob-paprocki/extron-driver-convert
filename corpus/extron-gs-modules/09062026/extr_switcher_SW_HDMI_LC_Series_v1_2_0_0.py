...
class DeviceClass:
    def __init__(self):
        ...
        self.Debug = False
        self.Models = {
            'SW4 HDMI LC': self.extr_2_2519_4,
            'SW2 HDMI LC': self.extr_2_2519_2,
            }
        self.Commands = {
    ...
    def UpdateInput(self, value, qualifier):
        ...
        if res:
            try:
                input_ = self.InputStates[res[1:2]]
                self.WriteStatus('Input', input_, qualifier)
            except(KeyError, IndexError):
                self.Error(['Input: Invalid/unexpected response'])
            try:
                video_mute = VideoMuteValues[res[9:10]]
                self.WriteStatus('VideoMute', video_mute, qualifier)
            except(KeyError, IndexError):
            ...
            if res:
                try:
                    for x in range(1, self.InputSize + 1):
                        ...
                        value = ValueStateValues[temp[2 + x]]
    ...
    def __CheckResponseForErrors(self, sourceCmdName, response):
        ...
        if response[0:3] in DEVICE_ERROR_CODES:
            self.Error(['{0} {1}'.format(sourceCmdName, DEVICE_ERROR_CODES[response[0:2]])])
            response = ''
        return response
