...
class DeviceClass:
    def __init__(self):
        ...
        self.Commands = {
            ...
            'Firmware': { 'Status': {}},
            'Pair': {'Parameters':['Transmitter','Receiver'], 'Status': {}},
            'Reboot': { 'Status': {}},
            'Resolution': {'Parameters':['Receiver'], 'Status': {}},
        }
    ...
    def SetPair(self, value, qualifier):
        ...
        TxVal = qualifier['Transmitter']
        RxVal = qualifier['Receiver']
        if 1 <= int(TxVal) <= 128 and 1 <= int(RxVal) <= 128:
            PairCmdString = '!Switch={0},{1}\n'.format(TxVal, RxVal)
            self.__SetHelper('Pair', PairCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetPair')
    ...
    def SetResolution(self, value, qualifier):
        ...
        RxVal = qualifier['Receiver']
        if 1 <= int(RxVal) <= 128:
            ResolutionCmdString = '!Resolution={0},{1}\n'.format(RxVal, ResolutionState[value])
