...
class DeviceClass:
    def __init__(self):
        ...
        self.Commands = {
            ...
            'AlarmRelay': {'Parameters': ['Pulse Duration'], 'Status': {}},
            'AlarmRelayPulseDuration': { 'Status': {}},
        ...
        if self.Unidirectional == 'False':
            self.AddMatchString(compile(b'Cpn0?1 Rly([01])\r\n'), self.__MatchAlarmRelay, None)
            self.AddMatchString(compile(b'Exe([01])\r\n'), self.__MatchExecutiveMode, None)
            self.AddMatchString(compile(b'Cpn0([1-4]) Pcs([0-2])\r\n'), self.__MatchOutputReferenceThreshold, None)
            self.AddMatchString(compile(b'Cpn0([1-4]) Ppc([01])\r\n'), self.__MatchPowerControl, None)
    ...
    def SetAlarmRelay(self, value, qualifier):
        ...
        if value in ValueStateValues:
            if value != 'Pulse':
                AlarmRelayCmdString = '1*{}O\r'.format(ValueStateValues[value])
            else:
                pulse_duration = qualifier['Pulse Duration']
                if pulse_duration is None:
                    pulse_duration = 0
                pulse_duration //= 20

                AlarmRelayCmdString = '1*3*{}O\r'.format(pulse_duration)
                
            self.__SetHelper('AlarmRelay', AlarmRelayCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetAlarmRelay')

    def UpdateAlarmRelay(self, value, qualifier):

        AlarmRelayCmdString = '1O\r'
        self.__UpdateHelper('AlarmRelay', AlarmRelayCmdString, value, qualifier)

    def __MatchAlarmRelay(self, match, tag):
        ...
        self.WriteStatus('AlarmRelay', value, None)
    ...
    def SetOutputReferenceThreshold(self, value, qualifier):
        ...
        if 1 <= output <= 4 and value in ValueStateValues:
            OutputReferenceThresholdCmdString = 'w{}*{}TH\r'.format(output, ValueStateValues[value])
            self.__SetHelper('OutputReferenceThreshold', OutputReferenceThresholdCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetOutputReferenceThreshold')
    ...
    def UpdateOutputReferenceThreshold(self, value, qualifier):
        ...
        if 1 <= output <= 4:
            OutputReferenceThresholdCmdString = 'w{}PS\r'.format(output)
            self.__UpdateHelper('OutputReferenceThreshold', OutputReferenceThresholdCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for UpdateOutputReferenceThreshold')
    ...
    def __MatchOutputReferenceThreshold(self, match, tag):
        ...
        self.WriteStatus('OutputReferenceThreshold', value, qualifier)
    ...
    def UpdatePowerControl(self, value, qualifier):
        ...
        if 1 <= output <= 4:
            ...
            self.Discard('Invalid Command for UpdatePowerControl')
    ...
    def __MatchError(self, match, tag):
        ...
        code = int(match.group(1).decode())
        msg = error_map.get(code, 'Unknown error')

        self.Error(['An error occured: {}: {}.'.format(code, msg)])
