...
class DeviceClass:
    def __init__(self):
        ...
        self._DeviceID = 0
        self.Models = {
            'WM55B': self.smsg_39_6098_65_55,
            'WM65B': self.smsg_39_6098_65_55,
            'WM75B': self.smsg_39_6098_85_75,
            'WM85B': self.smsg_39_6098_85_75,
        }
        ...
        if self.Unidirectional == 'False':
            self.AddMatchString(re.compile(b'\xAA\xFF[\x00-\xFD]\x03\x41\x14([\x21\x23\x25\x31\x50\x62\x64-\x66])[\x00-\xFF]'), self.__MatchInput, None)
    ...
    def DeviceID(self, value):
        if value == 'Broadcast':
            ...
            self.Error(['Device ID set to an invalid value. It should be a number between 0-253 or Broadcast'])

    def SetInput(self, value, qualifier):
        
        if value in self.SetInputValues:
            cks = (0x14 + self._DeviceID + 0x01 + self.SetInputValues[value]) & 0xFF
            InputCmdString = pack('6B', 0xAA, 0x14, self._DeviceID, 0x01, self.SetInputValues[value], cks)
    ...
    def __MatchInput(self, match, tag):
        ...
        value = self.UpdateInputValues[match.group(1)]
