...
class DeviceClass:
    ...
    def __init__(self):
        ...
        self.Debug = False
        self.Models = {
            'MTPX Plus 128': self.extr_15_25_128,
            'MTPX Plus 1616': self.extr_15_25_1616,
            'MTPX Plus 1632': self.extr_15_25_1632,
            'MTPX Plus 168': self.extr_15_25_168,
            'MTPX Plus 3216': self.extr_15_25_3216,
            'MTPX Plus 3232': self.extr_15_25_3232,
            'MTPX Plus 816': self.extr_15_25_816,
            }
        ...
        self.Commands = {
            ...
            'RS232OutputInsert': {'Parameters': ['Output'], 'Status': {}},
        ...
        if self.Unidirectional == 'False':
            ...
            self.AddMatchString(compile(b'Out([0-8]) Vol(\d{2})\r\n'), self.__MatchVolume, None)
            ...
            self.AddMatchString(compile(b'Lrpt(\d+)\*([0-1])\r\n'), self.__MatchRS232OutputInsert, None)
    ...
    def SetInputGain(self, value, qualifier):
        ...
        if 1 <= int(inputValue) <= self.InputSize and -18 <= value <= 24:
            if value < 0:
                InputGainCmdString = '{0}*{1}g\r'.format(inputValue, abs(value))
            else:
                InputGainCmdString = '{0}*{1}G\r'.format(inputValue, value)
    ...
    def SetRS232OutputInsert(self, value, qualifier):
        ...
        channel = qualifier['Output']
        if 1 <= int(channel) <= self.OutputSize:
            RS232OutputInsertCmdString = 'W{0}*{1}Lrpt\r'.format(channel, ValueStateValues[value])
            self.__SetHelper('RS232OutputInsert', RS232OutputInsertCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetRS232OutputInsert')

    def UpdateRS232OutputInsert(self, value, qualifier):

        channel = qualifier['Output']
        if 1 <= int(channel) <= self.OutputSize:
            self.__UpdateHelper('RS232OutputInsert', 'W{0}Lrpt\r'.format(channel), value, qualifier)
        else:
            self.Discard('Invalid Command for UpdateRS232OutputInsert')

    def __MatchRS232OutputInsert(self, match, tag):
        ...
        self.WriteStatus('RS232OutputInsert', OutputInsertName[match.group(2)], {'Output': str(int(match.group(1)))})
    ...
    def __MatchVolume(self, match, fncN):
        self.WriteStatus('Volume', int(match.group(2).decode()), {'Output': str(int(match.group(1)))})
    ...
    def extr_15_25_128(self):
        self.InputSize = 12
        self.OutputSize = 8

    def extr_15_25_1616(self):
        self.InputSize = 16
        self.OutputSize = 16

    def extr_15_25_1632(self):
        self.InputSize = 16
        self.OutputSize = 32

    def extr_15_25_168(self):
        self.InputSize = 16
        self.OutputSize = 8

    def extr_15_25_3216(self):
        self.InputSize = 32
        self.OutputSize = 16

    def extr_15_25_3232(self):
        self.InputSize = 32
        self.OutputSize = 32

    def extr_15_25_816(self):
        self.InputSize = 8
        self.OutputSize = 16
