...
class DeviceClass:
    ...
    def __init__(self):
        ...
        self.Commands = {
            'DimLevel': {'Parameters': ['Application Address', 'Group Address', 'Dimming Rate'], 'Status': {}},
            'SwitchLight': {'Parameters': ['Application Address', 'Group Address'], 'Status': {}},
        }
    ...
    def Checksum(self, value):
        if value in ['Enabled','Disabled']:
            ...
            print('Invalid value for Checksum, value can be Enabled or Disabled')      
    ...
    def SetDimLevel(self, value, qualifier):
        ...
        AppAddress = int(qualifier['Application Address'])
        GroupAddress = int(qualifier['Group Address'])
        DimmingRate = ord(DimmingRateStates[qualifier['Dimming Rate']])
        DimmingLevel = value

        if (
            AppAddressConstraints['Min'] <= AppAddress <= AppAddressConstraints['Max'] and
            DimmingLevelConstraints['Min'] <= DimmingLevel <= DimmingLevelConstraints['Max'] and
            GroupAddressConstraints['Min'] <= GroupAddress <= GroupAddressConstraints['Max']
        ):
            if self._Checksum == 'Enabled':
                CKS = (~((0x05 + AppAddress + 0x00 + DimmingRate + GroupAddress + DimmingLevel) % 0x100) + 1) & 0xFF
                DimLevelCmdString = '\\05{:02X}00{:02X}{:02X}{:02X}{:02X}\r'.format(AppAddress, DimmingRate, GroupAddress, DimmingLevel, CKS)
            else:
                DimLevelCmdString = '\\05{:02X}00{:02X}{:02X}{:02X}\r'.format(AppAddress, DimmingRate, GroupAddress, DimmingLevel)
    ...
    def __MatchDimLevel(self, match, tag):
        ...
        qualifier = {}
        qualifier['Application Address'] = str(int(match.group(1).decode(), 16))
    ...
    def SetSwitchLight(self, value, qualifier):
        ...
        if (
            ...
            if self._Checksum == 'Enabled':
                CKS = (~((0x05 + AppAddress + int(SwitchLightState) + GroupAddress) % 0x100) + 1) & 0xFF
                SwitchLightCmdString = '\\05{:02X}00{:02X}{:02X}{:02X}\r'.format(AppAddress, SwitchLightState, GroupAddress, CKS)
            else:
                SwitchLightCmdString = '\\05{:02X}00{:02X}{:02X}\r'.format(AppAddress, SwitchLightState, GroupAddress)
...
class EthernetClass(EthernetClientInterface, DeviceClass):
    ...
    def Connect(self, *args, **kwargs):
        ...
        if result == 'Connected':
            self.SendAndWait('~~~\r', 1, deliTag='\r')  # response not documented, but deliTag added in case there is one
            self.SendAndWait('A3300051\r', 1, deliTag='\r')
        return result
