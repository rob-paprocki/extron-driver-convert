...
class DeviceClass:
    ...
    def __init__(self):
        ...
        if self.Unidirectional == 'False':
            ...
            self.UpdateDelim = {
                'AspectRatioStatus': self.GetRegAspectRatio,
    ...
    def UpdateAspectRatioStatus(self, value, qualifier):
        ...
        AspectRatioStatusCmdString = b'\x03\x04\x00\x00\x03\x18\x00\x00\x22'
        ...
        if res:
            try:
                if int(res[5]) != 0x02:
                    ...
                    if int(res[12]) in [0x0B, 0x0D]:
                        input_ = self.ReadStatus('Input', qualifier)
                        index = 0 if input_ == 'Computer' else 1
                        value = ValueStateValues[int(res[12])][index]
                    else:
                        value = ValueStateValues[int(res[12])]
                self.WriteStatus('AspectRatioStatus', value, qualifier)
            except (KeyError, ValueError, IndexError):
                print('Aspect Ratio Status: Invalid/Unexpected Response for UpdateAspectRatioStatus')
    ...
    def __SetHelper(self, command, commandstring, value, qualifier):
        ...
        if self.Unidirectional == 'True' or command in 'UserDefinedCommand':
            ...
            if not res:
                print('{0}: Invalid/Unexpected or No Response'.format(command))
            else:
