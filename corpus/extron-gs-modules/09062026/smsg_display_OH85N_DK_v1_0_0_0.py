...
class DeviceClass:
    def __init__(self):
        ...
        if self.Unidirectional == 'False' and self._DeviceID != 254:
            self.AddMatchString(re.compile(b'\xAA\xFF[\x00-\xE0]\x03\x41\x14([\x21-\x25])[\x00-\xFF]'), self.__MatchInput, None)
    ...
    def SetVideoWallSize(self, value, qualifier):
        ...
        if 1 <= horizontal <= 15 and 1 <= vertical <= 15 and 1 <= valueInt <= 100:
