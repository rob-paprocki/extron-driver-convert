...
class DeviceClass:
    def __init__(self):
        ...
        if self.Unidirectional == 'False':
            ...
            self.AddMatchString(re.compile(b'b [0-9a-f]{2} OK(00|01|10|11|20|21|40|41|60|90|91|92)x', re.I), self.__MatchInput, None)
            self.AddMatchString(re.compile(b'l [0-9a-f]{2} OK0([01])x', re.I), self.__MatchOSDScreenDisplay, None)
            self.AddMatchString(re.compile(b'a [0-9a-f]{2} OK0([01])x', re.I), self.__MatchPower, None)
            self.AddMatchString(re.compile(b'd [0-9a-f]{2} OK0([01])x', re.I), self.__MatchVideoMute, None)
            self.AddMatchString(re.compile(b'f [0-9a-f]{2} OK([0-5][0-9a-f]|6[0-4])x', re.I), self.__MatchVolume, None)
            self.AddMatchString(re.compile(b'([cembladf]) ([0-9a-f]{1,2}) NG(.*?)x', re.I), self.__MatchError, None)
    ...
    def SetChannelKeypad(self, value, qualifier):
        ...
        self.__SetHelper('ChannelKeypad', CmdString, value, qualifier)
    ...
    def SetInput(self, value, qualifier):
        ...
        InputCmdString = 'xb {0} {1}\r'.format(self._DeviceID, States[value])
    ...
    def SetOSDScreenDisplay(self, value, qualifier):
        ...
        self.__SetHelper('OSDScreenDisplay', OnScreenDisplayCmdString, value, qualifier)

    def UpdateOSDScreenDisplay(self, value, qualifier):

        OnScreenDisplayCmdString = 'kl {0} FF\r'.format(self._DeviceID)
        self.__UpdateHelper('OSDScreenDisplay', OnScreenDisplayCmdString, value, qualifier)

    def __MatchOSDScreenDisplay(self, match, tag):
        ...
        value = ValueStateValues[match.group(1).decode()]
        self.WriteStatus('OSDScreenDisplay', value, None)
