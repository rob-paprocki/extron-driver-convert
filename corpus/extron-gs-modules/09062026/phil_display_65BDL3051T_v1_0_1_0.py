...
class DeviceClass:
    ...
    def SetVolume(self, value, qualifier):
        ...
        if 0 <= value <= 100:
            CmdString = pack('>6B', 0x07, self._DeviceID, self._GroupID, 0x44, value, value)
            ChkSum = self.calcChkSum(CmdString)
            CmdString = CmdString + ChkSum
    ...
    def SetVolumeStep(self, value, qualifier):
        ...
        CmdString = pack('>6B', 0x07, self._DeviceID, self._GroupID, 0x41, States[value], 0x00)
        ChkSum = self.calcChkSum(CmdString)
        CmdString = CmdString + ChkSum
    ...
    def UpdatePIP(self, value, qualifier):
        ...
        if res:
            try:
                if res[-5] == b'\x00':  # res index corresponds to PIP on/off byte
                    value = 'Off'
                else:
                    value = States[res[-4]]
