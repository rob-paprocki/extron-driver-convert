...
class DeviceClass:
    def __init__(self):
        ...
        if self.Unidirectional == 'False':
            self.AddMatchString(re.compile(br'\[ASPR!([0-5])\]'), self.__MatchAspectRatio, None)
            self.AddMatchString(re.compile(br'\[PMUT!([01])\]'), self.__MatchAVMute, None)
            self.AddMatchString(re.compile(br'\[DPMO!(\d|10)\]'), self.__MatchDisplayMode, None)
            self.AddMatchString(re.compile(br'\[MSRC!([0-6])\]'), self.__MatchInput, None)
            self.AddMatchString(re.compile(br'\[LSAT!(\d+?)\]'), self.__MatchLaserHours, None)
            self.AddMatchString(re.compile(br'\[LPMO!([0-2])\]'), self.__MatchLightSourceMode, None)
            self.AddMatchString(re.compile(br'\[LPTH!(\d+?)\]'), self.__MatchOperationHours, None)
            self.AddMatchString(re.compile(br'\[SSRC!([0-6])\]'), self.__MatchPIPInput, None)
    ...
    def SetLightSourceMode(self, value, qualifier):
        ...
        if value in ValueStateValues:
            LightSourceModeCmdString = '[LPMO{}]'.format(ValueStateValues[value])
            self.__SetHelper('LightSourceMode', LightSourceModeCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetLightSourceMode')

    def UpdateLightSourceMode(self, value, qualifier):

        LightSourceModeCmdString = '[LPMO?]'
