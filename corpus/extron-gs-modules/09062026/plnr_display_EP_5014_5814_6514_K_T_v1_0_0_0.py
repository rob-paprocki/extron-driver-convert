...
class DeviceClass:
    ...
    def __init__(self):
        ...
        if self.Unidirectional == 'False':
            ...
            self.AddMatchString(re.compile(b'\x07\x01\x00\x43\x41\x53([\x00-\x07])\x08'), self.__MatchAudioSource, None)
            self.AddMatchString(re.compile(b'\x07\x01\x00\x4D\x49\x4E([\x00\x09\x0A\x0B\x0C\x0D\x0E])\x08'), self.__MatchInput, None)
            self.AddMatchString(re.compile(b'\x07\x01\x00\x53\x43\x4D([\x00-\x04])\x08'), self.__MatchPictureMode, None)
            self.AddMatchString(re.compile(b'\x07\x01\x00\x50\x49([\x4E\x4F\x50])([\x00\x09\x0A\x0B\x0C\x0D\x0E])\x08'), self.__MatchPIPInput, None)
            self.AddMatchString(re.compile(b'\x07\x01\x00\x50\x53\x43([\x00-\x07])\x08'), self.__MatchPIPMode, None)
    ...
    def SetAudioSource(self, value, qualifier):
        ...
        AudioSourceCmdString = b'\x07\x01\x02\x43\x41\x53' + ValueStateValues[value] + b'\x08'
        self.__SetHelper('AudioSource', AudioSourceCmdString, value, qualifier)

    def UpdateAudioSource(self, value, qualifier):

        AudioSourceCmdString = b'\x07\x01\x01\x43\x41\x53\x08'
    ...
    def SetMenuControl(self, value, qualifier):
        ...
        MenuControlCmdString = b'\x07\x01\x02\x52\x43\x55' + ValueStateValues[value] + b'\x08'
    ...
    def SetVolume(self, value, qualifier):
        ...
        if ValueConstraints['Min'] <= value <= ValueConstraints['Max']:
            VolumeCmdString = b'\x07\x01\x02\x56\x4F\x4C' + pack('>B', value) + b'\x08'
    ...
    def __MatchVolume(self, match, tag):
        ...
        value = int(unpack('>B', match.group(1))[0])
