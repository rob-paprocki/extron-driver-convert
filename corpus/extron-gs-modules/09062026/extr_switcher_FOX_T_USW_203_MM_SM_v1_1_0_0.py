...
class DeviceClass:
    ...
    def __init__(self):
        ...
        self.Commands = {
            ...
            'AnalogAudioStatus': {'Status': {}},
            'AudioGainAttenuation': {'Status': {}},
            'AutoSwitchMode': {'Status': {}},
            'ExecutiveMode': {'Status': {}},
            'FiberLinkStatus': {'Parameters': ['Link'], 'Status': {}},
    ...
    def UpdateAnalogAudioStatus(self, value, qualifier):
        ...
        AnalogAudioStatusCmdString = '4S'
        res = self.__UpdateHelper('AnalogAudioStatus', AnalogAudioStatusCmdString, value, qualifier)
        if res:
            try:
                value = ValueStateValues[res[0]]
                self.WriteStatus('AnalogAudioStatus', value, qualifier)
            except (KeyError, IndexError):
                self.Error(['Analog Audio Status: Invalid/unexpected response'])
    ...
    def UpdateFiberLinkStatus(self, value, qualifier):
        ...
        link = qualifier['Link']
        if link in ['1', '2']:
            FiberLinkStatusCmdString = '{}S'.format(link)
            res = self.__UpdateHelper('FiberLinkStatus', FiberLinkStatusCmdString, value, qualifier)
            if res:
                try:
                    value = ValueStateValues[res[0]]
                    self.WriteStatus('FiberLinkStatus', value, qualifier)
                except (KeyError, IndexError):
                    self.Error(['Fiber Link Status: Invalid/unexpected response'])
        else:
    ...
    def SetHDCPInputAuthorization(self, value, qualifier):
        ...
        if 2 <= inputVal <= 3:
            HDCPInputAuthorizationCmdString = 'WE{0}*{1}HDCP\r'.format(inputVal, ValueStateValues[value])
    ...
    def UpdateHDCPInputAuthorization(self, value, qualifier):
        ...
        inputVal = int(qualifier['Input'])
        if 2 <= inputVal <= 3:
            HDCPInputAuthorizationCmdString = 'WE{0}HDCP\r'.format(inputVal)
    ...
    def SetInputAudioSelection(self, value, qualifier):
        ...
        inputVal = int(qualifier['Input'])
        if 2 <= inputVal <= 3:
            InputAudioSelectionCmdString = 'WI{0}*{1}AFMT\r'.format(inputVal, ValueStateValues[value])
    ...
    def UpdateInputAudioSelection(self, value, qualifier):
        ...
        inputVal = int(qualifier['Input'])
        if 2 <= inputVal <= 3:
            InputAudioSelectionCmdString = 'WI{0}AFMT\r'.format(inputVal)
            ...
            self.Discard('Invalid Command for UpdateInputAudioSelection')
