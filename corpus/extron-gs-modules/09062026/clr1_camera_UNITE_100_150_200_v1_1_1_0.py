...
class DeviceClass:
    def __init__(self):
        ...
        self.Commands = {
            ...
            'Focus': {'Status': {}},
            'Home': {'Status': {}},
            'ImageFlip': {'Parameters': ['Type'], 'Status': {}},
            'Iris': {'Status': {}},
            'PanTilt': {'Parameters': ['Tilt Speed', 'Pan Speed'], 'Status': {}},
            'Power': {'Status': {}},
    ...
    def DeviceID(self, value):
        if 1 <= int(value) <= 7:
            ...
            self.Error(['DeviceID should be a value between 1 - 7.'])
    ...
    def SetAutoFocus(self, value, qualifier):
        ...
        AutoFocusCmdString = bytes([self._DeviceID, 0x01, 0x04, 0x38, ValueStateValues[value], 0xFF])
    ...
    def UpdateAutoFocus(self, value, qualifier):
        ...
        AutoFocusCmdString = bytes([self._DeviceID, 0x09, 0x04, 0x38, 0xFF])
    ...
    def SetImageFlip(self, value, qualifier):
        ...
        if Type in TypeStates:
            ImageFlipCmdString = bytes([self._DeviceID, 1, 4, TypeStates[Type], ValueStateValues[value], 0xFF])
    ...
    def SetPanTilt(self, value, qualifier):
        ...
        PanSpeedConstraints = {
            'Min': 1,
            'Max': 24,
            'Value': int(qualifier['Pan Speed'])
        }
        TiltSpeedConstraints = {
            'Min': 1,
            'Max': 20,
            'Value': int(qualifier['Tilt Speed'])
        }
        if self.__ConstraintChecker(PanSpeedConstraints, TiltSpeedConstraints):
            PanTiltCmdString = bytes([self._DeviceID, 0x01, 0x06, 0x01, PanSpeedConstraints['Value'], TiltSpeedConstraints['Value'], ValueStateValues[value][0], ValueStateValues[value][1], 0xFF])
    ...
    def SetPreset(self, value, qualifier):
        ...
        if self.__ConstraintChecker(PresetConstraints) and Action in ActionStates:
            PresetCmdString = bytes([self._DeviceID, 0x01, 0x04, 0x3F, ActionStates[Action], PresetConstraints['Value'], 0xFF])
    ...
    def SetResetPanTilt(self, value, qualifier):
        ...
        ResetPanTiltCmdString = bytes([self._DeviceID, 0x01, 0x06, 0x05, 0xFF])
    ...
    def SetShutter(self, value, qualifier):
        ...
        ShutterCmdString = bytes([self._DeviceID, 0x01, 0x04, 0x0A, ValueStateValues[value], 0xFF])
    ...
    def SetZoom(self, value, qualifier):
        ...
        ZoomSpeedConstraints = {
            'Min': 0,
            'Max': 7,
            'Value': int(qualifier['Zoom Speed'])
        }
        if self.__ConstraintChecker(ZoomSpeedConstraints):
            if value == 'Stop':
                value = 0
            else:
                value = ValueStateValues[value] + ZoomSpeedConstraints['Value']
            ZoomCmdString = bytes([self._DeviceID, 0x01, 0x04, 0x07, value, 0xFF])
    ...
    def UpdateZoomPosition(self, value, qualifier):
        ...
        ZoomPositionCmdString = bytes([self._DeviceID, 9, 4, 0x47, 0xFF])
        self.__UpdateHelper('ZoomPosition', ZoomPositionCmdString, value, qualifier)
    ...
    def __CheckResponseForErrors(self, sourceCmdName, res):
        ...
        if len(res) < 3:
            self.Error(['{}: Invalid/unexpected response: too short'.format(sourceCmdName)])
            response = ''
        elif res[1] & 0xf0 in (0x50, 0x40):
            response = res
        elif res[1] & 0xf0 == 0x60:
            errorstring = DEVICE_ERROR_CODES[res[2]]
            self.Error(['{0}: {1}'.format(sourceCmdName, errorstring)])
            response = ''
        else:
