...
class DeviceClass:
    def __init__(self):
        ...
        if self.Unidirectional == 'False':
            self.AddMatchString(re.compile(b'OP ASPECT = (0|1|2|3|4|5|6|7)\r\n'), self.__MatchAspectRatio, None)
            self.AddMatchString(re.compile(b'OP INPUT.SEL = (0|1|2|3|4|5|6|7)\r\n'), self.__MatchInput, None)
            self.AddMatchString(re.compile(b'OP LAMPS = (0|1|2|3)\r\n'), self.__MatchLampMode, None)
            self.AddMatchString(re.compile(b'OP LAMP.MODE = (0|1|2)\r\n'), self.__MatchLampPower, None)
            self.AddMatchString(re.compile(b'OP LAMP(1|2).STAT = (0|1)\r\n'), self.__MatchLampStatus, None)
            self.AddMatchString(re.compile(b'OP LAMP(1|2).HOURS = ([0-9]{1,4})\r\n'), self.__MatchLampUsage, None)
            self.AddMatchString(re.compile(b'OP PROJ.RUNTIME = ([0-9]{1,4})\r\n'), self.__MatchOperationHours, None)
            self.AddMatchString(re.compile(b'OP STATUS = (0|1|2|3|4)\r\n'), self.__MatchPower, None)
            self.AddMatchString(re.compile(b'ERR:Invalid (State|Cmd)\r\n'), self.__MatchError, None)
    ...
    def SetLampMode(self, value, qualifier):
        ...
        LampModeCmdString = 'op lamps = {0}\r'.format(LampModeState[value])
        self.__SetHelper('LampMode', LampModeCmdString, value, qualifier)

    def UpdateLampMode(self, value, qualifier):

        LampModeCmdString = 'op lamps ?\r'
    ...
    def SetLampPower(self, value, qualifier):
        ...
        LampPowerState = {
            'Standard': '0',
            'Economy': '1',
            'Custom Power Level': '2'
        }

        LampPowerCmdString = 'op lamp.mode = {0}\r'.format(LampPowerState[value])
        self.__SetHelper('LampPower', LampPowerCmdString, value, qualifier)

    def UpdateLampPower(self, value, qualifier):

        LampPowerCmdString = 'op lamp.mode ?\r'
        self.__UpdateHelper('LampPower', LampPowerCmdString, value, qualifier)
    ...
    def __MatchLampPower(self, match, tag):
        ...
        value = LampPowerState[match.group(1).decode()]
        self.WriteStatus('LampPower', value, None)
    ...
    def __MatchLampStatus(self, match, tag):
        ...
        value = LampStatusState[match.group(2).decode()]
    ...
    def __MatchError(self, match, tag):
        ...
        errorstring = 'An error has occured: {0}'.format(DEVICE_ERROR_CODES[match.group(1).decode()])
