...
class DeviceSerialClass:
    def __init__(self):
        ...
        self.Models = {
            ...
            'PT-DZ21K2E': self.pana_1_407_4lamps
        }
        ...
        self.Commands = {
            ...
            'HorizontalLensShift': {'Parameters':['Speed'], 'Status': {}},
            'VerticalLensShift': {'Parameters':['Speed'], 'Status': {}},
    ...
    def DeviceID(self, value):
        ...
        if self._DeviceID.isnumeric():
            self._DeviceID = int(self._DeviceID)
            if 1 <= self._DeviceID <= 64:
                self._DeviceID = '{0:02}'.format(self._DeviceID)
            else:
                self.Error(['Invalid Device ID given, should be between 1 and 64, or between A and Z, or ZZ for broadcast.'])
        elif len(self._DeviceID) == 1 and self._DeviceID.isalpha():
            self._DeviceID = '0' + self._DeviceID
        elif self._DeviceID != 'ZZ':
            self.Error(['Invalid Device ID given, should be between 1 and 64, or between A and Z, or ZZ for broadcast.'])
    ...
    def UpdateDeviceStatus(self, value, qualifier):
        ...
        if res:
            try:
                matchObject = re.search(matchString, res)
                if matchObject:
                    value = int(matchObject.group(1).decode(),16)
    ...
    def SetFocus(self, value, qualifier):  
        ...
        if self._DeviceID and qualifier['Speed'] in SpeedStates and value in ValueStateValues:
            FocusCmdString = '\x02AD{0};VXX:LNSI4=+00{1}0{2}\x03'.format(self._DeviceID, ValueStateValues[value], SpeedStates[qualifier['Speed']])
    ...
    def SetInput(self, value, qualifier):
        ...
        InputStateValues = {
            ...
            'SDI2':                     'SD2',
            'Digital Link':             'DL1',
            'Digital Link - PC 1':      'DL1:PC1',
            'Digital Link - PC 2':      'DL1:PC2',
    ...
    def UpdateInput(self, value, qualifier): 
        ...
        InputStateNames = {
            ...
            'SD2': 'SDI2',
            'DL1': 'Digital Link',
            'DL1:PC1': 'Digital Link - PC 1',
            'DL1:PC2': 'Digital Link - PC 2',
            'DL1:VID': 'Digital Link - Video',
            'DL1:HD1': 'Digital Link - HDMI 1',
            'DL1:HD2': 'Digital Link - HDMI 2',
            'DL1:SVD': 'Digital Link - S-Video',
        }
        matchString = re.compile('\x02(RG1|RG2|VID|DVI|HD1|SD1|SD2|DL1:?(PC[12]|VID|HD[12]|SVD)?)\x03')
    ...
    def UpdateLampUsage(self, value, qualifier): 
        ...
        LampNumber = qualifier['Lamp Number']
            ...
            if res:
                try:
                    ...
                    if matchObject:
                        ...
                        self.WriteStatus('LampUsage', value, {'Lamp Number': LampNumber })
    ...
    def SetLensMemoryDelete(self, value, qualifier):
        ...
        if ValueConstraints['Min'] <= int(value) <= ValueConstraints['Max']:
            ...
            self.Discard('Invalid Command for SetLensMemoryDelete')
    ...
    def SetLensMemoryLoad(self, value, qualifier):
        ...
        if ValueConstraints['Min'] <= int(value) <= ValueConstraints['Max']:
            ...
            self.Discard('Invalid Command for SetLensMemoryLoad')
    ...
    def SetLensMemorySave(self, value, qualifier):
        ...
        if ValueConstraints['Min'] <= int(value) <= ValueConstraints['Max']:
            ...
            self.Discard('Invalid Command for SetLensMemorySave')
    ...
    def SetHorizontalLensShift(self, value, qualifier):
        ...
        if self._DeviceID and qualifier['Speed'] in SpeedStates and value in ValueStateValues:
            HorizontalLensShiftCmdString = '\x02AD{0};VXX:LNSI2=+00{1}0{2}\x03'.format(self._DeviceID, ValueStateValues[value], SpeedStates[qualifier['Speed']])
            self.__SetHelper('HorizontalLensShift', HorizontalLensShiftCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetHorizontalLensShift')
    ...
    def SetVerticalLensShift(self, value, qualifier):
        ...
        if self._DeviceID and qualifier['Speed'] in SpeedStates and value in ValueStateValues:
            VerticalLensShiftCmdString = '\x02AD{0};VXX:LNSI3=+00{1}0{2}\x03'.format(self._DeviceID, ValueStateValues[value], SpeedStates[qualifier['Speed']])
            self.__SetHelper('VerticalLensShift', VerticalLensShiftCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetVerticalLensShift')
    ...
    def SetZoom(self, value, qualifier):
        ...
        if qualifier['Speed'] in SpeedStates and value in ValueStateValues:
            ZoomCmdString = '\x02AD{0};VXX:LNSI5=+00{1}0{2}\x03'.format(self._DeviceID, SpeedStates[qualifier['Speed']], ValueStateValues[value])
    ...
    def __SetHelper(self, command, commandstring, value, qualifier):
        ...
        if self.Unidirectional == 'True' or self._DeviceID == 'ZZ':
            ...
            if not res:
                self.Error(['Set commands: No response received'])
            else:
    ...
    def __UpdateHelper(self, command, commandstring, value, qualifier):
        ...
        if self.Unidirectional == 'True' or self._DeviceID == 'ZZ':
            ...
            if not res:
                ...
                res = res.decode() if command != 'DeviceStatus' else res
                return self.__CheckResponseForErrors(command, res)

    def __CheckResponseForErrors(self, sourceCmdName, response):

        if isinstance(response,str) == False:
            res = response.decode('iso-8859-1')
        else: 
            res = response
        if 'ER40' in res:
            if '401' in res:
                self.Error(['Commands can not currently be recieved by the device.'])
            elif '402' in res:
                self.Error(['Device responded with a Parameter Error.'])
            else:
...
class DeviceEthernetClass:
    def __init__(self):
        ...
        if self.Unidirectional == 'False':
            ...
            self.AddMatchString(re.compile(b'00OFZ:(0|1)\r'), self.__MatchFreeze, None)
    ...
    def SetZoom(self, value, qualifier):
        ...
        if qualifier['Speed'] in SpeedStates and value in ValueStateValues:
            ZoomCmdString = '00VXX:LNSI5=+' + SpeedStates[qualifier['Speed']] + ValueStateValues[value] + '\r'
            self.__SetHelper('Zoom', ZoomCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetZoom')
    ...
    def __MatchError(self, match, tag):
        ...
        self.Error([ErrorValue[match.group(1).decode()]])
