...
class DeviceClass:
    def __init__(self):
        ...
        if self.Unidirectional == 'False':
            ...
            self.AddMatchString(re.compile(b'In([1-4]) All\r\n'), self.__MatchInput, None)
    ...
    def __MatchInputSignalStatus(self, match, tag):
        ...
        for input_num, signal in enumerate(val):
            self.WriteStatus('InputSignalStatus', InputValues[signal], {'Input': str(input_num + 1)})
    ...
    def __MatchOutputSignalStatus(self, match, tag):
        ...
        val = match.group(1).decode().split(' ')
        for output_num, signal in enumerate(val):
            self.WriteStatus('OutputSignalStatus', OutputValues[signal], {'Output': str(output_num + 1)})
    ...
    def __MatchPeripheralEmulation(self, match, tag):
        ...
        if tag == 'Unsolicited':
            ...
            val = match.group(1).decode().split(' ')
            for input_num, signal in enumerate(val):
                self.WriteStatus('PeripheralEmulation', ValueStateValues[signal], {'Input': str(input_num + 1)})
