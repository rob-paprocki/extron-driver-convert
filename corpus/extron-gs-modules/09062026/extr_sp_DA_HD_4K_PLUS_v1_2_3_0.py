...
class DeviceClass:
    def __init__(self):
        ...
        self.AddMatchString(re.compile(rb'Amt([1-6])\*([01])\r\n'), self.MatchAudioMute, 'Set')
        self.AddMatchString(re.compile(b'Amt([01 ]{2,11})\r\n'), self.MatchAudioMute, 'Update')
        self.AddMatchString(re.compile(b'Vmt([012])\r\n'), self.MatchGlobalVideoMute, None)
        self.AddMatchString(re.compile(rb'Vmt([1-6])\*([012])\r\n'), self.MatchVideoMute, 'Set')
        self.AddMatchString(re.compile(b'Vmt([012 ]{2,11})\r\n'), self.MatchVideoMute, 'Update')
        ...
        self.AddMatchString(re.compile(rb'HdcpO([1-6])\*([012])\r\n'), self.__MatchHDCPOutputStatus, None)
        self.AddMatchString(re.compile(rb'Vtpo([1-6])\*([1-6])\r\n'), self.__MatchOutputFormat, 'Set')
        self.AddMatchString(re.compile(b'Vtpo([123456 ]+)\r\n'), self.__MatchOutputFormat, 'Update')
        self.AddMatchString(re.compile(rb'Sig([01])\*([01 ]+)\r\n'), self.__MatchSignalStatus, None)
        self.AddMatchString(re.compile(b'Vrb3\r\n'), self.__MatchVerboseMode, None)
        self.AddMatchString(re.compile(b'E(01|10|13|14)\r\n'), self.__MatchError, None)
    ...
    def SetAudioMute(self, value, qualifier):
        ...
        if 1 <= output <= self.OutputSize and value in ValueStateValues:
            self.__SetHelper('AudioMute', '{0}*{1}Z'.format(output, ValueStateValues[value]), value, qualifier)
        else:
    ...
    def MatchAudioMute(self, match, tag):
        ...
        if tag in ['Set', 'Update']:
            if tag == 'Set':
                ...
                self.WriteStatus('AudioMute', ValueStateValues[state], {'Output': output})
            else:
    ...
    def MatchGlobalAudioMute(self, match, tag):
        ...
        state = ValueStateValues[match.group(1).decode()]
    ...
    def __MatchHDCPInputStatus(self, match, tag):
        ...
        self.WriteStatus('HDCPInputStatus', ValueStateValues[match.group(1).decode()], None)
    ...
    def __MatchHDCPOutputStatus(self, match, tag):
        ...
        self.WriteStatus('HDCPOutputStatus', ValueStateValues[match.group(2).decode()], {'Output': match.group(1).decode()})
    ...
    def __MatchSignalStatus(self, match, tag):
        ...
        for i in states:
            self.WriteStatus('OutputSignalStatus', ValueStateValues[i], {'Output': str(output)})
            output += 1
    ...
    def SetVideoMute(self, value, qualifier):
        ...
        output = int(qualifier['Output'])
        if 1 <= output <= self.OutputSize and value in ValueStateValues:
            VideoMuteCmd = '{0}*{1}B'.format(qualifier['Output'], ValueStateValues[value])
    ...
    def MatchVideoMute(self, match, tag):
        ...
        if tag in ['Set', 'Update']:
            if tag == 'Set':
                output, state = match.group(1).decode(), match.group(2).decode()
                self.WriteStatus('VideoMute', ValueStateValues[state], {'Output': output})
            else:
                output, states = 1, match.group(1).decode().split()
                for i in states:
                    self.WriteStatus('VideoMute', ValueStateValues[i], {'Output': str(output)})
                    output += 1
