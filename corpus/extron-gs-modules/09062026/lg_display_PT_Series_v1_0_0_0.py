...
class DeviceClass:
    def __init__(self):
        ...
        if self.Unidirectional == 'False':
            self.AddMatchString(re.compile(b'c [0-9a-f]{2} OK(01|02|04|06|07|09|1[0-9a-f])x'), self.__MatchAspectRatio, None)
            self.AddMatchString(re.compile(b'e [0-9a-f]{2} OK(01|00)x'), self.__MatchAudioMute, None)
            self.AddMatchString(re.compile(b'm [0-9a-f]{2} OK(01|00)x'), self.__MatchExecutiveMode, None)
            self.AddMatchString(re.compile(b'b [0-9a-f]{2} OK(1|2|4|6|9)([0-3])x'), self.__MatchInput, None)
            self.AddMatchString(re.compile(b'l [0-9a-f]{2} OK(01|00)x'), self.__MatchOnScreenDisplay, None)
            self.AddMatchString(re.compile(b'a [0-9a-f]{2} OK(01|00)x'), self.__MatchPower, None)
            ...
            self.AddMatchString(re.compile(b'(c|e|m|b|l|a|d|f) [0-9a-f]{2} NG(.*)x'), self.__MatchError, None)
    ...
    def SetChannel(self, value, qualifier):
        ...
        ChannelCmdString = 'mc {0} {1}\r'.format(self.DeviceID, ValueStateValues[value])
    ...
    def SetInput(self, value, qualifier):
        ...
        numberVal = qualifier['Number']
        if numberVal in NumberStates:
            InputCmdString = 'xb {0} {1}{2}\r'.format(self.DeviceID, ValueStateValues[value], NumberStates[numberVal])
    ...
    def __MatchInput(self, match, tag):
        ...
        qualifier = {}
        qualifier['Number'] = NumberStates[match.group(2).decode()]
        value = ValueStateValues[match.group(1).decode()]
    ...
    def __SetHelper(self, command, commandstring, value, qualifier):
        ...
        if self.Unidirectional == 'True' or self.DeviceID == '00' or command == 'UserDefinedCommand':
    ...
    def __MatchError(self, match, tag):
        ...
        errorstring = 'Error with Command: {0}, State: {1}'.format(CommandCode[match.group(1).decode()], match.group(2).decode())
