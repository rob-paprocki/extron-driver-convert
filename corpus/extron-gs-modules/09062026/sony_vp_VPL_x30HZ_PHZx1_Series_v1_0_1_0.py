...
class DeviceClass:
    def __init__(self):
        ...
        self.AspectRatioRegex = re.compile('(?P<value>\"(?:4_3|16_9|full2|normal|full|zoom)\")\r\n')
        self.AudioMuteRegex   = re.compile('(?P<value>\"(?:on|off)\")\r\n')
        self.FreezeRegex      = re.compile('(?P<value>\"(?:on|off)\")\r\n')
        self.InputRegex       = re.compile('(?P<value>\"(?:video1|hdmi2|rgb1|hdmi1|hdbaset1)\")\r\n') 
        self.PowerRegex       = re.compile('(?P<value>\"(?:on|off|standby|cooling1|saving_cooling1|saving_standby|startup)\")\r\n')
        self.VideoMuteRegex   = re.compile('(?P<value>\"(?:on|off)\")\r\n') 
        self.ExecutiveModeRegex = re.compile('(?P<value>\"(?:on|off)\")\r\n')
    ...
    def UpdateExecutiveMode(self, value, qualifier):
        ...
        if res:
            try:
                valueMatch = self.ExecutiveModeRegex.match(res)
                value = ValueStateValues[valueMatch.group('value')]
        ...
        if res:
            try:
                ValueStateValues = {
                    ...
                    '"off"'            : 'Off',
                    '"standby"'        : 'Standby',
                    '"saving_standby"' : 'Saving Standby',
                    '"cooling1"'       : 'Cooling Down',
                    '"saving_cooling1"': 'Saving Cooling',
                    '"startup"'        : 'Warming Up'
                    }
    ...
    def __CheckResponseForErrors(self, sourceCmdName, response):
        ...
        DEVICE_ERROR_CODES = {'err_cmd\r\n'      : 'Command format error.',
                              'err_option\r\n'   : 'Command option error.',
                              'err_inactive\r\n' : 'Command temporarily invalidated.',
                              'err_val\r\n'      : 'Value error.',
        ...
        if response.lower() in DEVICE_ERROR_CODES:
            self.Error(['{} Error: {}'.format(sourceCmdName, DEVICE_ERROR_CODES[response.lower()])])
            response = ""
        return response
