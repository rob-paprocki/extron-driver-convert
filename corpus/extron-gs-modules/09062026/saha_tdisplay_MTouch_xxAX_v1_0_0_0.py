...
class DeviceClass:
    ...
    def __init__(self):
        ...
        if self.Unidirectional == 'False':
            self.AddMatchString(re.compile(b'#\*headphone volume level is ([0-9]|[1-9][0-9]|100)[\r\n]'), self.__MatchHeadphoneVolume, None)
            self.AddMatchString(re.compile(b'#\*source is (HDMI|VGA|DP|OPS)1?[\r\n]'), self.__MatchInput, None)
            self.AddMatchString(re.compile(b'#\*MUTE (ON|OFF)[\r\n]'), self.__MatchMute, None)
            self.AddMatchString(re.compile(b'#\*Picture Mode is ([1-4])[\r\n]'), self.__MatchPictureMode, None)
            self.AddMatchString(re.compile(b'#\*Quick Standby is (off|on).?[\r\n]'), self.__MatchPower, None)
            self.AddMatchString(re.compile(b'#\*volume level is ([0-9]|[1-9][0-9]|100)[\r\n]'), self.__MatchVolume, None)
