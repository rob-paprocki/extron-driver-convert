...
class DeviceClass:
    def __init__(self):
        ...
        if self.Unidirectional == 'False':
            self.AddMatchString(compile(br'\(SZP!([0-8])\)'), self.__MatchAspectRatio, None)
            self.AddMatchString(compile(br'\(AIG!([01])\)'), self.__MatchAutoImage, None)
            self.AddMatchString(compile(br'\(FRZ!([01])\)'), self.__MatchFreeze, None)
            self.AddMatchString(compile(br'\(SIN\+MAIN!([134578])\)'), self.__MatchInput, None)
            self.AddMatchString(compile(br'\(LPM!([0-4])\)'), self.__MatchLampMode, None)
            self.AddMatchString(compile(br'\(OSD!([01])\)'), self.__MatchOnScreenDisplay, None)
            self.AddMatchString(compile(br'\(PIP!([01])\)'), self.__MatchPIP, None)
            self.AddMatchString(compile(br'\(SIN\+PIIP!([134578])\)'), self.__MatchPIPInput, None)
            self.AddMatchString(compile(br'\(PPP!([0-7])\)'), self.__MatchPIPPosition, None)
            self.AddMatchString(compile(br'\(PHS!([0-2])\)'), self.__MatchPIPSize, None)
            self.AddMatchString(compile(br'\(PWR!([01])\)'), self.__MatchPower, None)
            self.AddMatchString(compile(br'\(SHU!([01])\)'), self.__MatchVideoMute, None)
            self.AddMatchString(compile(br'\(\d+ \d+ (ERR\d+ \".+\")\)'), self.__MatchError, None)
    ...
    def SetAutoImage(self, value, qualifier):
        ...
        AutoImageCmdString = '(AIG {})'.format(ValueStateValues[value])
        self.__SetHelper('AutoImage', AutoImageCmdString, value, qualifier)

    def UpdateAutoImage(self, value, qualifier):

        AutoImageCmdString = '(AIG?)'
        self.__UpdateHelper('AutoImage', AutoImageCmdString, value, qualifier)

    def __MatchAutoImage(self, match, tag):
        ...
        value = ValueStateValues[match.group(1).decode()]
        self.WriteStatus('AutoImage', value, None)
    ...
    def SetPIP(self, value, qualifier):
        ...
        PIPCmdString = '(PIP {})'.format(ValueStateValues[value])
    ...
    def SetPIPInput(self, value, qualifier):
        ...
        PIPInputCmdString = '(SIN+PIIP {})'.format(ValueStateValues[value])
        self.__SetHelper('PIPInput', PIPInputCmdString, value, qualifier)

    def UpdatePIPInput(self, value, qualifier):

        PIPInputCmdString = '(SIN+PIIP?)'
    ...
    def SetPIPPosition(self, value, qualifier):
        ...
        PIPPositionCmdString = '(PPP {})'.format(ValueStateValues[value])
    ...
    def SetPIPSize(self, value, qualifier):
        ...
        PIPSizeCmdString = '(PHS {})'.format(ValueStateValues[value])
    ...
    def __MatchError(self, match, tag):
        self.counter = 0
        self.Error(['Device Response Error: ' + match.group(1).decode()])
