...
class DeviceClass:
    ...
    def __init__(self):
        ...
        if self.Unidirectional == 'False':
            self.AddMatchString(re.compile(b'HdcpE(1|0) 0\r\n|HdcpE1\*(0|1)\r\n|HdcpE(0|1)\r\n'), self.__MatchHDCPInputAuthorization, None)
            self.AddMatchString(re.compile(b'HdcpI(0|1|2)\r\n'), self.__MatchHDCPInputStatus, None)
            self.AddMatchString(re.compile(b'HdcpO(0|1|2)\r\n'), self.__MatchHDCPOutputStatus, None)
            self.AddMatchString(re.compile(b'Afmt(0|1)\r\n'), self.__MatchHDMIAnalogOutputAudioMute, None)
            self.AddMatchString(re.compile(b'Hsrt2\*([0-9]{1,3})\r\n'), self.__MatchHorizontalStart, None)
            self.AddMatchString(re.compile(b'In(0|1|2) All\r\n'), self.__MatchStatus, None)
            self.AddMatchString(re.compile(b'In(0|1|2) Aflw(0|1|2) Ausw(0|1|2) Vmt(0|1) Amt(0|1)\r\n|Amt(0|1)\r\n'), self.__MatchInput, None)
            self.AddMatchString(re.compile(b'Aflw(0|1|2)\r\n'), self.__MatchAnalogAudioInputSwitching, None)
    ...
    def SetAnalogAudioInputSwitching(self, value, qualifier):

        ValueStateValues = {
            'Always Output': 'w0AFLW\r',
            'HDMI Input': 'w1AFLW\r',
            'VGA Input': 'w2AFLW\r'
        }

        AnalogAudioInputSwitchingCmdString = ValueStateValues[value]
        self.__SetHelper('AnalogAudioInputSwitching', AnalogAudioInputSwitchingCmdString, value, qualifier)

    def UpdateAnalogAudioInputSwitching(self, value, qualifier):
        self.UpdateInput(value, qualifier)

    def __MatchAnalogAudioInputSwitching(self, match, tag):
        ...
        self.WriteStatus('AnalogAudioInputSwitching', value, None)
    ...
    def SetHDMIAnalogOutputAudioMute(self, value, qualifier):
        ...
        HDMIAnalogOutputAudioMuteCmdString = ValueStateValues[value]
        self.__SetHelper('HDMIAnalogOutputAudioMute', HDMIAnalogOutputAudioMuteCmdString, value, qualifier)

    def UpdateHDMIAnalogOutputAudioMute(self, value, qualifier):

        HDMIAnalogOutputAudioMuteCmdString = 'wAFMT\r'
        self.__UpdateHelper('HDMIAnalogOutputAudioMute', HDMIAnalogOutputAudioMuteCmdString, value, qualifier)

    def __MatchHDMIAnalogOutputAudioMute(self, match, tag):
        ...
        value = ValueStateValues[match.group(1).decode()]
        self.WriteStatus('HDMIAnalogOutputAudioMute', value, None)
    ...
    def __MatchInput(self, match, tag):
        ...
        if match.group(6):
            value = MuteValues[match.group(6).decode()]
            self.WriteStatus('AudioMute', value, None)
        else:
            self.WriteStatus('Input', InputValues[match.group(1).decode()], None)
            self.WriteStatus('AnalogAudioInputSwitching', SwitchingValues[match.group(2).decode()], None)
            self.WriteStatus('AutoSwitchMode', ModeValues[match.group(3).decode()], None)
            self.WriteStatus('VideoMute', MuteValues[match.group(4).decode()], None)
            self.WriteStatus('AudioMute', MuteValues[match.group(5).decode()], None)
