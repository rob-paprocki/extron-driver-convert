...
class DeviceClass:
    ...
    def __init__(self):
        ...
        if self.Unidirectional == 'False':
            self.AddMatchString(compile(b'c ([0-9a-f]{2}) OK(0[124679B]|1[0-9a-f])x', I), self.__MatchAspectRatio, None)
            self.AddMatchString(compile(b'e ([0-9a-f]{2}) OK0([01])x', I), self.__MatchAudioMute, None)
            self.AddMatchString(compile(b'm ([0-9a-f]{2}) OK0([01])x', I), self.__MatchExecutiveMode, None)
            self.AddMatchString(compile(b'b ([0-9a-f]{2}) OK0([24789])x', I), self.__MatchInput, None)
            self.AddMatchString(compile(b'l ([0-9a-f]{2}) OK0([01])x', I), self.__MatchOnScreenDisplay, None)
            self.AddMatchString(compile(b'a ([0-9a-f]{2}) OK0([10])x', I), self.__MatchPower, None)
            self.AddMatchString(compile(b'd ([0-9a-f]{2}) OK0([10])x', I), self.__MatchVideoMute, None)
            self.AddMatchString(compile(b'f ([0-9a-f]{2}) OK([0-5][0-9a-f]|6[0-4])x', I), self.__MatchVolume, None)
            self.AddMatchString(compile(b'([ucembladf]) ([0-9a-f]{2}) NG(.*)x', I), self.__MatchError, None)
    ...
    def SetChannelStep(self, value, qualifier):
        ...
        ID = self.GetDeviceID(qualifier['Device ID'])
        if ID:
            ChannelStepCmdString = 'mc {0} {1}\r'.format(ID, ValueStateValues[value])
    ...
    def SetOnScreenDisplay(self, value, qualifier):
        ...
        ID = self.GetDeviceID(qualifier['Device ID'])
        if ID:
            OnScreenDisplayCmdString = 'kl {0} {1}\r'.format(ID, ValueStateValues[value])
    ...
    def UpdateOnScreenDisplay(self, value, qualifier):
        ...
        ID = self.GetDeviceID(qualifier['Device ID'])
        if ID:
            OnScreenDisplayCmdString = 'kl {0} FF\r'.format(ID)
    ...
    def SetTileMode(self, value, qualifier):
        ...
        ID = self.GetDeviceID(qualifier['Device ID'])
        column_val = int(qualifier['Column'])
        row_val = int(qualifier['Row'])
        if ID and 1 <= row_val <= 15 and 1 <= column_val <= 15:
            TileModeCmdString = 'dd {0} {1:X}{2:X}\r'.format(ID, column_val, row_val)
    ...
    def SetTilePosition(self, value, qualifier):
        ...
        ID = self.GetDeviceID(qualifier['Device ID'])
        tileID_val = int(qualifier['Tile ID'])
        if ID and 1 <= tileID_val <= 225:
            TilePositionCmdString = 'di {0} {1:02X}\r'.format(ID, tileID_val)
