...
class DeviceClass:
    def __init__(self):
        ...
        self.Debug = False
        self.Models = {
            'NP-P554U': self.nec_1_3127_A,
            'NP-P474U': self.nec_1_3127_A,
            'NP-P554W': self.nec_1_3127_A,
            'NP-P474W': self.nec_1_3127_A,
            'NP-P604X': self.nec_1_3127_C,
            'NP-P603X': self.nec_1_3127_B,
        }
        ...
        self.SetAspectRatioRe = re.compile(b'(\x23\x10[\x00-\xFF]{6})|(\xA3\x10[\x00-\xFF]{6})')
        self.SetAudioMuteRe = re.compile(b'(\x22[\x00-\xFF]{3}\x00[\x00-\xFF])|(\xA2[\x00-\xFF]{3}\x02[\x00-\xFF]{3})')
        self.SetAutoImageRe = re.compile(b'(\x22[\x00-\xFF]{6})|(\xA2[\x00-\xFF]{7})')
        self.SetFreezeRe = re.compile(b'(\x21\x98[\x00-\xFF]{2}\x01[\x00-\xFF]{2})|(\xA1\x98[\x00-\xFF]{2}\x02[\x00-\xFF]{3})')
        self.SetInputRe = re.compile(b'(\x22\x03[\x00-\xFF]{2}\x01[\x00-\xFF]{2})|(\xA2\x03[\x00-\xFF]{2}\x02[\x00-\xFF]{3})')
        self.SetLampModeRe = re.compile(b'(\x23\xB1[\x00-\xFF]{2}\x02\x07[\x00-\xFF]{2})|(\xA3\xB1[\x00-\xFF]{2}\x02[\x00-\xFF]{3})')
        self.SetMenuNavigationRe = re.compile(b'(\x22\x0F[\x00-\xFF]{2}\x01[\x00-\xFF]{2})|(\xA2\x0F[\x00-\xFF]{2}\x02[\x00-\xFF]{3})')
        self.SetPowerRe = re.compile(b'(\x22[\x00-\xFF]{3}\x00[\x00-\xFF])|(\xA2[\x00-\xFF]{3}\x02[\x00-\xFF]{3})')
        self.SetVideoMuteRe = re.compile(b'(\x22[\x00-\xFF]{3}\x00[\x00-\xFF])|(\xA2[\x00-\xFF]{3}\x02[\x00-\xFF]{3})')
        self.SetVolumeRe = re.compile(b'(\x23\x10[\x00-\xFF]{2}\x02[\x00-\xFF]{3})|(\xA3\x10[\x00-\xFF]{2}\x02[\x00-\xFF]{3})')

        self.GetDeviceStatusRe = re.compile(b'(\x20\x88[\x00-\xFF]{16})|(\xA0\x88[\x00-\xFF]{6})')
        self.GetFilterUsageRe = re.compile(b'(\x23\x95[\x00-\xFF]{12})|(\xA3\x95[\x00-\xFF]{6})')
        self.GetLampModeRe = re.compile(b'(\x23\xB0[\x00-\xFF]{2}\x02\x07[\x00-\xFF]{2})|(\xA3\xB0[\x00-\xFF]{2}\x02[\x00-\xFF]{3})')
        self.GetLampUsageRe = re.compile(b'(\x23\x96[\x00-\xFF]{10})|(\xA3[\x00-\xFF]{7})')
        self.GetPowerRe = re.compile(b'(\x20\xBF[\x00-\xFF]{2}\x10[\x00-\xFF]{17})|(\xA0\xBF[\x00-\xFF]{2}\x02[\x00-\xFF]{3})')
        self.GetVolumeRe = re.compile(b'(\x23\x05[\x00-\xFF]{2}\x0D\x02[\x00-\xFF]{6}[\x00-\x1F]\x00[\x00-\xFF]{7})|(\xA3[\x00-\xFF]{7})')

        self.CommandDeliRex1 = {
            'AspectRatio': self.SetAspectRatioRe,
            'AudioMute': self.SetAudioMuteRe,
            'AutoImage': self.SetAutoImageRe,
            'Freeze': self.SetFreezeRe,
            'Input': self.SetInputRe,
            'LampMode': self.SetLampModeRe,
            'MenuNavigation': self.SetMenuNavigationRe,
            'Power': self.SetPowerRe,
            'VideoMute': self.SetVideoMuteRe,
            'Volume': self.SetVolumeRe
        }

        self.CommandDeliRex2 = {
            'DeviceStatus': self.GetDeviceStatusRe,
            'FilterUsage': self.GetFilterUsageRe,
            'LampMode': self.GetLampModeRe,
            'LampUsage': self.GetLampUsageRe,
            'Power': self.GetPowerRe,
            'Volume': self.GetVolumeRe
        }

    def SetAspectRatio(self, value, qualifier):

        AspectRatioCmdString = self.SetAspectRatioStates[value]
    ...
    def UpdateLampUsage(self, value, qualifier):
        ...
        LampValue = Lamp - 1
        if Lamp == 1 or Lamp == 2:
            Chksum = 0x9C + LampValue
            LampUsageCmdString = pack('>BBBBBBBB', 0x03, 0x96, 0x00, 0x00, 0x02, LampValue, 0x01, Chksum)
    ...
    def UpdateSignalStaus(self, value, qualifier):
        self.UpdatePower(value, qualifier)
    ...
    def UpdatePower(self, value, qualifier):
        ...
        if res:
            ...
            try:
                SignalValue = SignalStatusStates[res[7]]
                self.WriteStatus('SignalStatus', SignalValue, None)
            except (KeyError, IndexError):
                self.Error(['Signal Status: Invalid/Unexpected Response'])                

            try:
                InputValue = self.UpdateInputStates[res[8:10]]
    ...
    def SetVolume(self, value, qualifier):
        ...
        if 0 <= value <= 31:
            Chksum = 0x1D + value
            VolumeCmdString = pack('>11B', 0x03, 0x10, 0x00, 0x00, 0x05, 0x05, 0x00, 0x00, value, 0x00, Chksum)
    ...
    def nec_1_3127_A(self):

        self.SetAspectRatioStates = {
