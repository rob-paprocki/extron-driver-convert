...
class DeviceClass:
    def __init__(self):
        ...
        if self.Unidirectional == 'False':
            self.AddMatchString(re.compile(b'\xEA\xE8\x0F\x41([\x00-\x64])([\x00-\xFF]{,200})\xED'), self.__MatchActiveMicrophoneStatus, None)
    ...
    def __MatchActiveMicrophoneStatus(self, match, tag):
        ...
        numb = ord(match.group(1).decode())
        value = match.group(2)
        i = 0
        while i < numb*2:
            id = unpack('>H', value[i:i+2])[0]
            num = '{0}'.format(int(i/2+1))
            self.WriteStatus('ActiveMicrophoneStatus', id, {'Number' : num})
            i = i+2
        self.WriteStatus('ActiveMicrophones', numb, None)
