...
class DeviceClass:
    def __init__(self):
        ...
        self.Commands = {
            ...
            'VideoWallSize': {'Parameters':['Column','Row'], 'Status': {}},
            'Volume': { 'Status': {}}
        }

        if self.Unidirectional == 'False':
            self.AddMatchString(re.compile(b'\xAA\xFF[\x00-\xFF]\x03\x41\x13([\x01\x00])[\x00-\xFF]'), self.__MatchAudioMute, None)
            self.AddMatchString(re.compile(b'\xAA\xFF[\x00-\xE0]\x03\x41\x14([\x21\x22\x23\x24\x31\x32\x33\x34\x60])[\x00-\xFF]'), self.__MatchInput, None)
