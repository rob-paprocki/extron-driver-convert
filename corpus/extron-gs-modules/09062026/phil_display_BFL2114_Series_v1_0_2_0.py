...
class DeviceEthernetClass:
    ...
    def SetAudioMute(self, value, qualifier):
        ...
        if value in ValueStateValues:
            AudioMuteCmdString = pack('>4BH', 0x06, 0x01, 0x00, 0x47, ValueStateValues[value])
            self.__SetHelper('AudioMute', AudioMuteCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetAudioMute')
    ...
    def UpdateAudioMute(self, value, qualifier):
        ...
        AudioMuteCmdString = pack('5B', 0x05, 0x01, 0x00, 0x46, 0x42)
    ...
    def SetChannel(self, value, qualifier):
        ...
        if value in ValueStateValues:
            ChannelCmdString = pack('>4BH', 0x06, 0x01, 0x00, 0xC3, ValueStateValues[value])
            self.__SetHelper('Channel', ChannelCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetChannel')
    ...
    def SetChannelNumber(self, value, qualifier):
        ...
        if ValueConstraints['Min'] <= value <= ValueConstraints['Max']:
            checksum_val = 0xC4 ^ value
            ChannelNumberCmdString = pack('7B', 0x07, 0x01, 0x00, 0xC2, 0x00, value, checksum_val)
    ...
    def UpdateChannelNumber(self, value, qualifier):
        ...
        ChannelNumberCmdString = pack('5B', 0x05, 0x01, 0x00, 0xC1, 0xC5)
        res = self.__UpdateHelper('ChannelNumber', ChannelNumberCmdString, value, qualifier)
        if res:
            try:
                value = res[5]
    ...
    def SetInput(self, value, qualifier):
        ...
        if value in self.SetSource:
            InputCmdString = pack('9B', 0x09, 0x01, 0x00, 0xAC, self.SetSource[value][0], 0x09, 
                                    0x01, 0x00, self.SetSource[value][1])
            self.__SetHelper('Input', InputCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetInput')
    ...
    def UpdateInput(self, value, qualifier):
        ...
        InputCmdString = pack('5B', 0x05, 0x01, 0x00, 0xAD, 0xA9)
        res = self.__UpdateHelper('Input', InputCmdString, value, qualifier)
        if res:
            try:
                value = self.GetSource[res[4]]
                self.WriteStatus('Input', value, qualifier)
    ...
    def SetPower(self, value, qualifier):
        ...
        if value in ValueStateValues:
            PowerCmdString = pack('>4BH', 0x06, 0x01, 0x00, 0x18, ValueStateValues[value])
            self.__SetHelper('Power', PowerCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetPower')
    ...
    def UpdatePower(self, value, qualifier):
        ...
        PowerCmdString = pack('5B', 0x05, 0x01, 0x00, 0x19, 0x1D)
        res = self.__UpdateHelper('Power', PowerCmdString, value, qualifier)
        if res:
            try:
                value = ValueStateValues[res[4]]
    ...
    def SetVolume(self, value, qualifier):
        ...
        if value in ValueStateValues:
            VolumeCmdString = pack('7B', 0x07, 0x01, 0x00, 0x41, ValueStateValues[value][0],
                                    0x02, ValueStateValues[value][1])
            self.__SetHelper('Volume', VolumeCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetVolume')
    ...
    def __UpdateHelper(self, command, commandstring, value, qualifier):
        ...
        if self.Unidirectional == 'True':
            ...
            res = self.SendAndWait(commandstring, self.DefaultResponseTimeout, deliLen=self.deliLen_values[command])
            if not res:
                return ''
            else:
                return self.__CheckResponseForErrors(command, res)
