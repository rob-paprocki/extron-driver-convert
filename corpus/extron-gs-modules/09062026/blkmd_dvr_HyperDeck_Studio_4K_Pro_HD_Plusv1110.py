...
class DeviceSerialClass:
    ...
    def UpdateDeviceStatus(self, value, qualifier):
        ...
        DeviceStatusCmdString = b'\x70\x20\x90'
        ...
        if res:
            try:
                ...
                value = ValueStateValues[bytes(res[1:2])]
...
class DeviceEthernetClass:
    def __init__(self):
        ...
        if self.Unidirectional == 'False':
            self.AddMatchString(re.compile(b'status: (preview|stopped|play|forward|rewind|jog|shuttle|record)\r\n[\s\S]+?\r\n\r\n'), self.__MatchTransportInformation, None)
            self.AddMatchString(re.compile(b'video input: (SDI|HDMI)\r\n'), self.__MatchVideoInput, None)
            self.AddMatchString(re.compile(
                b'([0-9]{3}) (syntax error|unsupported parameter|invalid value|unsupported|disk full|no disk|disk error'
                b'|timeline empty|internal error|out of range|no input|remote control disabled|clip not found'
                b'|connection rejected|invalid state|invalid codec|invalid format|invalid token|format not prepared)'
                b'|parameterized single line command not supported.*?\r\n', re.DOTALL), self.__MatchError, None)
    ...
    def __MatchTransportInformation(self, match, tag):
        ...
        self.WriteStatus('TransportInformation', value, None)

    def SetVideoInput(self, value, qualifier):

        ValueStateValues = ('SDI', 'HDMI')

        if value in ValueStateValues:
            VideoInputCmdString = 'configuration: video input: {}\r\n'.format(value)
            self.__SetHelper('VideoInput', VideoInputCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetVideoInput')
