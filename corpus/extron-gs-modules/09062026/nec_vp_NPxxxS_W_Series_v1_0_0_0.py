...
class DeviceClass:
    ...
    def __init__(self):
        ...
        self._SetAspectRatio = re.compile(b'(\x23\x10[\x00-\xFF]{6})|(\xA3\x10[\x00-\xFF]{6})')
        self._SetAudioMute = re.compile(b'(\x22[\x00-\xFF]{3}\x00[\x00-\xFF])|(\xA2[\x00-\xFF]{3}\x02[\x00-\xFF]{3})')
        self._SetAutoImage = re.compile(b'(\x22[\x00-\xFF]{6})|(\xA2[\x00-\xFF]{7})')
        self._SetFreeze = re.compile(b'(\x21\x98[\x00-\xFF]{2}\x01[\x00-\xFF]{2})|(\xA1\x98[\x00-\xFF]{2}\x02[\x00-\xFF]{3})')
        self._SetInput = re.compile(b'(\x22\x03[\x00-\xFF]{2}\x01[\x00-\xFF]{2})|(\xA2\x03[\x00-\xFF]{2}\x02[\x00-\xFF]{3})')
        self._SetLampMode = re.compile(b'(\x23\xB1[\x00-\xFF]{2}\x02\x07[\x00-\xFF]{2})|(\xA3\xB1[\x00-\xFF]{2}\x02[\x00-\xFF]{3})')
        self._SetPower = re.compile(b'(\x22[\x00-\xFF]{3}\x00[\x00-\xFF])|(\xA2[\x00-\xFF]{3}\x02[\x00-\xFF]{3})')
        self._SetVideoMute = re.compile(b'(\x22[\x00-\xFF]{3}\x00[\x00-\xFF])|(\xA2[\x00-\xFF]{3}\x02[\x00-\xFF]{3})')
        self._SetVolume = re.compile(b'(\x23\x10[\x00-\xFF]{2}\x02[\x00-\xFF]{3})|(\xA3\x10[\x00-\xFF]{2}\x02[\x00-\xFF]{3})')

        self.GetDeviceStatus = re.compile(b'(\x20\x88[\x00-\xFF]{16})|(\xA0\x88[\x00-\xFF]{6})')
        self.GetLampMode = re.compile(b'(\x23\xB0[\x00-\xFF]{2}\x02\x07[\x00-\xFF]{2})|(\xA3\xB0[\x00-\xFF]{2}\x02[\x00-\xFF]{3})')
        self.GetLampUsage = re.compile(b'(\x23\x96[\x00-\xFF]{10})|(\xA3[\x00-\xFF]{7})')
        self.GetPower = re.compile(b'(\x20\xBF[\x00-\xFF]{2}\x10[\x00-\xFF]{17})|(\xA0\xBF[\x00-\xFF]{2}\x02[\x00-\xFF]{3})')
        self.GetVolume = re.compile(b'(\x23\x05[\x00-\xFF]{2}\x0D\x02[\x00-\xFF]{6}[\x00-\x1F]\x00[\x00-\xFF]{7})|(\xA3[\x00-\xFF]{7})')

        self.CommandDeliRex1 = {
            'AspectRatio': self._SetAspectRatio,
            'AudioMute': self._SetAudioMute,
            'AutoImage': self._SetAutoImage,
            'Freeze': self._SetFreeze,
            'Input': self._SetInput,
            'LampMode': self._SetLampMode,
            'Power': self._SetPower,
            'VideoMute': self._SetVideoMute,
            'Volume': self._SetVolume
            }

        self.CommandDeliRex2 = {
            'DeviceStatus': self.GetDeviceStatus,
            'LampMode': self.GetLampMode,
            'LampUsage': self.GetLampUsage,
            'Power': self.GetPower,
    ...
    def SetAspectRatio(self, value, qualifier):
        ...
        ValueStateValues = {
            'Normal': b'\x03\x10\x00\x00\x05\x18\x00\x00\x00\x00\x30',
            'Wide Zoom': b'\x03\x10\x00\x00\x05\x18\x00\x00\x00\x00\x31',
            '16:9': b'\x03\x10\x00\x00\x05\x18\x00\x00\x00\x00\x32',
            '4:3': b'\x03\x10\x00\x00\x05\x18\x00\x00\x00\x00\x34',
            '15:9': b'\x03\x10\x00\x00\x05\x18\x00\x00\x00\x00\x35',
            '16:10': b'\x03\x10\x00\x00\x05\x18\x00\x00\x00\x00\x36'
        }
    ...
    def SetAutoImage(self, value, qualifier):
        ...
        AutoImageCmdString = b'\x03\xBA\x00\x00\x01\x00\xBE'
    ...
    def SetInput(self, value, qualifier):
        ...
        ValueStateValues = {
            ...
            'DVI (Analog)': b'\x02\x03\x00\x00\x02\x01\x02\x0A',
    ...
    def __UpdateHelper(self, command, commandstring, value, qualifier):
        ...
        if self.Unidirectional == 'True':
            self.Discard('Inappropriate Command {0}, Unidirectional is True'.format(command))
            return ''
        else:
