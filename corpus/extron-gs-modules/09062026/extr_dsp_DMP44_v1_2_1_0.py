...
class DeviceClass:
    def __init__(self):
        ...
        if self.Unidirectional == 'False':
            ...
            self.AddMatchString(compile(rb'GrpmD([0-9]{2})\*([0-9+-]{1,5})\r\n'), self.__MatchGroup, None)
            self.AddMatchString(compile(rb'DsG(3000[0-3])\*([0-9+-]{1,5})\r\n'), self.__MatchInputGain, None)
            self.AddMatchString(compile(rb'DsM(3000[0-3])\*(0|1)\r\n'), self.__MatchInputMute, None)
            self.AddMatchString(compile(rb'DsG2([0-3]{2})([0-3]{2})\*([0-9+-]{1,5})\r\n'), self.__MatchMixpointGain, None)
            self.AddMatchString(compile(rb'DsM2([0-3]{2})([0-3]{2})\*(0|1)\r\n'), self.__MatchMixpointMute, None)
            self.AddMatchString(compile(rb'DsM(6000[0-3])\*(0|1)\r\n'), self.__MatchOutputMute, None)
            self.AddMatchString(compile(rb'DsG(6000[0-3])\*([0-9+-]{1,5})\r\n'), self.__MatchOutputAttenuation, None)
            self.AddMatchString(compile(b'(Pno60-1095-01)\r\n'), self.__MatchPartNumber, None)
            self.AddMatchString(compile(rb'DsG(6010[0-3])\*([0-9 -]{1,5})\r\n'), self.__MatchOutputPostmixerTrim, None)
            self.AddMatchString(compile(rb'DsG(3010[0-3])\*([0-9+-]{1,5})\r\n'), self.__MatchPremixerGain, None)
            self.AddMatchString(compile(rb'DsM(3010[0-3])\*(0|1)\r\n'), self.__MatchPremixerMute, None)
    ...
    def SetGroupPostmixerTrim(self, value, qualifier):
        ...
        group = qualifier['Group']
        if 1 <= int(group) <= 16 and -12 <= value <= 6:
            level = round(value * 10)
            commandString = 'wd{0}*{1:+06d}grpm\r\n'.format(group, level)
    ...
    def SetGroupPremixerGain(self, value, qualifier):
        ...
        group = qualifier['Group']
        if 1 <= int(group) <= 16 and -100 <= value <= 6:
            level = round(value * 10)
            commandString = 'wd{0}*{1:+06d}grpm\r\n'.format(group, level)
    ...
    def __MatchGroup(self, match, tag):     
        ...
        if group in self.GroupFunction:
            ...
            elif command in ['GroupInputGain', 'GroupPremixerGain',
    ...
    def __MatchInputGain(self, match, tag):
        ...
        channel = str(int(match.group(1)) - 29999)
    ...
    def SetInputMute(self, value, qualifier):
        ...
        channel = int(qualifier['Input'])
        if 1 <= channel <= 4:
            commandString = 'wM{0}*{1}AU\r\n'.format(channel + 29999, ValueStateValues[value])
    ...
    def SetPremixerGain(self, value, qualifier):
        ...
        channel = int(qualifier['Input'])
        if 1 <= channel <= 4 and -100 <= value <= 6:
            level = round(value * 10)
            commandString = 'wG{0}*{1:05d}AU\r\n'.format(channel + 30099, level)
    ...
    def __MatchPremixerGain(self, match, tag):
        ...
        channel = str(int(match.group(1)) - 30099)
        qualifier = {'Input': channel}
        value = (int(match.group(2))) / 10
    ...
    def SetPremixerMute(self, value, qualifier):
        ...
        channel = int(qualifier['Input'])
        if 1 <= channel <= 4:
            commandString = 'wM{0}*{1}AU\r\n'.format(channel + 30099, ValueStateValues[value])
