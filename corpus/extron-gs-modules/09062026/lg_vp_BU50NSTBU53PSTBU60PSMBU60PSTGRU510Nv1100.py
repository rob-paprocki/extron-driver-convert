...
class DeviceSerialClass:
    def __init__(self):
        ...
        if self.Unidirectional == 'False' and self._DeviceID != '00':
            self.AddMatchString(re.compile(b'c [a-f0-9]{2} OK(0[126])x', re.I), self.__MatchAspectRatio, None)
            ...
            self.AddMatchString(re.compile(b'b [a-f0-9]{2} OK(9[0-2])x', re.I), self.__MatchInput, None)
        ...
        self.regexSet = re.compile(b'[cembladf] [a-f0-9]{2} (?:OK|NG).*?x', re.I)
    ...
    def SetEcoMode(self, value, qualifier):
        ...
        if value in ValueStateValues:
            EcoModeCmdString = 'jq {0} {1}\r'.format(self._DeviceID, ValueStateValues[value])
    ...
    def __MatchError(self, match, tag):
        ...
        self.Error(['An error occurred: {}.'.format(DEVICE_ERROR_CODES[match.group(1).decode().lower()])])
...
class DeviceEthernetClass:
    ...
    def KeyCode(self, value):
        if value:
            self.__aes = AESCipher(value)
        else:
            self.Error(['Missing Key Code parameter.'])
