...
class DeviceSerialClass:
    ...
    def __init__(self):
        ...
        self.Commands = {
            ...
            '3DMode': {'Parameters': ['Mode', 'Pattern', 'Direction', '3D Depth'], 'Status': {}},
            'AspectRatio': {'Status': {}},
            'AudioMute': {'Status': {}},
            'AutoImage': {'Status': {}},
            'ChannelStep': {'Status': {}},
        ...
        if self.Unidirectional == 'False':
            ...
            self.AddMatchString(re.compile(b'b [0-9a-f]{2} OK(00|01|02|10|11|20|21|40|41|60|90|91|92|93)x', re.I), self.__MatchInput, None)
            ...
            self.AddMatchString(re.compile(b'd [0-9a-f]{2} OK0(1|0)x', re.I), self.__MatchScreenMute, None)
            self.AddMatchString(re.compile(b'f [0-9a-f]{2} OK([0-9a-fA-F]{2})x', re.I), self.__MatchVolume, None)
            self.AddMatchString(re.compile(b'(u|c|e|m|b|l|a|d|f|t) [0-9a-f]{2} NG(.*)x', re.I), self.__MatchError, None)
    ...
    def Set3DMode(self, value, qualifier):
        ...
        depth = '{0:02X}'.format(qualifier['3D Depth'])

        if 0 <= qualifier['3D Depth'] <= 20:
            ModeCmdString = 'xt {0} {1} {2} {3} {4}\r'.format(self._DeviceID, mode, pattern, direction, depth)
    ...
    def SetScreenMute(self, value, qualifier):
        ...
        ScreenMuteCmdString = 'kd {0} {1}\r'.format(self._DeviceID, ValueStateValues[value])
        self.__SetHelper('ScreenMute', ScreenMuteCmdString, value, qualifier)

    def UpdateScreenMute(self, value, qualifier):

        ScreenMuteCmdString = 'kd {0} FF\r'.format(self._DeviceID)
        self.__UpdateHelper('ScreenMute', ScreenMuteCmdString, value, qualifier)
    ...
    def __MatchScreenMute(self, match, tag):
        ...
        value = ValueStateValues[match.group(1).decode()]
        self.WriteStatus('ScreenMute', value, None)
...
class DeviceEthernetClass:
    ...
    def Set3DMode(self, value, qualifier):
        ...
        pattern = {
            ...
            'Frame Sequential': 'framesequential',
            'Column Interleaving': 'columninterleaving',
            'Row Interleaving': 'rowinterleaving'
        }[qualifier['Pattern']]
        ...
        if 0 <= depth <= 20:
            if mode == 'off' or mode == '3dto2d':
                ModeCmdString = 'PICTURE_3D {0}\r'.format(mode)
            elif mode == '2dto3d':
                ModeCmdString = 'PICTURE_3D {0} {1} {2}\r'.format(mode, direction, depth)
            elif mode == 'on':
                ModeCmdString = 'PICTURE_3D {0} {1} {2} {3}\r'.format(mode, pattern, direction, depth)
            self.__SetHelper('3DMode', ModeCmdString, value, qualifier)
        else:
            print('Invalid Command for Set3DMode')
    ...
    def SetAutoImage(self, value, qualifier):
        ...
        AutoImageCmdString = 'KEY_ACTION autoconfig\r'
        self.__SetHelper('AutoImage', AutoImageCmdString, value, qualifier)
    ...
    def SetAspectRatio(self, value, qualifier):
        ...
        AspectRatioCmdString = 'ASPECT_RATIO {0}\r'.format(ValueStateValues[value])
        self.__SetHelper('AspectRatio', AspectRatioCmdString, value, qualifier)
    ...
    def SetAudioMute(self, value, qualifier):
        ...
        MuteCmdString = 'VOLUME_MUTE {0}\r'.format(ValueStateValues[value])
        self.__SetHelper('AudioMute', MuteCmdString, value, qualifier)
    ...
    def SetChannelStep(self, value, qualifier):
        ...
        ChannelStepCmdString = 'KEY_ACTION channel{0}\r'.format(ValueStateValues[value])
        self.__SetHelper('ChannelStep', ChannelStepCmdString, value, qualifier)

    def SetClosedCaption(self, value, qualifier):

        ClosedCaptionCmdString = 'KEY_ACTION captionsubtitle\r'
    ...
    def SetExecutiveMode(self, value, qualifier):
        ...
        ExecutiveModeCmdString = 'REMOTECONTROLER_LOCK {0}\r'.format(ValueStateValues[value])
        self.__SetHelper('ExecutiveMode', ExecutiveModeCmdString, value, qualifier)
    ...
    def SetInput(self, value, qualifier):
        ...
        InputCmdString = 'INPUT_SELECT {0}\r'.format(ValueStateValues[value])
        self.__SetHelper('Input', InputCmdString, value, qualifier)

    def SetKeypad(self, value, qualifier):

        if 0 <= int(value) <= 9:
            KeypadCmdString = 'KEY_ACTION number{0}\r'.format(value)
            self.__SetHelper('Keypad', KeypadCmdString, value, qualifier)
        else:
    ...
    def SetMenuNavigation(self, value, qualifier):
        ...
        MenuNavigationCmdString = 'KEY_ACTION {0}\r'.format(ValueStateValues[value])
        self.__SetHelper('MenuNavigation', MenuNavigationCmdString, value, qualifier)
    ...
    def SetOnScreenDisplay(self, value, qualifier):
        ...
        OnScreenDisplayCmdString = 'OSD_SELECT {0}\r'.format(ValueStateValues[value])
        self.__SetHelper('OnScreenDisplay', OnScreenDisplayCmdString, value, qualifier)

    def SetPowerOff(self, value, qualifier):

        PowerOffCmdString = 'POWER off\r'
        self.__SetHelper('PowerOff', PowerOffCmdString, value, qualifier)

    def SetScreenMute(self, value, qualifier):

        ValueStateValues = {
            'Screen Mute On': 'screenmuteon',
            'Video Mute On': 'videomuteon',
            'All Mute Off': 'allmuteoff'
        }

        ScreenMuteCmdString = 'SCREEN_MUTE {0}\r'.format(ValueStateValues[value])
        self.__SetHelper('ScreenMute', ScreenMuteCmdString, value, qualifier)
    ...
    def SetVolume(self, value, qualifier):
        ...
        if ValueConstraints['Min'] <= value <= ValueConstraints['Max']:
            VolumeCmdString = 'VOLUME_CONTROL {0}\r'.format(value)
            self.__SetHelper('Volume', VolumeCmdString, value, qualifier)
        else:
            print('Invalid Command for SetVolume')
