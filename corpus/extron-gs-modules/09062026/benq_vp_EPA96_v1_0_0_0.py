...
class DeviceClass:
    def __init__(self):
        ...
        if self.Unidirectional == 'False':
            self.AddMatchString(re.compile(b'\*ASP=(4:3|16:9|16:10|AUTO|REAL)#', re.I), self.__MatchAspectRatio, None)
            self.AddMatchString(re.compile(b'\*MUTE=(ON|OFF)#', re.I), self.__MatchAudioMute, None)
            self.AddMatchString(re.compile(b'\*SOUR=(RGB|HDMI|HDMI2|HDMI3|VID|SVID|HDBASET)#', re.I), self.__MatchInput, None)
            self.AddMatchString(re.compile(b'\*LAMPM=(LNOR|ECO|DIMMING|CUSTOM)#', re.I), self.__MatchLampMode, None)
            self.AddMatchString(re.compile(b'\*LTIM=(\d+)#', re.I), self.__MatchLampUsage, None)
            self.AddMatchString(re.compile(b'\*APPMOD=(PRESET|SRGB|BRIGHT|DICOM|VIVID|USER1|USER2)#', re.I), self.__MatchPictureMode, None)
            ...
            self.AddMatchString(re.compile(b'\*VOL=([0-9]{1,3})#', re.I), self.__MatchVolume, None)
    ...
    def SetVolume(self, value, qualifier):
        ...
        if 0 <= value <= 100:
            VolumeCmdString = '\r*vol={}#\r'.format(value)
