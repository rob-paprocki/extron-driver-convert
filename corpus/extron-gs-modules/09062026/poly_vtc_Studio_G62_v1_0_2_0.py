...
class DeviceClass:
    def __init__(self):
        ...
        if self.Unidirectional == 'False':
            ...
            self.AddMatchString(re.compile(b'provider:(Poly|Zoom|Teams|LogMeIn|RingCentral|camuvc|StarLeaf|DialPad|Tencent|Google) lockState:(locked|unlocked)\r'), self.__MatchProviderMode, None)
            ...
            self.AddMatchString(re.compile(b'vcbutton source get (2|none)\r'), self.__MatchVideoContentSource, None)
        ...
        self.UsernameTimer = None
        self.PasswordTimer = None
        ...
        self.CallHistoryScroller = Scroller([], self._NumberofCallHistoryResults, end='*** End of List ***')
        ...
        self.ContentScroller = Scroller([], self._NumberofContentResults, end='*** End of List ***')
        ...
        # Dial String variable for: SetCallHistoryResultSet, SetHook, SetMeetingNumberSet
        self.DialString = ''

        # Phonebook Group Search variable for: SetPhonebookGroupSearch, SetPhonebookGroupResultSet
        self.PhonebookGroupSearch = ''

        # Phonebook Search variable for: SetPhonebookSearch, SetPhonebookUpdate
    ...
    def MeetingsTimeFormat(self, value):
        ...
        if value in meeting_time_formats:
            self._MeetingsTimeFormat = meeting_time_formats[value]
        else:
    ...
    def MeetingsTimeInterval(self, value):
        ...
        if value in time_intervals:
            self._MeetingsTimeInterval = time_intervals[value]
        else:
    ...
    def NumberofContentResults(self, value):
        if 1 <= int(value) <= 10:
            ...
            self.Error(['Missing Number of Content Results Parameter.'])
    ...
    def NumberofMeetingResults(self, value):
        if 1 <= int(value) <= 10:
            self._NumberofMeetingResults = value
            self.MeetingScroller = Scroller([], self._NumberofMeetingResults, end='*** End of List ***')
        else:
            self.Error(['Missing Number of Meeting Results Parameter.'])
    ...
    def NumberofCallHistoryResults(self, value):
        if 1 <= int(value) <= 10:
            self._NumberofCallHistoryResults = value
            self.CallHistoryScroller = Scroller([], self._NumberofCallHistoryResults, end='*** End of List ***')
        else:
            self.Error(['Missing Number of Call History Results Parameter.'])
    ...
    def NumberofPhonebookResults(self, value):
        if 1 <= int(value) <= 15:
            ...
            self.Error(['Missing Number of Phonebook Results Parameter.'])
    ...
    def __WriteMeeting(self):
        for meeting, info in enumerate(self.MeetingScroller, 1):
            if isinstance(info, dict):
                self.WriteStatus('MeetingStart', info['start'].strftime(self._MeetingsTimeFormat), {'Meeting': meeting})
                self.WriteStatus('MeetingEnd', info['end'].strftime(self._MeetingsTimeFormat), {'Meeting': meeting})
                self.WriteStatus('MeetingSubject', info['subject'], {'Meeting': meeting})
                self.WriteStatus('MeetingTimeCombined', '{}-{}'.format(info['start'].strftime('%a {}'.format(self._MeetingsTimeFormat)), info['end'].strftime(self._MeetingsTimeFormat)), {'Meeting': meeting})
    ...
    def SetCallHistoryResultSet(self, value, qualifier):
        ...
        if 1 <= call <= self.CallHistoryScroller.window and self.CallHistoryScroller.offset + call <= self.CallHistoryScroller.current_size:
            dial_string = self.CallHistoryScroller[call - 1]
    ...
    def SetContentUpdate(self, value, qualifier):
        ...
        if value in ValueStateValues:
            ...
            if res:
                res = res.decode()
                self.ContentScroller.overwrite(self.ContentRegex.findall(res))
    ...
    def UpdateMeetingListQuery(self, value, qualifier):
        ...
        if self._MeetingsTimeInterval == 0:
            ...
            MeetingListCmdString = 'calendarmeetings list today:{} {}\r'.format(current_time.strftime('%H:%M'), (current_time + timedelta(self._MeetingsTimeInterval)).strftime('%Y-%m-%d:%H:%M'))

        res = self.SendAndWait(MeetingListCmdString, 5, deliRex=self.MeetingListRegex)
        if res:
            res = res.decode()
            for meeting in self.MeetingRegex.finditer(res):
    ...
    def UpdateMeetingOrganizer(self):
        for meeting in self.MeetingScroller.all_items:
            ...
            if res:
                res = res.decode()
                meetingid = self.MeetingIDRegex.search(res)
                if meetingid and meeting['meetingid'] == meetingid.group(1):
                    organizer = self.MeetingOrganizerRegex.search(res)
                    if organizer:
                        meeting['organizer'] = organizer.group(1)
                    else:
                        meeting['organizer'] = ''

                    location = re.search(self.MeetingLocationRegex, res)
                    if location:
                    ...
                    numbers = {}
                    for number in re.findall(self.MeetingNumberRegex, res):
    ...
    def __PhonebookCallback(self, size, chunk):
        ...
        if res:
            res = res.decode()
            results = self.PhonebookRegex.findall(res)
    ...
    def SetVideoContentSource(self, value, qualifier):
        ...
        if value in ['Play', 'Stop']:
            VideoContentSourceCmdString = 'vcbutton {}\r'.format(value.lower())
    ...
    def __UpdateHelper(self, command, commandstring, value, qualifier):
        ...
        if self.Unidirectional == 'True':
            ...
            sleepmode = self.ReadStatus('SleepMode', None)
            if sleepmode in ['Sleep', 'Unknown', None] and command in ['CameraNearTracking', 'CameraSource', 'VideoContentSource']:
                self.Discard('Inappropriate Command ' + command)
            else:
                self.Send(commandstring)
