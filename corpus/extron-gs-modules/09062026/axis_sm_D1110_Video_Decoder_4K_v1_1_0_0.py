...
class DeviceClass:
    ...
    def SetPlaySequence(self, value, qualifier):

        if 1 <= value:
            PlaySequenceCmdString = 'axis-cgi/decoder/player.cgi'
            data = {
                "apiVersion": "2.0",
                "method": "playSequence",
                "params": {
            ...
            self.__SetHelper('PlaySequence', value, qualifier, url=PlaySequenceCmdString, data=json.dumps(data).encode())
        else:
            self.Discard('Invalid Command for SetPlaySequence')

    def UpdateVersion(self, value, qualifier):

        VersionCmdString = 'axis-cgi/basicdeviceinfo.cgi'
        data = {
            "apiVersion" : "1.0",
            "method" : "getAllProperties"
        }
        res = self.__UpdateHelper('Version', value, qualifier, url=VersionCmdString, data=json.dumps(data).encode())
        if res:
            try:
                value = res['data']['propertyList']['Version']
