...
class DeviceClass:
    def __init__(self):
        ...
        if self.Unidirectional == 'False':
            self.AddMatchString(re.compile(b'\x20\x88[\x00-\xFF]{2}\x0C[\x00-\xFF]{13}'), self.__MatchDeviceStatus, None)
            self.AddMatchString(re.compile(b'\x23\xB0[\x00-\xFF]{2}\x02\x07[\x00-\xFF]{2}'), self.__MatchLampMode, None)
            self.AddMatchString(re.compile(b'\x23\x96[\x00-\xFF]{2}\x06[\x00-\xFF]{7}'), self.__MatchLampUsage, None)
            self.AddMatchString(re.compile(b'\x20\xBF[\x00-\xFF]{2}\x10\x02[\x00-\xFF]{16}'), self.__MatchPower, None)
            self.AddMatchString(re.compile(b'\x23\x05[\x00-\xFF]{2}\x10[\x00-\xFF]{7}[\x00-\x1F]\x00[\x00-\xFF]{8}'), self.__MatchVolume, None)
            self.AddMatchString(re.compile(b'[\xA0-\xA3][\x00-\xFF]{7}'), self.__MatchError, None)
    ...
    def UpdateDeviceStatus(self, value, qualifier):
        ...
        DeviceStatusCmdString = b'\x00\x88\x00\x00\x88'
    ...
    def __MatchDeviceStatus(self, match, tag):
        ...
        ValueStateValues = {
            ...
            b'\x40\x00\x00\x00': 'Light off or LD Driver communication error',
            b'\x00\x02\x00\x00': 'Phosphor wheel error',
            b'\x00\x00\x01\x00': 'Driver version error',
            b'\x00\x00\x02\x00': 'CPU communication error',
            b'\x00\x00\x04\x00': 'Temperature error (temperature sensor)',
            b'\x00\x00\x20\x00': 'TEC Driver error',
            b'\x00\x00\x00\x04': 'Color sensor error',
    ...
    def SetLampMode(self, value, qualifier):
        ...
        ValueStateValues = {
            ...
            'Silent':       b'\x03\xB1\x00\x00\x02\x07\x06\xC3'
            }
    ...
    def __MatchError(self, match, tag):
        ...
        value = ErrorStates[match.group()[5:7]]
        self.Error([value])
