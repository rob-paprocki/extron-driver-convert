...
class DeviceClass:
    ...
    def __init__(self):
        ...
        if self.Unidirectional == 'False':
            self.SetAspectRatioRex = compile(b'(\x23\x10[\x00-\xFF]{6})|(\xA3\x10[\x00-\xFF]{6})')
            self.SetAudioMuteRex = compile(b'(\x22[\x00-\xFF]{3}\x00[\x00-\xFF])|(\xA2[\x00-\xFF]{3}\x02[\x00-\xFF]{3})')
            self.SetAutoImageRex = compile(b'(\x22[\x00-\xFF]{6})|(\xA2[\x00-\xFF]{7})')
            self.SetFreezeRex = compile(b'(\x21\x98[\x00-\xFF]{2}\x01[\x00-\xFF]{2})|(\xA1\x98[\x00-\xFF]{2}\x02[\x00-\xFF]{3})')
            self.SetInputRex = compile(b'(\x22\x03[\x00-\xFF]{2}\x01[\x00-\xFF]{2})|(\xA2\x03[\x00-\xFF]{2}\x02[\x00-\xFF]{3})')
            self.SetLampModeRex = compile(b'(\x23\xB1[\x00-\xFF]{2}\x02\x07[\x00-\xFF]{2})|(\xA3\xB1[\x00-\xFF]{2}\x02[\x00-\xFF]{3})')
            self.SetMenuNavigationRex = compile(b'(\x22\x0F[\x00-\xFF]{2}\x01[\x00-\xFF]{2})|(\xA2\x0F[\x00-\xFF]{2}\x02[\x00-\xFF]{3})')
            self.SetPowerRex = compile(b'(\x22[\x00-\xFF]{3}\x00[\x00-\xFF])|(\xA2[\x00-\xFF]{3}\x02[\x00-\xFF]{3})')
            self.SetVideoMuteRex = compile(b'(\x22[\x00-\xFF]{3}\x00[\x00-\xFF])|(\xA2[\x00-\xFF]{3}\x02[\x00-\xFF]{3})')
            self.SetVolumeRex = compile(b'(\x23\x10[\x00-\xFF]{2}\x02[\x00-\xFF]{3})|(\xA3\x10[\x00-\xFF]{2}\x02[\x00-\xFF]{3})')
            self.GetDeviceStatusRex = compile(b'(\x20\x88[\x00-\xFF]{16})|(\xA0\x88[\x00-\xFF]{6})')
            self.GetLampModeRex = compile(b'(\x23\xB0[\x00-\xFF]{2}\x02\x07[\x00-\xFF]{2})|(\xA3\xB0[\x00-\xFF]{2}\x02[\x00-\xFF]{3})')
            self.GetLampUsageRex = compile(b'(\x23\x96[\x00-\xFF]{10})|(\xA3[\x00-\xFF]{7})')
            self.GetPowerRex = compile(b'(\x20\xBF[\x00-\xFF]{2}\x10[\x00-\xFF]{17})|(\xA0\xBF[\x00-\xFF]{2}\x02[\x00-\xFF]{3})')
            self.GetVolumeRex = compile(b'(\x23\x05[\x00-\xFF]{2}\x0D\x02[\x00-\xFF]{6}[\x00-\x1F]\x00[\x00-\xFF]{7})|(\xA3[\x00-\xFF]{7})')

        self.CommandDeliRex1 = {
            'AspectRatio': self.SetAspectRatioRex,
            'AudioMute': self.SetAudioMuteRex,
            'AutoImage': self.SetAutoImageRex,
            'Freeze': self.SetFreezeRex,
            'Input': self.SetInputRex,
            'LampMode': self.SetLampModeRex,
            'MenuNavigation': self.SetMenuNavigationRex,
            'Power': self.SetPowerRex,
            'VideoMute': self.SetVideoMuteRex,
            'Volume': self.SetVolumeRex
            }
            
        self.CommandDeliRex2 = {
            'DeviceStatus': self.GetDeviceStatusRex,
            'LampMode': self.GetLampModeRex,
            'LampUsage': self.GetLampUsageRex,
            'Power': self.GetPowerRex,
            'Volume': self.GetVolumeRex
            }
    ...
    def UpdateDeviceStatus(self, value, qualifier):
        ...
        cmdString = b'\x00\x88\x00\x00\x00\x88'
        ...
        if res:
            try:
                if res[13:14] != b'\x00':
                    ...
                    value = DeviceStatusStates.get(res[5:9], 'Multiple Errors')
    ...
    def SetFreeze(self, value, qualifier):
        ...
        ValueStateValues = {
            'On': b'\x01\x98\00\x00\x01\x01\x9B',
            'Off': b'\x01\x98\00\x00\x01\x02\x9C'
        }
    ...
    def SetInput(self, value, qualifier):
        ...
        ValueStateValues = {
            ...
            'Computer 3': b'\x02\x03\x00\x00\x02\x01\x1A\x22',
    ...
    def UpdateLampMode(self, value, qualifier):
        ...
        cmdString = b'\x03\xB0\x00\x00\x01\x07\xBB'
    ...
    def UpdateLampUsage(self, value, qualifier):
        ...
        cmdString = b'\x03\x96\x00\x00\x02\x00\x01\x9C'
    ...
    def UpdatePower(self, value, qualifier):
        ...
        cmdString =  b'\x00\xBF\x00\x00\x01\x02\xC2'
    ...
    def UpdateVolume(self, value, qualifier):
        ...
        cmdString = b'\x03\x05\x00\x00\x03\x05\x00\x00\x10'
