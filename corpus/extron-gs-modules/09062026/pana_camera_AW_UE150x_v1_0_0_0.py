...
class DeviceHTTPClass:
    ...
    def SetAutoTracking(self, value, qualifier):
        ...
        if value in ValueStateValues:
            url = 'cgi-bin/aw_cam?'
            AutoTrackingCmdString = 'cmd=OSL:B6:{}&res=1'.format(ValueStateValues[value])
            self.__SetHelper('AutoTracking', value, qualifier, url=url, data=AutoTrackingCmdString)
        else:
            self.Discard('Invalid Command for SetAutoTracking')
    ...
    def UpdateAutoTracking(self, value, qualifier):
        ...
        url = 'cgi-bin/aw_cam?'
        AutoTrackingCmdString = 'cmd=QSL:B6&res=1'
        res = self.__UpdateHelper('AutoTracking', value, qualifier, url=url, data=AutoTrackingCmdString)
        if res:
            try:
                ValueStateValues = {
                    '1': 'On',
    ...
    def __SetHelper(self, command, value, qualifier, url='', data=None):
        ...
        try:
            if self.DefaultPort == 443:
                res = urllib.request.urlopen(my_request, context=self.Context, timeout=1)
            else:
