...
class DeviceClass:
    ...
    def SetFocus(self, value, qualifier):
        ...
        if value in ValueStateValues and 0 <= int(qualifier['Focus Speed']) <= 15:
            if value == 'Stop':
                speed = 0
            else:
                speed = ValueStateValues[value] + int(qualifier['Focus Speed'])
    ...
    def SetPanTilt(self, value, qualifier):
        ...
        if value in ValueStateValues and 1 <= int(qualifier['Pan Speed']) <= 24 and 1 <= int(qualifier['Tilt Speed']) <= 20:
            if value in ['Home', 'Reset']:
                ...
                PanTiltCmdString = pack('>6BHB', self._DeviceID, 0x01, 0x06, 0x01, int(qualifier['Pan Speed']), int(qualifier['Tilt Speed']), ValueStateValues[value], 0xFF)
    ...
    def SetZoom(self, value, qualifier):
        ...
        if value in ValueStateValues and 0 <= int(qualifier['Zoom Speed']) <= 15:
            if value == 'Stop':
                speed = 0
            else:
                speed = ValueStateValues[value] + int(qualifier['Zoom Speed'])
    ...
    def __CheckResponseForErrors(self, sourceCmdName, response):
        ...
        if response and len(response) == 4:
            ...
            if error_byte & 0x60 == 0x60:
                self.Error(
                    ['An error occurred: {}: {}.'.format(sourceCmdName, error_map.get(error_code, 'Unknown Error'))])
                response = b''
        return response
