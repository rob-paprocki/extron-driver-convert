...
class DeviceClass:
    def __init__(self):
        ...
        self.Commands = {
            ...
            'Power': { 'Status': {}},
            'Temperature': { 'Status': {}},
            'Version': {'Parameters': ['Type'], 'Status': {}},
        }
                        
        if self.Unidirectional == 'False':
            self.AddMatchString(re.compile(b'BRIGHTNESS=(\d+)[\r\n]+'), self.__MatchBrightness, None)
            self.AddMatchString(re.compile(b'STANDBY=([01])[\r\n]+'), self.__MatchPower, None)
            self.AddMatchString(re.compile(b'TEMPERATURE VM=(\d+)[\r\n]+'), self.__MatchTemperature, None)
            self.AddMatchString(re.compile(b'VERSION (FIRMWARE|PROTOCOL)=([\s\S]+)[\r\n]+'), self.__MatchVersion, None)
            self.AddMatchString(re.compile(b'Error ([\s\S]+)[\r\n]+'), self.__MatchError, None)
    ...
    def UpdateTemperature(self, value, qualifier):
        ...
        TemperatureCmdString = '?TEMPERATURE VM\n'
