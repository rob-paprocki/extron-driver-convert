...
class DeviceClass:
    def __init__(self, ipAddress, port, deviceUsername, devicePassword, SSLVerifyMode):
        ...
        self.Commands = {
            ...
            'AudioOutput': { 'Status': {}},
            'HDMIInput': {'Parameters':['Index','Source Name'], 'Status': {}},
            'HDMIInputSignalStatus': {'Parameters':['Index'], 'Status': {}},
    ...
    def SetAudioOutput(self, value, qualifier):
        ...
        ValueStateValues = ('Jack', 'HDMI', 'SPDIF')

        if value in ValueStateValues:
            AudioOutputCmdString = 'v2/configuration/audio'
            data = dumps({
                              "enabled": True,
                              "output": value
                            }) 
            self.__SetHelper('AudioOutput', value, qualifier, AudioOutputCmdString, data.encode('iso-8859-1'))
        else:
    ...
    def SetHDMIInput(self, value, qualifier):
        ...
        index_val = qualifier['Index']
        source_val = qualifier['Source Name']
        if 1 <= index_val <= 99 and value in ValueStateValues and source_val:
            HDMIInputCmdString = 'v2/configuration/input-cards/{}'.format(index_val)
            data = dumps({
                          "enabled": ValueStateValues[value],
                          "sourceName": source_val
                        })
            self.__SetHelper('HDMIInput', value, qualifier, HDMIInputCmdString, data.encode('iso-8859-1'))
        else:
            self.Discard('Invalid Command for SetHDMIInput')

    def UpdateHDMIInputSignalStatus(self, value, qualifier):
        ...
        index_val = qualifier['Index']
        if 1 <= index_val <= 99:
            HDMIInputSignalStatusCmdString = 'v2/configuration/input-cards/{}'.format(index_val)
            res = self.__UpdateHelper('HDMIInputSignalStatus', value, qualifier, HDMIInputSignalStatusCmdString)
            if res:
                try:
                    value = ValueStateValues[str(res["hasSignal"])]
                    self.WriteStatus('HDMIInputSignalStatus', value, qualifier)
                except (KeyError, IndexError):
                    self.Error(['HDMI Input Signal Status: Invalid/unexpected response'])
        else:
            self.Discard('Invalid Command for UpdateHDMIInputSignalStatus')
    ...
    def UpdateOutputScreenStatus(self, value, qualifier):
        ...
        index_val = qualifier['Index']
        if 1 <= index_val <= 99:
            OutputScreenStatusCmdString = 'v2/configuration/video-outputs/{}'.format(index_val)
            res = self.__UpdateHelper('OutputScreenStatus', value, qualifier, OutputScreenStatusCmdString)
            if res:
                try:
                    value = ValueStateValues[str(res["connected"])]
    ...
    def SetPairedButtonUpdate(self, value, qualifier):
        ...
        PairedButtonUpdateCmdString = 'v2/configuration/buttons'
        res = self.__UpdateHelper('PairedButtonUpdate', value, qualifier, PairedButtonUpdateCmdString)
        if res:
            try:
    ...
    def SetReboot(self, value, qualifier):
        ...
        RebootCmdString = 'v2/operations/reboot'
        self.__SetHelper('Reboot', value, qualifier, RebootCmdString)

    def SetStandby(self, value, qualifier):

        StandbyCmdString = 'v2/operations/standby'
        self.__SetHelper('Standby', value, qualifier, StandbyCmdString)
    ...
    def UpdateSystemUseStatus(self, value, qualifier):
        ...
        SystemUseStatusCmdString = 'v2/configuration/system/status'
        res = self.__UpdateHelper('SystemUseStatus', value, qualifier, SystemUseStatusCmdString)
        if res:
            try:
    ...
    def SetVideoMode(self, value, qualifier):
        ...
        if value in ValueStateValues:
            VideoModeCmdString = 'v2/configuration/video'
            data = dumps({
                          "cecEnabled": True,
                          "mode": value
                        })
            self.__SetHelper('VideoMode', value, qualifier, VideoModeCmdString, data.encode('iso-8859-1'))
        else:
    ...
    def UpdateVideoMode(self, value, qualifier):
        ...
        VideoModeCmdString = 'v2/configuration/video'
        res = self.__UpdateHelper('VideoMode', value, qualifier, VideoModeCmdString)
        if res:
            try:
                value = res["mode"]
    ...
    def SetWallpaper(self, value, qualifier):
        ...
        if value in ValueStateValues:
            WallpaperCmdString = 'v2/configuration/personalization'
            data = dumps({
                              "showWallpaper": ValueStateValues[value]
                            }) 
            self.__SetHelper('Wallpaper', value, qualifier, WallpaperCmdString, data.encode('iso-8859-1'))
        else:
            self.Discard('Invalid Command for SetWallpaper')

    def UpdateWallpaper(self, value, qualifier):

        WallpaperCmdString = 'v2/configuration/personalization'
        res = self.__UpdateHelper('Wallpaper', value, qualifier, WallpaperCmdString)
        if res:
            try:
                ValueStateValues = {
                    True:  'On',
                ...
                value = ValueStateValues[res["showWallpaper"]]
                self.WriteStatus('Wallpaper', value, qualifier)
            except (KeyError, IndexError, AttributeError):
                self.Error(['Wallpaper: Invalid/unexpected response'])
    ...
    def __SetHelper(self, command, value, qualifier, resource, data=None):
        ...
        rootURL = self.RootURL.replace('http', 'https') if self.DefaultPort == 4003 else self.RootURL
        url = ''.join([rootURL, resource])
