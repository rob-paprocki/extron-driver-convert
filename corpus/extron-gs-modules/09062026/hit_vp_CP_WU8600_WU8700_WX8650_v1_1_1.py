...
class DeviceClass:
    def __init__(self):
        ...
        self.Commands = {
            ...
            'Power': {'Status': {}},
            'VideoMute': {'Status': {}},
            'Volume': {'Parameters': ['Input Select'], 'Status': {}},
            'VolumeStatus': {'Parameters': ['Input Select'], 'Status': {}},
        }
    ...
    def SetInput(self, value, qualifier):
        ...
        InputStateValues = {
            ...
            'USB Type A': b'\xBE\xEF\x03\x06\x00\x5A\x3B\x01\x00\x20\x36\x06\x00'
        }
    ...
    def SetPbyPPinPModeSelect(self, value, qualifier):
        ...
        PbyPPinPCmdString = PbyPPinPStateValues[value]
        self.__SetHelper('PbyPPinPModeSelect', PbyPPinPCmdString, value, qualifier)

    def UpdatePbyPPinPModeSelect(self, value, qualifier):
        ...
        res = self.__UpdateHelper('PbyPPinPModeSelect', PbyPPinPCmdString, value, qualifier)
        if res:
            try:
                value = PbyPPinPStateNames[res[1:2]]
                self.WriteStatus('PbyPPinPModeSelect', value, qualifier)
            except (KeyError, IndexError):
                self.Error(['PbyP PinP Mode Select: Invalid/unexpected response'])
    ...
    def SetVolume(self, value, qualifier):
        ...
        HDBaseT = {
            'Up': b'\xBE\xEF\x03\x06\x00\xA7\xEA\x04\x00\xD5\x20\x00\x00',
            'Down': b'\xBE\xEF\x03\x06\x00\x76\xEB\x05\x00\xD5\x20\x00\x00'
        }

        SDI = {
            'Up': b'\xBE\xEF\x03\x06\x00\xA7\xE5\x04\x00\xE5\x20\x00\x00',
            'Down': b'\xBE\xEF\x03\x06\x00\x76\xE4\x05\x00\xE5\x20\x00\x00'
        }

        DisplayPort = {
            'Up': b'\xBE\xEF\x03\x06\x00\x67\xE1\x04\x00\xF5\x20\x00\x00',
            'Down': b'\xBE\xEF\x03\x06\x00\xB6\xE0\x05\x00\xF5\x20\x00\x00'
        }
        ...
        input_type = qualifier['Input Select']
        if input_type in InputSelectStates:
            VolumeCmdString = (InputSelectStates[input_type])[value]
    ...
    def UpdateVolumeStatus(self, value, qualifier):
        ...
        if input_type in VolumeStatusStateQualifier:
            VolumeStatusCmdString = VolumeStatusStateQualifier[input_type]
