...
class DeviceClass:
    def __init__(self):
        ...
        if self.Unidirectional == 'False':
            ...
            self.AddMatchString(re.compile(b'HdcpE(1|2|3)\*(0|1)\r\n'), self.__MatchHDCPInputAuthorization, 'Set')
    ...
    def SetHDCPInputAuthorization(self, value, qualifier):
        ...
        if value in HDCPStateValues and Input in InputNames:
    ...
    def UpdateHDCPInputAuthorization(self, value, qualifier):
        Input = qualifier['Input']
        if 1 <= int(Input) <= 3:
            HDCPCmdString = '\x1BEHDCP\x0D'
            self.__UpdateHelper('HDCPInputAuthorization', HDCPCmdString, value, qualifier)
        else:
            self.Discard('Device Is Busy for UpdateHDCPInputAuthorization')
