...
class DeviceClass:
    ...
    def __init__(self):
        ...
        if self.Unidirectional == 'False':
            self.AddMatchString(re.compile(b'#\*source is (FAV|HDMI1|HDMI2|YPbPr|VGA|DVI|DP|OPS)[\r\n]'), self.__MatchInput, None)
            self.AddMatchString(re.compile(b'#\*MUTE (ON|OFF)[\r\n]'), self.__MatchMuteStatus, None)
            self.AddMatchString(re.compile(b'#\*Picture Mode is (1|2|3|4)[\r\n]'), self.__MatchPictureMode, None)
            self.AddMatchString(re.compile(b'#\*Quick Standby is (off|on).?[\r\n]'), self.__MatchPower, None)
            self.AddMatchString(re.compile(b'#\*(volume|headphone volume) level is ([0-9]|[1-9][0-9]|100)[\r\n]'), self.__MatchVolume, None)
    ...
    def SetSoundMode(self, value, qualifier):
        ...
        SoundModeCmdString = 'SETSOUNDMODE {}\r\n'.format(state)
