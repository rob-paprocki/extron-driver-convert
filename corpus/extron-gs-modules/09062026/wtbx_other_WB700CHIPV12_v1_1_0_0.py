...
class DeviceClass:
    ...
    def __init__(self, ipAddress, port, deviceUsername, devicePassword):
        ...
        self._devicePassword = devicePassword

        self.RootURL = 'http://{0}:{1}/'.format(self._ipAddress, self._port)
        if self._deviceUsername is not None and self._devicePassword is not None:
            self.authentication = b'Basic ' + base64.b64encode(self._deviceUsername.encode() + b':' + self._devicePassword.encode())
        else:
    ...
    def UpdateAutoReboot(self, value, qualifier):
        ...
        if res:
            try:
                auto_reboot = search('<auto_reboot>([01])</auto_reboot>', res)
    ...
    def __CheckResponseForErrors(self, sourceCmdName, response):
        ...
        res = response.read(1053).decode()
        return res

    def __SetHelper(self, command, value, qualifier, url='', data=None):
        self.Debug = True
        url = '{0}/control.cgi?{1}'.format(self.RootURL.rstrip('/'), url)
