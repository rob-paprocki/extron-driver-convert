...
class DeviceSerialClass:
    ...
    def SetClosedCaption(self, value, qualifier):
        ...
        ValueStateValues = {
            ...
            'Off' : b'\x8C\x00\x10\x03\x01\x01\xA0',
        }
    ...
    def SetStandby(self, value, qualifier):
        ...
        ValueStateValues = {
            'Enable': b'\x8C\x00\x01\x02\x01\x90',
            'Disable': b'\x8C\x00\x01\x02\x00\x8F'
        }
    ...
    def SetVolume(self, value, qualifier):
        ...
        if 0 <= value <= 100:
            data = b''.join([b'\x8C\x00\x05\x03\x01', pack('B', value)])
            checksum = 0
            for i in range(0, len(data)):
                checksum = checksum + data[i]
            VolumeCmdString = data + pack('B', checksum & 0xFF)
        ...
        if res:
            try:
                if res[14] == '1':
                    ...
                    self.WriteStatus('Input', ValueStateValues[res[14]], qualifier)
            except (KeyError, IndexError):
                self.Error(['Input: Invalid/unexpected response'])
