...
class DeviceClass:
    def __init__(self):
        ...
        if self.Unidirectional == 'False':
            self.AddMatchString(re.compile(b'\x0F\d{3}MIS#(HD[2-3])#\x0D'), self.__MatchInput, None)
            self.AddMatchString(re.compile(b'\x0F\d{3}PWS#(-ON|OFF)#\x0D'), self.__MatchPower, None)
            self.AddMatchString(re.compile(b'\x0F\d{3}VOS#(\d{3})#\x0D'), self.__MatchVolume, None)
            self.AddMatchString(re.compile(b'\x0F\d{3}(MUT|MI[NS]|PW[RS]|VO[LS])ERROR\x0D'), self.__MatchError, None)
    ...
    def SetInput(self, value, qualifier):
        ...
        if value in ValueStateValues:
            InputCmdString = '\x0F{0:03d}MINW{1}0\x0D'.format(self._DeviceID, ValueStateValues[value]).encode()
    ...
    def UpdateVolume(self, value, qualifier):
        ...
        VolumeCmdString = '\x0F{:03d}VOSR0000\x0D'.format(self._DeviceID).encode()
