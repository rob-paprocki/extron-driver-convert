...
class DeviceSerialClass:
    def __init__(self):
        ...
        if self.Unidirectional == 'False':
            self.AddMatchString(re.compile(b'c [0-9A-F]{2} OK0(1|2|6|9)x'), self.__MatchAspectRatio, None)
            ...
            self.AddMatchString(re.compile(b'b [0-9A-F]{2} OK([01246789A][0123])x'), self.__MatchInput, None)
            ...
            self.AddMatchString(re.compile(b'd [0-9A-F]{2} OK([01]0)x'), self.__MatchVideoMute, None)
            self.AddMatchString(re.compile(b'f [0-9A-F]{2} OK([0-5][0-9A-F]|6[0-4])x'), self.__MatchVolume, None)
            self.AddMatchString(re.compile(b'([abcdeflm]) [0-9A-F]{2} NG(.*)x'), self.__MatchError, None)

        self.setHelperRex = re.compile(b'(OK|NG)[0-9A-F]{2}x')
    ...
    def DeviceID(self, value):
        if value == 'Broadcast':
            ...
            self.Error(['Invalid DeviceID value.'])
    ...
    def SetInput(self, value, qualifier):
        ...
        inp_num = qualifier['Number']
        if inp_num in NumberStates:
            inp_val = (ValueStateValues[value] << 4) + NumberStates[inp_num]
            InputCmdString = 'xb {0} {1:02X}\r'.format(self._DeviceID, inp_val)
    ...
    def __MatchInput(self, match, tag):
        ...
        temp_val = int(match.group(1).decode(), 16)
        qualifier = {'Number': NumberStates[temp_val & 15]}
        value = ValueStateValues[temp_val >> 4]
