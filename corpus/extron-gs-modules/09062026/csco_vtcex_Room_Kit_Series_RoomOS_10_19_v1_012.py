...
class DeviceClass:
    def __init__(self):
        ...
        self.Models = {
            'Webex Room Kit Mini RoomOS 10.19.X': self.csco_12_6000_mini,
            'Webex Room Kit RoomOS 10.19.X': self.csco_12_6000_non_p,
            'Webex Room Kit Plus RoomOS 10.19.X': self.csco_12_6000_plus,
            'Webex Room Kit Pro RoomOS 10.19.X': self.csco_12_6000_pro,
        }
        ...
        self.dirName = compile('\*r PhonebookSearchResult Contact (\d+) Name: "([\S ]+?)"\r\n')
        self.dirNumber = compile('\*r PhonebookSearchResult Contact (\d+) ContactMethod 1 Number: "([\S ]+?)"\r\n')
        self.FolderNameRex = compile('\*r PhonebookSearchResult Folder (\d+) Name: "([\S ]+?)"\r\n')
        self.FolderIDRex = compile('\*r PhonebookSearchResult Folder (\d+) FolderId: "([\S ]+?)"\r\n')
        self.peripheralName = compile('\*s Peripherals ConnectedDevice \d+ Name: "([\S ]+?)"\r\n')
        self.peripheralType = compile('\*s Peripherals ConnectedDevice \d+ Type: ([\S ]+?)\r\n')
        self.peripheralMAC = compile('\*s Peripherals ConnectedDevice \d+ ID: "([\S ]+?)"\r\n')
        self.callBackNumber = compile('Entry (\d+) CallbackNumber: "([\S ]+?)"\r\n')
        self.displayName = compile('Entry (\d+) DisplayName: "([\S ]+?)"\r\n')
        ...
        self.lastOccurrenceTime = compile('Entry (\d+) LastOccurrenceStartTime: "([\S ]+?)"\r\n')
        self.occurrenceCount = compile('Entry (\d+) OccurrenceCount: (\d+)\r\n')
        self.occurrenceType = compile('Entry (\d+) OccurrenceType: ([\S ]+?)\r\n')
        
        if self.Unidirectional == 'False':
            self.AddMatchString(compile(b'\*c xConfiguration Audio Output (InternalSpeaker|Line [1-6]|HDMI [1-3]|USBInterface 1) Mode: (On|Off|UltrasoundOnly)\r\n'), self.__MatchAudioOutput, None)
            self.AddMatchString(compile(b'\*c xConfiguration Conference AutoAnswer Mode: (Off|On)\r\n'), self.__MatchAutoAnswer, None)
            self.AddMatchString(compile(b'\*s Conference Call (\d+) AuthenticationRequest: (None|HostPinOrGuest|HostPinOrGuestPin|PanelistPin|AnyHostPinOrGuestPin|GuestPin)\r\n'), self.__MatchCallAuthenticationRequestStatus, None)
            ...
            self.AddMatchString(compile(b'\*s Video Selfview PIPPosition: (UpperLeft|UpperCenter|UpperRight|CenterLeft|CenterRight|LowerLeft|LowerRight|DockCenterLeft|DockLowerLeft)\r\n'), self.__MatchSelfViewPosition, None)
    ...
    def SetSelfViewPosition(self, value, qualifier):
        ...
        ValueStateValues = {
            ...
            'Dock Center Left' : 'DockCenterLeft',
            'Dock Lower Left'  : 'DockLowerLeft'
        }
    ...
    def __MatchSelfViewPosition(self, match, tag):
        ...
        ValueStateValues = {
            ...
            'DockCenterLeft' : 'Dock Center Left',
            'DockLowerLeft'  : 'Dock Lower Left'
        }
