...
class DeviceClass:
    def __init__(self):
        ...
        self.Commands = {
            ...
            'OverloadProtect': {'Status': {}},
            'PremixerGain': {'Parameters':['Input'], 'Status': {}},
            'PresetRecall': { 'Status': {}},
            'SignalPresence': {'Status': {}},
            'Temperature': { 'Status': {}},
            'ThermalLimiting': {'Status': {}},
        ...
        if self.Unidirectional == 'False':
            self.AddMatchString(re.compile(b'32Stat([0-4]{5})((?:\*[0-3]{5}){1,4})\r\n'), self.__MatchFaultStatus, None)
            self.AddMatchString(re.compile(b'(53|54|60|61)Stat([0-2])\r\n', re.I), self.__MatchSystemFault, None)
            self.AddMatchString(re.compile(b'(55|57|58|62|64)Stat([0-2])\r\n', re.I), self.__MatchChannelFault, None)
            self.AddMatchString(re.compile(b'GrpmD([0-9]{1,2})\*([0-9 -]{1,5})\r\n'), self.__MatchGroup, None)
            self.AddMatchString(re.compile(b'DsG(40000|40001)\*([0-9 -]{1,5})\r\n'), self.__MatchInputGain, None)
            self.AddMatchString(re.compile(b'DsG(40100|40101)\*([0-9 -]{1,5})\r\n'), self.__MatchPremixerGain, None)
            self.AddMatchString(re.compile(b'DsG(50100|50101)\*([0-9 -]{1,5})\r\n'), self.__MatchATInputPremixerGain, None)
            self.AddMatchString(re.compile(b'DsG(60000|60001|60002|60003|60004)\*([0-9 -]{1,5})\r\n'), self.__MatchOutputAttenuation, None)
    ...
    def SetAmpOutputAttenuation(self, value, qualifier):
        ...
        if self.__CheckValidLevelValue('AmpOutputAttenuation', value):
            level = round(value*10)
            commandString = 'WG60000*{0:05d}AU\r'.format(level)
    ...
    def __MatchOutputAttenuation(self, match, tag):
        ...
        if 'AMP' in self.OutputAttenuationStates[match.group(1).decode()]:
            ...
            self.WriteStatus('AmpOutputAttenuation', value, None)
        elif 'Line' in self.OutputAttenuationStates[match.group(1).decode()]:
            channel = str(int(match.group(1)) - 60000)
            ...
            channel = str(int(match.group(1)) - 60002)
            qualifier = {'Output' : channel}
            value = int(match.group(2))/10
    ...
    def SetATOutputAttenuation(self, value, qualifier):
        ...
        if 1 <= channel <= self.OutputSize and self.__CheckValidLevelValue('ATOutputAttenuation', value):
            level = round(value*10)
            commandString = 'WG{0}*{1:05d}AU\r'.format(channel + 60002, level)
    ...
    def UpdateATOutputAttenuation(self, value, qualifier):
        ...
        if 1 <= channel <= self.OutputSize:
            commandString = 'WG{0}AU\r'.format(channel + 60002)
    ...
    def __MatchChannelFault(self, match, tag):
        ...
        fltsplit = flt.split('*')
        if type == '55':
            self.WriteStatus('ThermalLimiting', faulttable[flt], None)
        elif type == '57':
            self.WriteStatus('OverloadProtect', faulttable[flt], None)
        elif type == '58':
            self.WriteStatus('OpenCircuit', faulttable[flt], None)
        elif type == '62':
            self.WriteStatus('SignalPresence', signalpresence[flt], None)
        elif type == '64':
            self.WriteStatus('DigitalClip', faulttable[flt], None)
    ...
    def __MatchFaultStatus(self, match, tag):
        ...
        for index, items in enumerate(values):
            self.WriteStatus('SignalPresence', signalpresence[items[0]], None)
            self.WriteStatus('OverloadProtect', faulttable[items[1]], None)
            self.WriteStatus('ThermalLimiting', faulttable[items[2]], None)
            self.WriteStatus('OpenCircuit', faulttable[items[3]], None)
            self.WriteStatus('DigitalClip', faulttable[items[4]], None)
