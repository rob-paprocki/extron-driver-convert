...
class DeviceClass:
    def __init__(self):
        ...
        self.Commands = {
            ...
            'Backlight': { 'Status': {}},
            'Bright': { 'Status': {}},
            'CameraTrackingMode': { 'Status': {}},
            'CameraTrackingStatus': { 'Status': {}},
            'Focus': { 'Status': {}},
            'FocusMode': { 'Status': {}},
            'Freeze': { 'Status': {}},
            'Gain': { 'Status': {}},
    ...
    def SetAutoExposure(self, value, qualifier):
        ...
        if value in ValueStateValues:
            AutoExposureCmdString = self.SetHeader(pack('6B', self._DeviceID, 0x01, 0x04, 0x39, 
                                        ValueStateValues[value], 0xFF), 'Set')
    ...
    def SetAutoTracking(self, value, qualifier):
        ...
        if value in ValueStateValues:
            AutoTrackingCmdString = self.SetHeader(pack('6B', self._DeviceID, 0x01, 0x04, 0x7D,
                                                ValueStateValues[value], 0xFF), 'Set')
    ...
    def SetBacklight(self, value, qualifier):
        ...
        if value in ValueStateValues:
            BacklightCmdString = self.SetHeader(pack('6B', self._DeviceID, 0x01, 0x04, 0x33, 
                                                        ValueStateValues[value], 0xFF), 'Set')
    ...
    def SetBright(self, value, qualifier):
        ...
        if value in ValueStateValues:
            BrightCmdString = self.SetHeader(pack('6B', self._DeviceID, 0x01, 0x04, 0x0D, 
                                                        ValueStateValues[value], 0xFF), 'Set')
            self.__SetHelper('Bright', BrightCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetBright')

    def UpdateCameraTrackingMode(self, value, qualifier):

        CameraTrackingModeCmdString = self.SetHeader(pack('6B', self._DeviceID, 0x09, 0x36, 0x69, 0x01, 0xFF), 'Get')
        res = self.__UpdateHelper('CameraTrackingMode', CameraTrackingModeCmdString, value, qualifier)
        if res:
            try:
                ValueStateValues = {
                    0x01: 'Presenter',
                ...
                value = ValueStateValues[res[-2]]
                self.WriteStatus('CameraTrackingMode', value, qualifier)
            except (KeyError, IndexError, AttributeError):
                self.Error(['Camera Tracking Mode: Invalid/unexpected response'])

    def UpdateCameraTrackingStatus(self, value, qualifier):

        CameraTrackingStatusCmdString = self.SetHeader(pack('6B', self._DeviceID, 0x09, 0x36, 0x69, 0x02, 0xFF), 'Get')
        res = self.__UpdateHelper('CameraTrackingStatus', CameraTrackingStatusCmdString, value, qualifier)
        if res:
            try:
                ValueStateValues = {
                    0x01: 'On',
                ...
                value = ValueStateValues[res[-2]]
                self.WriteStatus('CameraTrackingStatus', value, qualifier)
            except (KeyError, IndexError, AttributeError):
                self.Error(['Camera Tracking Status: Invalid/unexpected response'])
    ...
    def SetFocus(self, value, qualifier):
        ...
        if value in ValueStateValues:
            FocusCmdString = self.SetHeader(pack('6B', self._DeviceID, 0x01, 0x04, 0x08, 
                                    ValueStateValues[value], 0xFF), 'Set')
    ...
    def SetFreeze(self, value, qualifier):
        ...
        if value in ValueStateValues:
            FreezeCmdString = self.SetHeader(pack('6B', self._DeviceID, 0x01, 0x04, 0x62, 
                                            ValueStateValues[value], 0xFF), 'Set')
    ...
    def SetGain(self, value, qualifier):
        ...
        if value in ValueStateValues:
            GainCmdString = self.SetHeader(pack('6B', self._DeviceID, 0x01, 0x04, 0x0C, 
                                        ValueStateValues[value], 0xFF), 'Set')
    ...
    def SetHome(self, value, qualifier):
        ...
        HomeCmdString = self.SetHeader(pack('5B', self._DeviceID, 0x01, 0x06, 0x04, 0xFF), 'Set')
    ...
    def SetIris(self, value, qualifier):
        ...
        if value in ValueStateValues:
            IrisCmdString = self.SetHeader(pack('6B', self._DeviceID, 0x01, 0x04, 0x0B, 
                                        ValueStateValues[value], 0xFF), 'Set')
            self.__SetHelper('Iris', IrisCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetIris')

    def SetMemorySpecial(self, value, qualifier):

        ValueStateValues = {
            'Turn On OSD Menu' : 0x5F, 
            'Full Body'        : 0xA0, 
            'Upper Body'       : 0xA1, 
            'Tracking Point'   : 0xA2, 
            'Switch'           : 0xA3, 
        ...
        if value in ValueStateValues:
            MemorySpecialCmdString =  self.SetHeader(pack('7B', self._DeviceID, 0x01, 0x04, 0x3F, 0x01,
                                                ValueStateValues[value], 0xFF), 'Set')
            self.__SetHelper('MemorySpecial', MemorySpecialCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetMemorySpecial')
    def SetMenu(self, value, qualifier):

        MenuCmdString = self.SetHeader(pack('6B', self._DeviceID, 0x01, 0x06, 0x06, 0x10, 0xFF), 'Set')
        self.__SetHelper('Menu', MenuCmdString, value, qualifier)
    def SetMenuEnter(self, value, qualifier):

        MenuEnterCmdString = self.SetHeader(pack('8B', self._DeviceID, 0x01, 0x7E, 0x01, 0x02, 0x00, 0x01, 0xFF), 'Set')
        self.__SetHelper('MenuEnter', MenuEnterCmdString, value, qualifier)
    ...
    def SetPanTiltReset(self, value, qualifier):
        ...
        PanTiltResetCmdString = self.SetHeader(pack('5B', self._DeviceID, 0x01, 0x06, 0x05, 0xFF), 'Set')
    ...
    def SetPower(self, value, qualifier):
        ...
        if value in ValueStateValues:
            PowerCmdString = self.SetHeader(pack('6B', self._DeviceID, 0x01, 0x04, 0x00,
                                            ValueStateValues[value], 0xFF), 'Set')
    ...
    def SetPresetFreeze(self, value, qualifier):
        ...
        if value in ValueStateValues:
            PresetFreezeCmdString = self.SetHeader(pack('6B', self._DeviceID, 0x01, 0x04, 0x62, 
                                                        ValueStateValues[value], 0xFF), 'Set')
            self.__SetHelper('PresetFreeze', PresetFreezeCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetPresetFreeze')
    ...
    def SetPresetReset(self, value, qualifier):
        ...
        if ValueConstraints['Min'] <= value <= ValueConstraints['Max']:
            PresetResetCmdString = self.SetHeader(pack('7B', self._DeviceID, 0x01, 0x04, 0x3F, 0x00, value, 0xFF), 'Set')
    ...
    def SetPresetSave(self, value, qualifier):
        ...
        if ValueConstraints['Min'] <= value <= ValueConstraints['Max'] and value not in [0x5F, 0xA0, 0xA1, 0xA2, 0xA3, 0xA4, 0xA5]:
    ...
    def SetTallyLamp(self, value, qualifier):
        ...
        if value in ValueStateValues:
            TallyLampCmdString = self.SetHeader(pack('8B', self._DeviceID, 0x01, 0x7E, 0x01, 0x0A, 0x00, 
                                                        ValueStateValues[value], 0xFF), 'Set')
    ...
    def SetWDR(self, value, qualifier):
        ...
        if value in ValueStateValues:
            WDRCmdString = self.SetHeader(pack('6B', self._DeviceID, 0x01, 0x04, 0x3D, 
                                                        ValueStateValues[value], 0xFF), 'Set')
    ...
    def SetWhiteBalance(self, value, qualifier):
        ...
        if value in ValueStateValues:
            WhiteBalanceCmdString = self.SetHeader(pack('6B', self._DeviceID, 0x01, 0x04, ValueStateValues[value][0],
                                        ValueStateValues[value][1], 0xFF), 'Set')
