...
class DeviceClass:
    ...
    def SetPIPInput(self, value, qualifier):
        ...
        if q2 in QuadrantStates and q3 in QuadrantStates and q4 in QuadrantStates:
            PIPInputCmdString = pack('>8B', 0x09, self._DeviceID, self._GroupID, 0x84, 0xFD, QuadrantStates[q2], QuadrantStates[q3], QuadrantStates[q4])
    ...
    def UpdatePIPInputQuadrant2(self, value, qualifier):
        ...
        if res:
            try:
                ...
                print('Invalid/unexpected response for UpdatePIPInputQuadrant2')

            try:
                value = ValueStateValues[res[-3]]
                self.WriteStatus('PIPInputQuadrant3', value, qualifier)
            except (KeyError, IndexError):
                print('Invalid/unexpected response for UpdatePIPInputQuadrant3')

            try:
                value = ValueStateValues[res[-2]]
                self.WriteStatus('PIPInputQuadrant4', value, qualifier)
            except (KeyError, IndexError):
                print('Invalid/unexpected response for UpdatePIPInputQuadrant4')
    ...
    def SetPIP(self, value, qualifier):
        ...
        if value == 'Off':
            PIPCmdString = pack('>8B', 0x09, self._DeviceID, self._GroupID, 0x3C, 0x00, 0x00, 0x00, 0x00)
        else:
            PIPCmdString = pack('>8B', 0x09, self._DeviceID, self._GroupID, 0x3C, 0x01, ValueStateValues[value], 0x00, 0x00)
    ...
    def UpdatePIP(self, value, qualifier):
        ...
        PIPCmdString = pack('>4B', 0x05, self._DeviceID, self._GroupID, 0x3D)
        ChkSum = self.calcChkSum(PIPCmdString)
        PIPCmdString = PIPCmdString + ChkSum
    ...
    def SetVolume(self, value, qualifier):
        ...
        if ValueConstraints['Min'] <= value <= ValueConstraints['Max']:
            VolumeCmdString = pack('>6B', 0x07, self._DeviceID, self._GroupID, 0x44, value, 0xFF)
