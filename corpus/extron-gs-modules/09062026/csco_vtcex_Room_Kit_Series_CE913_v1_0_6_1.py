...
class DeviceClass:
    def __init__(self):
        ...
        self._CallHistoryOccurrenceType = None
        ...
        self.Models = {
            'Webex Room Kit CE9.13.X': self.csco_12_4932_non_p,
            'Webex Room Kit Plus CE9.13.X': self.csco_12_4932_plus,
            'Webex Room Kit Pro CE9.13.X': self.csco_12_4932_pro,
        }
        ...
        self.Commands = {
            ...
            'CameraFocus': {'Status': {}},
            'CameraFocusP': {'Parameters': ['Camera'], 'Status': {}},
            'CameraPan': {'Parameters': ['Speed'], 'Status': {}},
            'CameraPanP': {'Parameters': ['Camera', 'Speed'], 'Status': {}},
            'CameraPresetRecall': {'Status': {}},
            'CameraPresetSave': {'Status': {}},
            'CameraPresetSaveP': {'Parameters': ['Camera'], 'Status': {}},
            'CameraTilt': {'Parameters': ['Speed'], 'Status': {}},
            'CameraTiltP': {'Parameters': ['Camera', 'Speed'], 'Status': {}},
            'CameraZoom': {'Parameters': ['Speed'], 'Status': {}},
            'CameraZoomP': {'Parameters': ['Camera', 'Speed'], 'Status': {}},
        ...
        if self.Unidirectional == 'False':
            self.AddMatchString(compile(b'\*c xConfiguration Audio Output (InternalSpeaker|Line [1-6]|HDMI [1-3]) Mode: (On|Off|UltrasoundOnly)\r\n'), self.__MatchAudioOutput, None)
            ...
            self.AddMatchString(compile(b'\*c xConfiguration Audio Input (Microphone [1-8]|HDMI [1-5]) Mode: (On|Off)\r\n'), self.__MatchInputMute, None)
    ...
    def UpdateCallAuthenticationRequestStatus(self, value, qualifier):
        ...
        if 1 <= call <= 5 and 'CallStatus' in self.initialStatusList:
            if 'CallAuthenticationRequestStatus' not in self.initialStatusList:
                self.initialStatusList.append('CallAuthenticationRequestStatus')
    ...
    def UpdateCallStatus(self, value, qualifier):
        ...
        if 1 <= int(qualifier['Call']) <= 5:
            if 'CallStatus' not in self.initialStatusList:
                self.initialStatusList.append('CallStatus')
                self.Send('xFeedback register /Status/Call\r')  # send subscription
                self.__UpdateHelper('CallStatus', 'xstatus call\r', value, qualifier)  # send query for initial status
        else:
            self.Discard('Device Is Busy for UpdateCallStatus')
    ...
    def __MatchCallStatus(self, match, tag):
        ...
        if ' Status: ' in res or ' CallType: ' in res or ' DisplayName: ' in res or ' RemoteNumber: ' in res or ' DeviceType: ' in res or ' AnswerState: ' in res or ' (ghost=True):' in res:  # only update status if useful to driver
            callEndIDList = findall(self.callEndPattern, res)  # there should only be 1
            try:
                if callEndIDList[0] in self.callStatusData:  # if call ID in self.callStatusData
                    del self.callStatusData[callEndIDList[0]]  # remove it
            except IndexError:
    ...
    def SetCameraFocusP(self, value, qualifier):

        CameraFocusPCmdString = ''
        camID = qualifier['Camera']
        if 1 <= int(camID) <= 7 and value in ['Far', 'Near', 'Stop']:
            CameraFocusPCmdString = 'xCommand Camera Ramp CameraId:{0} Focus:{1}\r'.format(camID, value)

        if CameraFocusPCmdString:
            self.__SetHelper('CameraFocusP', CameraFocusPCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetCameraFocusP')
    ...
    def SetCameraPanP(self, value, qualifier):

        camID = qualifier['Camera']
        camSpeed = qualifier['Speed']
        if 1 <= int(camID) <= 7 and 1 <= camSpeed <= 15 and value in ['Left', 'Right', 'Stop']:
            if value == 'Stop':
                CameraPanPCmdString = 'xCommand Camera Ramp CameraId:{0} Pan: Stop\r'.format(camID)
            else:
                CameraPanPCmdString = 'xCommand Camera Ramp CameraId:{0} Pan:{1} PanSpeed:{2}\r'.format(camID, value, camSpeed)

            self.__SetHelper('CameraPanP', CameraPanPCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetCameraPanP')
    ...
    def SetCameraPresetSaveP(self, value, qualifier):

        camID = qualifier['Camera']
        if 1 <= int(camID) <= 7 and 1 <= int(value) <= 35:
            CameraPresetSavePCmdString = 'xCommand Camera Preset Store PresetId:{0} CameraId:{1}\r'.format(value, camID)
            self.__SetHelper('CameraPresetSaveP', CameraPresetSavePCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetCameraPresetSaveP')
    ...
    def SetCameraTiltP(self, value, qualifier):

        camID = qualifier['Camera']
        camSpeed = qualifier['Speed']
        if 1 <= int(camID) <= 7 and 1 <= int(camSpeed) <= 15 and value in ['Up', 'Down', 'Stop']:
            if value == 'Stop':
                CameraTiltPCmdString = 'xCommand Camera Ramp CameraId:{0} Tilt: Stop\r'.format(camID)
            else:
                CameraTiltPCmdString = 'xCommand Camera Ramp CameraId:{0} Tilt:{1} TiltSpeed:{2}\r'.format(camID, value, camSpeed)

            self.__SetHelper('CameraTiltP', CameraTiltPCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetCameraTiltP')
    ...
    def SetCameraZoomP(self, value, qualifier):

        camID = qualifier['Camera']
        camSpeed = qualifier['Speed']
        if 1 <= int(camID) <= 7 and 1 <= int(camSpeed) <= 15 and value in ['In', 'Out', 'Stop']:
            if value == 'Stop':
                CameraZoomPCmdString = 'xCommand Camera Ramp CameraId:{0} Zoom: Stop\r'.format(camID)
            else:
                CameraZoomPCmdString = 'xCommand Camera Ramp CameraId:{0} Zoom:{1} ZoomSpeed:{2}\r'.format(camID, value, camSpeed)

            self.__SetHelper('CameraZoomP', CameraZoomPCmdString, value, qualifier)
        else:
            self.Discard('Invalid Command for SetCameraZoomP')
    ...
    def SetHook(self, value, qualifier):
        ...
        if value in ['Accept', 'Reject', 'Disconnect All']:
            ...
            if number and protocol in ProtocolStates and qualifier['Call Type'] in ['Audio', 'Video', 'Auto']:
                if protocol == 'Auto':
    ...
    def UpdateInputSignal(self, value, qualifier):
        ...
        if 1 <= int(qualifier['Input']) <= self.InputMax:
            if 'InputSignal' not in self.initialStatusList:
                self.initialStatusList.append('InputSignal')
                self.Send('xFeedback register /Status/Video/Input/Connector/Connected\r')  # send subscription
                self.__UpdateHelper('InputSignal', 'xStatus Video Input Connector {0} Connected\r'.format(qualifier['Input']), value, qualifier)  # send query for initial status
        else:
            self.Discard('Device Is Busy for UpdateInputSignal')
    ...
    def UpdateMicrophonesMute(self, value, qualifier):
        ...
        self.__UpdateHelper('MicrophonesMute', 'xStatus Audio Microphones Mute\r', value, qualifier)  # send query for initial status
    ...
    def UpdatePeopleCountCurrent(self, value, qualifier):
        ...
        self.__UpdateHelper('PeopleCountCurrent', 'xStatus RoomAnalytics PeopleCount Current\r', value, qualifier)  # send query for initial status
    ...
    def UpdatePeoplePresence(self, value, qualifier):
        ...
        self.__UpdateHelper('PeoplePresence', 'xStatus RoomAnalytics PeoplePresence\r', value, qualifier)  # send query for initial status
    ...
    def SetPhonebookFolderID(self, value, qualifier):

        if 0 <= len(value) <= 255:
            self.WriteStatus('PhonebookFolderID', value, qualifier)
        else:
            self.Discard('Invalid Command for SetPhonebookFolderID')
    ...
    def SetPhonebookFolderIDSearchSet(self, value, qualifier):
        ...
        if 1 <= value <= self._NumberofPhonebookFolderSearch:
            ...
            if folderName not in [self.phonebook_folder_id.end, self.phonebook_folder_id.fill]:
                ...
                self.WriteStatus('PhonebookFolderID', number, None)
        else:
    ...
    def SetPhonebookSearch(self, value, qualifier):
        ...
        if 0 <= len(value) <= 255 and qualifier['Phonebook Type'] in ['Corporate', 'Local']:
            self.WriteStatus('PhonebookSearch', value, qualifier)
        else:
    ...
    def SetPhonebookUpdate(self, value, qualifier):
        ...
        if qualifier['Phonebook Type'] in ['Corporate', 'Local'] and value in ['Refresh', 'Next Search', 'Previous Search']:
            if value == 'Refresh':
                self.phonebook_qualifier = qualifier
                self.phonebook.clear()
                self.phonebook.load()
            elif value == 'Next Search' and self.phonebook_qualifier:
                self.phonebook.scroll(self.phonebook.chunk_size)
            elif value == 'Previous Search' and self.phonebook_qualifier:
                self.phonebook.scroll(-self.phonebook.chunk_size)
            else:
    ...
    def __UpdatePhonebookHelper(self, offset, size):
        ...
        phonebookType = self.phonebook_qualifier['Phonebook Type']
        contact = self.ReadStatus('PhonebookSearch', self.phonebook_qualifier)
        folderID = self.ReadStatus('PhonebookFolderID', None)
    ...
    def UpdatePresentationModeStatus(self, value, qualifier):
        ...
        self.__UpdateHelper('PresentationModeStatus', 'xStatus Conference Presentation Mode\r', value, qualifier)  # send query for initial status
    ...
    def UpdatePresentationSendingModeStatus(self, value, qualifier):
        ...
        if 1 <= int(instanceID) <= 6:
            if 'PresentationSendingModeStatus' not in self.initialStatusList:
                self.initialStatusList.append('PresentationSendingModeStatus')
                self.Send('xFeedback register /Status/Conference/Presentation/LocalInstance/SendingMode\r')  # send subscription
            else:
                self.Discard('Device Is Busy for UpdatePresentationSendingModeStatus')
    ...
    def UpdatePresentationSourceStatus(self, value, qualifier):
        ...
        instanceID = qualifier['Instance']
        if 1 <= int(instanceID) <= 6:
            if 'PresentationSourceStatus' not in self.initialStatusList:
                self.initialStatusList.append('PresentationSourceStatus')
                self.Send('xFeedback register /Status/Conference/Presentation/LocalInstance/Source\r')  # send subscription
            else:
                self.Discard('Device Is Busy for UpdatePresentationSourceStatus')
            self.__UpdateHelper('PresentationSourceStatus', 'xStatus Conference Presentation LocalInstance {0} Source\r'.format(instanceID), value, qualifier)  # send query for initial status
        else:
            self.Discard('Device Is Busy for UpdatePresentationSourceStatus')
    ...
    def UpdateSelfView(self, value, qualifier):
        ...
        self.__UpdateHelper('SelfView', 'xStatus Video Selfview Mode\r', value, qualifier)  # send query for initial status
    ...
    def UpdateSelfViewPosition(self, value, qualifier):
        ...
        self.__UpdateHelper('SelfViewPosition', 'xStatus Video Selfview PIPPosition\r', value, qualifier)  # send query for initial status
