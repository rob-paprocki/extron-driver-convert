...
python -u tools/test_pkg_patch.py
...
python -u experiments/skeleton_p20/test_p20_wire.py
...
python -u experiments/corpus_sweep/test_sweep.py
...
One more suite runs only where Extron's loader can, and is not in that count:

```
python -u experiments/gcp_harness/test_load_package.py
```

It is the acceptance check below as a test: 32-bit PowerShell asks Extron's
`LoadFromFile` and `BinaryFormatter` about every sample package (the control)
and every package in `experiments/skeleton_i20/out/`, and each of ours must
carry exactly the commands our own parser reads. Anywhere else it prints a
skip and exits 0. 72 checks on the 2026-09 workstation with `20020`–`20028`;
it fails, as it should, on finding 18's `q_MIX.pkp`.

### Acceptance check

```
<PS32> -ExecutionPolicy Bypass -File experiments/gcp_harness/Load-Package.ps1
    -Path experiments/skeleton_i20/out/1bynd_19_20028_v1_0_0.pkp -Commands
```

**45 commands, 42 on `IV-CAM-I12` and 45 on `IV-CAM-I20`, exit 0** means the
