...
class TemplateIndex:
    ...
    # -- package-level diagnostic: IL-only (undeclared) Transformations ----
    def undeclared_transformations(self) -> "UndeclaredTransformationsReport":
        """List EVERY Transformation name this driver_definition references
        anywhere, minus the names actually declared in Transformations[].

        This is deliberately independent of resolve_text()/build_set_body():
        those raise CrestronTemplateError per-command the moment a Commands[]
        entry cannot be resolved (a binary/VISCA template failing JSON
        parsing, say) -- which, for a driver like the 1 Beyond cameras
        (findings/07), fires on a *different*, earlier problem for almost
        every command and so never even reaches the undeclared-Transformation
        check for most of them. It also stays raise-rather-than-degrade for
        actual TRANSLATION (see resolve_slot's own "referenced but not
        defined" raise) -- this pass never raises, because reporting a gap
        automatically at package level is not the same as skipping past it
        silently during code generation.

        A "reference" is any {"Transformation": "<name>", ...} entry
        anywhere in the tree -- not only inside Rules[] despite findings/07's
        phrasing, but also inside Commands[].Transformations[] and
        Responses[].Transformations[] (decode-side). Scanning the whole
        document, not just Rules[], is what reproduces findings/08's exact
        P20 count (39 referenced / 27 declared / 12 undeclared) -- see
        experiments/crestron2cs/test_crestron2cs.py.

        These undeclared names are exactly the IL-only residue findings/07
        and findings/08 describe: a Crestron V2 Entity Model driver's JSON
        *names* a Transformation its Rules/Commands/Responses need, but its
        actual behaviour is compiled IL in the driver .dll, invisible to a
        JSON-only reader. Never raises -- an undeclared name here is the
        reportable gap itself, not a failure to resolve one.
        """
        referenced: set[str] = set()

        def walk(node):
            if isinstance(node, dict):
                t = node.get("Transformation")
                if isinstance(t, str):
                    referenced.add(t)
        ...
        walk(self.dd)
        declared = set(self.transformations_by_name)
        return UndeclaredTransformationsReport(
...
class UndeclaredTransformationsReport:
    """Result of TemplateIndex.undeclared_transformations()."""
    referenced: list[str]
    declared: list[str]
    undeclared: list[str]


def find_undeclared_transformations(pkg_path: str) -> UndeclaredTransformationsReport:
    """Convenience entry point for the --list-il-only CLI path (and for
    tests): load `pkg_path`'s driver_definition and run the package-level
    undeclared-Transformation scan. Raises CrestronTemplateError if the
    package has no driver_definition at all (no .dll) -- that is a genuine
    "cannot even look" case, unlike an undeclared Transformation name, which
    ...
    return TemplateIndex(dd).undeclared_transformations()
...
def provenance_path(pkg_path: str) -> str:
    """Repo-relative, forward-slashed form of `pkg_path` for the module docstring.

    Never embed the caller's absolute path. It is meaningless in any other
    checkout, and a Windows one carries backslash escapes that make the
    emitted docstring unparseable: a repo under ``C:\\Users`` puts a literal
    ``\\U`` in a non-raw triple-quoted string, which is read as a truncated
    ``\\UXXXXXXXX`` escape and raises SyntaxError. Keeping it relative also
    makes the generated bytes independent of where the repo was cloned, which
    is what lets two checkouts be compared byte-for-byte.
    """
    target = os.path.abspath(pkg_path)
    root = os.path.normpath(os.path.join(_HERE, "..", ".."))
    try:
        rel = os.path.relpath(target, root)
    except ValueError:  # different drive on Windows
        return os.path.basename(target)
    if rel.split(os.sep)[0] == os.pardir:  # outside the repo
        return os.path.basename(target)
...
def emit_module(idx: TemplateIndex, class_name: str, source_pkg: str):
    ...
    module_src = HEADER % {
        ...
        "source_pkg": provenance_path(source_pkg),
...
def main(argv=None):
    ...
    ap.add_argument("-o", "--output", help="output .py path (required unless --list-il-only)")
    ap.add_argument("--report", help="optional generation-report .json path")
    ap.add_argument("--class-name", default="CrestronDerived")
    ap.add_argument(
        "--list-il-only", action="store_true",
        help="package-level diagnostic: list every Transformation name this "
             "driver references anywhere (Commands[]/Responses[]/Rules[]) "
             "but never declares in Transformations[] -- the IL-only "
             "residue of a Crestron V2 Entity Model driver (findings/07, "
             "findings/08) -- and exit without translating"
    )
    args = ap.parse_args(argv)

    if args.list_il_only:
        rep = find_undeclared_transformations(args.pkg)
        print("%s: %d Transformation name(s) referenced, %d declared, "
              "%d undeclared (IL-only)" % (
                  args.pkg, len(rep.referenced), len(rep.declared), len(rep.undeclared)
              ))
        for name in rep.undeclared:
            print("  %s" % name)
        return 0

    if not args.output:
        ap.error("-o/--output is required unless --list-il-only is given")

    module_src, report = translate_pkg(args.pkg, args.class_name)
    # Explicit encoding and newline: the platform default (cp1252 on Windows)
    # raises on non-ASCII in the Samsung translation, and would also make the
    # generated bytes depend on the machine that ran it.
