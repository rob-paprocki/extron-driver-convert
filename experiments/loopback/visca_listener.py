...
1bynd_19_20028 - and this records every VISCA frame the processor sends, decodes
...
change the feedback test needs:
    ...
    trackingmode group|presenter | trackingprofile N (1-4)
    expcompmode on|off | expcomp N (0-14) | focus N (12224-20664)
    afbehavior global|center|face | afsensitivity N (1-3)
    autoprivacy on|off | autosoftwareupdate on|off
"""
...
# [20028 / v1.6] MapBooleanToBinaryOnOff (AutoPrivacyMode, AutoSoftwareUpdate,
# and the GetGroupTracking / GetTrackingFramingProfile-adjacent booleans):
# 01/00, not VISCA's usual 02/03.
BINARY_ON_OFF = {0x01: "On", 0x00: "Off"}
# MapAutoFocusBehavior: global 0x00, center 0x01, face 0x04.
AUTO_FOCUS_BEHAVIOR = {0x00: "Global", 0x01: "Center", 0x04: "Face"}
...
_ONE_BYTE_SETS = {
    ...
    0x3E: ("ExposureCompensationMode", ON_OFF),  # [20028] 81 01 04 3E {OnOff} FF
}
...
_INQUIRIES = {
    ...
    # [20028 / v1.6] CRESTRON_PARITY.md Task A - every reply here is fixed
    # shape, no parameters, so a straight name lookup covers it like the
    # inquiries above.
    b"\x09\x04\x3E": "ExposureCompensationMode", b"\x09\x04\x4E": "ExposureCompensation",
    b"\x09\x04\x48": "FocusPosition", b"\xC2\x09\x02": "AutoFocusBehavior",
    b"\xC2\x09\x03": "AutoFocusSensitivity", b"\x09\x0E\x24\x26": "AutoPrivacyMode",
    b"\xC2\x09\x04": "AutoSoftwareUpdate", b"\xC2\x09\x06": "TrackingMode",
    b"\xC2\x09\x07": "TrackingProfile", b"\x09\x00\x02": "DeviceInformation",
    b"\x09\x06\x11": "PanTiltSpeedMax",
}
...
def decode(frame):
    ...
    if b[0] == 0x01:
        ...
        if b == b"\x01\x04\x18\x01":  # [20028] CAM_Focus One Push Trigger, own opcode
            return Decoded("command", "OnePushAutoFocus", "Trigger")
        if k == b"\x01\x04\x48" and n == 7:  # [20028] SetFocusPosition, 4 nibbles
            return Decoded("command", "FocusPosition", "%d" % nibbles_value(b[3:7]))
        if k == b"\x01\x04\x4E" and n == 7 and b[3] == 0x00 and b[4] == 0x00:
            # [20028] SetExposureCompensation: 00 00 {Y2} {Y1}, 2 nibbles only.
            return Decoded("command", "ExposureCompensation", "%d" % nibbles_value(b[5:7]))
        if b[:4] == b"\x01\x0E\x24\x26" and n == 6:  # [20028] SetAutoPrivacyMode
            return Decoded("command", "AutoPrivacyMode", _pick(BINARY_ON_OFF, b[5]))
    ...
    if n == 4 and k == b"\xC2\x01\x02":  # [20028] SetAutoFocusBehavior
        return Decoded("command", "AutoFocusBehavior", _pick(AUTO_FOCUS_BEHAVIOR, b[3]))
    if n == 4 and k == b"\xC2\x01\x03":  # [20028] SetAutoFocusSensitivity, raw 1-3
        return Decoded("command", "AutoFocusSensitivity",
                       "%d" % b[3] if 1 <= b[3] <= 3 else "?%02X" % b[3])
    if n == 4 and k == b"\xC2\x01\x04":  # [20028] SetAutoSoftwareUpdate
        return Decoded("command", "AutoSoftwareUpdate", _pick(BINARY_ON_OFF, b[3]))
...
class CameraModel(object):
    ...
    def __init__(self):
        ...
        self.state = {
            ...
            # [20028 / v1.6] Live statuses new in this build. Every default
            # below is this listener's own starting point, not documented
            # camera behaviour - nothing in the Crestron pages says what an
            # i20 powers on with.
            "tracking_mode": 0x00,          # Presenter
            "tracking_profile": 1,
            "exposure_comp_mode": 0x03,     # Off
            "exposure_comp": 7,             # 0 EV (doc table: 0x07 = 0 EV)
            "focus_position": 16444,        # midpoint of the driver's 12224-20664 union
            "auto_focus_behavior": 0x00,    # Global
            "auto_focus_sensitivity": 2,
            "auto_privacy_mode": 0x00,      # Off
            "auto_software_update": 0x00,   # Off
        }
    ...
    def _apply(self, b):
        ...
        one = {b"\x01\x04\x00": "power", b"\x01\x04\x38": "autofocus",
               ...
               b"\x01\x04\x3E": "exposure_comp_mode"}  # [20028]
        if n == 4 and k in one:
            s[one[k]] = b[3]
        ...
        elif k == b"\x01\x04\x48" and n == 7:  # [20028] SetFocusPosition
            s["focus_position"] = nibbles_value(b[3:7])
        elif k == b"\x01\x04\x4E" and n == 7 and b[3] == 0x00 and b[4] == 0x00:
            s["exposure_comp"] = nibbles_value(b[5:7])  # [20028] SetExposureCompensation
        elif b[:4] == b"\x01\x0E\x24\x26" and n == 6:  # [20028] SetAutoPrivacyMode
            s["auto_privacy_mode"] = b[5]
        elif k == b"\x01\x06\x02" and n == 13:
            s["pan"] = signed16(nibbles_value(b[5:9]))
            s["tilt"] = signed16(nibbles_value(b[9:13]))
        elif k == b"\x01\x04\x3F" and n == 5 and b[3] == 0x02:
            preset = b[4]
            if preset in (0x50, 0x51):
                s["tracking"] = 0x02 if preset == 0x50 else 0x03
            elif preset in (0x52, 0x53):
                # [20028] Reserved-preset recall, not a dedicated opcode
                # (build_i20.py SetTrackingMode). Which preset means which
                # name is unresolved on hardware - PROTOCOL.md T3b, and
                # build_i20.py's "DOCS AND IMPLEMENTATION DISAGREE ON 0x53"
                # note. This listener follows the driver's own reading (the
                # same one GetGroupTracking's reply rule assumes), which is
                # this listener's choice, not a documented camera behaviour.
                s["tracking_mode"] = 0x01 if preset == 0x52 else 0x00
            elif 0x69 <= preset <= 0x6C:  # [20028] TrackingProfile 1-4
                s["tracking_profile"] = preset - 0x68
        elif k == b"\xC2\x01\x02" and n == 4:  # [20028] SetAutoFocusBehavior
            s["auto_focus_behavior"] = b[3]
        elif k == b"\xC2\x01\x03" and n == 4:  # [20028] SetAutoFocusSensitivity
            s["auto_focus_sensitivity"] = b[3]
        elif k == b"\xC2\x01\x04" and n == 4:  # [20028] SetAutoSoftwareUpdate
            s["auto_software_update"] = b[3]
        elif k == b"\xC2\x01\x08" and n == 4:
            if b[3] == 0:
                s["switching"] = True
            else:
                s["output"] = b[3]
                # [20028] Calling a camera output (Z 1-5) does not, per
                # VISCA-Intelligent-Switching-Commands.md, say whether it
                # also changes the Intelligent Switching on/off flag - only
                # Resume and Pause are documented to do that. This listener
                # leaves "switching" unchanged here; that is this listener's
                # own choice, not a documented camera behaviour.
    ...
    def _inquiry(self, b):
        ...
        one = {b"\x09\x04\x00": "power", b"\x09\x04\x38": "autofocus",
               ...
               b"\x09\x04\x3E": "exposure_comp_mode"}  # [20028] y0 50 [02-03] FF
        if b in one:
        ...
        # [20028 / v1.6] CRESTRON_PARITY.md Task A - fixed reply shapes below.
        if b == b"\x09\x04\x4E":  # GetExposureCompensation: y0 50 00 00 0p 0q FF
            return bytes([0x00, 0x00]) + bytes(to_nibbles(s["exposure_comp"], 2))
        if b == b"\x09\x04\x48":  # GetFocusPosition: y0 50 0p 0q 0r 0s FF
            return bytes(to_nibbles(s["focus_position"], 4))
        if b == b"\xC2\x09\x02":  # GetAutoFocusBehavior: y0 50 00 0v FF
            return bytes([0x00, s["auto_focus_behavior"]])
        if b == b"\xC2\x09\x03":  # GetAutoFocusSensitivity: y0 50 00 0v FF
            return bytes([0x00, s["auto_focus_sensitivity"]])
        if b == b"\x09\x0E\x24\x26":  # GetAutoPrivacyMode: y0 50 00 0v FF
            return bytes([0x00, s["auto_privacy_mode"]])
        if b == b"\xC2\x09\x04":  # GetAutoSoftwareUpdate: y0 50 00 0v FF
            return bytes([0x00, s["auto_software_update"]])
        if b == b"\xC2\x09\x06":  # GetGroupTracking: y0 50 00 0v FF, v 01 group/00 presenter
            return bytes([0x00, s["tracking_mode"]])
        if b == b"\xC2\x09\x07":  # GetTrackingFramingProfile: y0 50 06 0x FF, x 9-C
            return bytes([0x06, (0x68 + s["tracking_profile"]) & 0x0F])
        if b == b"\x09\x00\x02":
            # GetDeviceInformation: y0 50 00 01 mn pq rs tu vw FF. Model code
            # 05 05 = IV-CAM-I20 - fixed, since this listener only ever
            # stands in for an I20. ROM version (rs tu = 01 00) and socket
            # (vw = 01) are this listener's own invention: FormatRomVersion's
            # real byte layout is undocumented IL (CRESTRON_PARITY.md's
            # GetDeviceInformation row).
            return bytes([0x00, 0x01, 0x05, 0x05, 0x01, 0x00, 0x01])
        if b == b"\x09\x06\x11":
            # GetPanTiltSpeedMax: y0 50 ww zz FF, one whole byte per axis (not
            # nibbles). 0x18 / 0x14 are the maxima SetPanTiltAngle accepts.
            return bytes([0x18, 0x14])
    ...
    def describe(self):
        ...
        return ("power %s, tracking %s, freeze %s, zoom %d, pan %d, tilt %d, "
                ...
                "tracking mode %s, tracking profile %d, exp comp mode %s, "
                "exp comp %d, focus %d, af behavior %s, af sensitivity %d, "
                "auto privacy %s, auto software update %s"
                % (ON_OFF[s["power"]], {0x02: "start", 0x03: "stop"}[s["tracking"]],
                   ...
                   {0x01: "group", 0x00: "presenter"}[s["tracking_mode"]],
                   s["tracking_profile"], ON_OFF[s["exposure_comp_mode"]],
                   s["exposure_comp"], s["focus_position"],
                   AUTO_FOCUS_BEHAVIOR[s["auto_focus_behavior"]],
                   s["auto_focus_sensitivity"], BINARY_ON_OFF[s["auto_privacy_mode"]],
                   BINARY_ON_OFF[s["auto_software_update"]]))
    ...
    def control(self, line):
        ...
        with self.lock:
            try:
                ...
                elif w == "trackingmode":       # [20028]
                    s["tracking_mode"] = {"group": 0x01, "presenter": 0x00}[words[1]]
                elif w == "trackingprofile":    # [20028]
                    s["tracking_profile"] = int(words[1])
                elif w == "expcompmode":        # [20028]
                    s["exposure_comp_mode"] = {"on": 0x02, "off": 0x03}[words[1]]
                elif w == "expcomp":            # [20028]
                    s["exposure_comp"] = int(words[1])
                elif w == "focus":              # [20028]
                    s["focus_position"] = int(words[1])
                elif w == "afbehavior":         # [20028]
                    s["auto_focus_behavior"] = {"global": 0x00, "center": 0x01,
                                                "face": 0x04}[words[1]]
                elif w == "afsensitivity":      # [20028]
                    s["auto_focus_sensitivity"] = int(words[1])
                elif w == "autoprivacy":        # [20028]
                    s["auto_privacy_mode"] = {"on": 0x01, "off": 0x00}[words[1]]
                elif w == "autosoftwareupdate": # [20028]
                    s["auto_software_update"] = {"on": 0x01, "off": 0x00}[words[1]]
                elif w != "state":
