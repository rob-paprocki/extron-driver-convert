...
|---|---|---|
| driver under test | `onebynd_camera_IV_CAM_I20_v1_0_0_0.py` | `1bynd_19_20028`, the synthesised `.pkp` |
...
1. **Driver.** Install `experiments/skeleton_i20/out/1bynd_19_20028_v1_0_0.pkp`
   ...
   Profile and Intelligent Switching live statuses too.
2. **Project.** In a new Pro project:
   - add the processor as the controller, by its address and web credentials;
   - on one of its Ethernet ports add *1 Beyond / IV-CAM-I20 v1.6*, addressed to
...
- **Any `wire DIFF`.** The processor sent bytes the module does not send under
  CPython - except the one explained shape `--reply full` can produce, next.
- **Any `read DIFF` against `--reply ack`.** The same reply parsed differently on
  the processor.
- **`--reply full`.** Offline, a runtime that keeps unread data diverges on 23 of
  the 54 reads (`test_run_loopback.py` [4]): each Completion lands on the next
  call. The same staleness can also cost a *wire* frame: a status that shares
  its request with another (Camera Output/Intelligent Switching, Pan/Tilt
  angle, Device Model/ROM Version, Pan/Tilt Speed Max, from 20028) only caches
  a reply it parses without error, so a stale read that raises leaves the next
  status in the pair to send its own, uncached, request - one genuine extra
  frame, which `test_run_loopback.py` [4] tells apart from a real mismatch. If
  a processor does the same, every driver built on this template misreads a
  camera that sends both. Worth a finding either way.
...
- **Some commands share their bytes.** Every named reserved preset is a
  `Preset Recall` of the same number (`Preset Recall 1` is
  `TrackingShot Tracking`), and before `20027` (v1.5) `CameraOutput 0` was
  `IntelligentSwitching Resume`. A capture cannot tell such a pair apart, so the
  decoder names both. The Camera Output overlap is gone from 20027, which
  offers 1–5 only; the decoder keeps both names for captures from older
  packages. The preset overlap stays by design: naming the reserved presets is
  most of what the driver adds.
