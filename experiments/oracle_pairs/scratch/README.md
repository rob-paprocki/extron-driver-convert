...
duplicated here. *(Since then `pair_index.json` has been regenerated by
`../build_index.py` (R39). `residuals.json` is left as the first run's tally
(2026-09-08); the current per-package tally is each row's `residual_reasons` in
`../out/scorecard.json`, re-scored 2026-09-23.)*
