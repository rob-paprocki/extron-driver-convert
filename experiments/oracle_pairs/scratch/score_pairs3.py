...
OUT = os.path.join(here, "scorecard3.json")
...
for i, p in enumerate(pairs, 1):
    ...
    try:
        ...
        rec["only_generated"] = len(d["only_in_a"])
        rec["only_shipped"] = len(d["only_in_b"])
        rec["shared"] = len(d["shared"])
        rec["differing"] = len(d["differences"])
...
print("  packages w/ residuals: %d of %d" % (sum(1 for r in rows if r.get("residuals")), len(rows)))
