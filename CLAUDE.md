...
**628 tests, all standard library, no pytest.** Run each file directly; the
...
- **Does a *synthesised* `.pkp` load?** Needs Windows *and* Global Configurator.
  Finding 12 showed GC ingests a package our writer produced, and one we mutated;
  finding 18 took a graph we assembled from scratch all the way into the GCP
  editor with its full command surface rendered. **Build, upload and control are
  what remain** — nothing has run on a processor.
