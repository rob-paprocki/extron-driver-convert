...
All four conversion directions work offline, and 518 standard-library tests pass. The i20 driver exists in two forms:
...
| pan/tilt | `81 09 06 12 FF` | nibble layout | `PanAngleStatus` / `TiltAngleStatus` (one shared inquiry) |
| camera output | `81 C2 09 08 FF` | reply undocumented ("see below", then nothing); parser assumes `res[2] & 0x0F` | CameraOutput parser |
...
| D7 | **GCP licence: what is the real term?** Read again 2026-09-13 on a different install, two days after it was installed: the status bar says "Expires in 30 Days" — the fourth reading of exactly 30, after 09-07, 09-09 and 09-10. A counter that never moves is not a countdown, so the ~2026-10-07 date was never supported. A rolling window renewed on sign-in would fit, but that is inferred. | A licensed GCP is the gate for H0, H4 and the render check. | Open GCP's licence / account dialog and record what it says the term is in §6. | §6 | S |
...
| GCP licence expiry | Unknown. The counter read "30 Days" on 09-07, 09-09, 09-10 and, on another install, 09-13, so it does not count down | §6, D7 |
...
|---|---|---|---|
| **GCP Pro licence** | The status bar read "Expires in 30 Days" on 09-07, 09-09, 09-10 and — on a different install — 09-13. A counter that never moves is not a countdown, so **no expiry date is known**. `user.config` holds licence records but no readable date. | Read the term from GCP's licence / account dialog and record it here, without copying licence data into git. Until then, treat the licence as neither a deadline nor safe. | S |
