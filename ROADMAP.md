...
All four conversion directions work offline, and 623 standard-library tests pass. The i20 driver exists in two forms:
...
| H1L | **Processor loopback, no camera:** a PC drives the i20 module on the processor through a command console, plays the camera, and checks every frame and every read against the module run locally | an Extron processor on the PC's network (**cabled 2026-09-13**: an IPCP Pro 360, PC on its AV LAN). Path A also needs **the ControlScript Deployment Utility and a certified account to certify the project; neither is on this workstation** | Path A: `controlscript/make_project.py --address <processor>`; certify and deploy `build/i20-loopback/`; `run_loopback.py --processor <processor>`, then with `--reply full`. Path B: the H0 project addressed to the PC, driven from a GUI Designer layout, captured by `visca_listener.py`. | Every step's `wire` and `read` result (report JSON). Whether reads survive `--reply full`. The processor's ControlScript and Python versions (`processor_info`). | `experiments/loopback/README.md`; covers H1, and H4 minus the camera | S |
